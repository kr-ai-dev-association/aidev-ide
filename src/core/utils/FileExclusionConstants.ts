...
export function getAllExclusionPaths(): string[] {
  try {
    // 기본 패턴에서 비활성화된 것 제거
    const base = EXCLUDED_LIBRARY_PATHS.filter(
      (p) => !_cachedDisabledPatterns.includes(p),
    );
 ...
 */
export function loadCustomExclusionPatterns(
  context: vscode.ExtensionContext,
): void {
  _cachedCustomPatterns =
    context.globalState.get<string[]>("contextExclusionPatterns", []) || [];
  _cachedDisabledPatterns =
    context.globalState.get<string[]>("contextExclusionDisabled", []) || [];
...
export async function loadServerExcludePatterns(): Promise<void> {
  try {
    ...
    const serverPatterns =
      SettingsManager.getInstance().getServerExcludePatterns();
