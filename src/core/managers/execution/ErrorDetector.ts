...
import { ErrorInfo, ErrorType, ErrorDetails } from './types';
...
export class ErrorDetector {
     ...
     * 출력에서 에러를 감지합니다
     */
    public detectError(output: string, source: 'terminal' | 'process' | 'system' = 'terminal'): ErrorInfo | null {
     ...
     * 에러 메시지를 추출합니다
     */
    private extractMessage(match: RegExpMatchArray | null, output: string): string {
        if (match && match[0]) {
            return match[0].trim();
        }
     ...
     * 심각도를 결정합니다
     */
    private determineSeverity(type: ErrorType): 'low' | 'medium' | 'high' | 'critical' {
     ...
     * 수정 제안을 생성합니다
     */
    private getSuggestion(type: ErrorType, details: ErrorDetails): string | undefined {
        switch (type) {
     ...
     */
    private hasGeneralErrorKeywords(output: string): boolean {
     ...
     * 에러 패턴을 추가합니다
     */
    public addPattern(type: ErrorType, patterns: RegExp[]): void {
     ...
     * 커스텀 패턴을 등록합니다
     */
    public registerCustomPattern(type: ErrorType, pattern: RegExp): void {
