...
export class SessionManager {
  ...
  public createSession(projectPath: string): Session {
    ...
    console.log(
      `[SessionManager] Created session: ${sessionId} for ${projectPath}`,
    );
  ...
  public async addConversationEntry(
    ...
    console.log(
      `[SessionManager] Conversation entry saved. Total entries: ${session.conversationHistory.length}`,
    );
  }
  ...
  public clearConversationHistory(sessionId?: string): void {
    ...
    this.saveSessions();
    console.log(
      `[SessionManager] Cleared conversation history for session: ${session.id}`,
    );
  }
   ...
   */
  public wasLastSessionInterrupted(): {
    interrupted: boolean;
    ...
    const lastEntry =
      currentSession.conversationHistory[
        currentSession.conversationHistory.length - 1
      ];
    if (!lastEntry || !lastEntry.userRequest) {
      return { interrupted: false };
    }

    // 정상 종료 케이스 (result === "success") — assistantResponse 가 빈 문자열이어도
    // interrupted 아님. AGENT 모드 text-only 종료처럼 finalSummary 가 빈 문자열로 저장되는
    // 합법적인 케이스를 인터럽트로 오감지하던 버그 차단.
    if (lastEntry.result === "success") {
      return { interrupted: false };
    }

    // 응답이 명시적으로 누락 (undefined 또는 빈 문자열) + success 마커 없음 → 진짜 인터럽트
  ...
  public addTokensUsed(tokens: number): void {
    ...
    this.saveSessions();
    console.log(
      `[SessionManager] Added ${tokens} tokens. Total: ${session.totalTokensUsed}`,
    );
  }
  ...
  public setTotalTokensUsed(tokens: number): void {
    ...
    this.saveSessions();
    console.log(
      `[SessionManager] Token usage set: ${previousTokens} -> ${tokens}`,
    );
  }
  ...
  public trimSessionHistory(keepRecentCount: number = 20): void {
    ...
    console.log(
      `[SessionManager] Session trimmed. Removed ${removedCount} old entries, kept ${keepRecentCount}`,
    );
  }
  ...
  public addCompactedSummary(sessionId: string, summary: string): void {
    ...
    console.log(
      `[SessionManager] Added compacted summary to session ${sessionId}`,
    );
  }
  ...
  private async compactSessionHistory(maxTokens: number): Promise<void> {
    ...
    try {
      ...
      const conversationText = oldEntries
        .map((entry) => {
          const files = [
            ...(entry.filesCreated || []),
            ...(entry.filesModified || []),
      ...
      const summaryResult =
        await this.compactorInstance.generateSummaryFromText(conversationText);
      ...
      this.saveSessions();
      console.log(
        `[SessionManager] Session compacted. ${oldEntries.length} entries summarized, ${keepRecentCount} kept. Tokens: ${compactedTokens}`,
      );
    } catch (error) {
      console.error(
        "[SessionManager] Failed to compact session history:",
        error,
      );
   ...
   * 엔트리에서 파일 목록 추출
   */
  private extractFilesFromEntries(
    ...
    for (const entry of entries) {
      const fileList =
        type === "created" ? entry.filesCreated : entry.filesModified;
      if (fileList) {
