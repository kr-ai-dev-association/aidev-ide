...
export class StateManager {
  ...
  public static getInstance(context?: vscode.ExtensionContext): StateManager {
    if (!StateManager.instance) {
      if (!context) {
        throw new Error(
          "StateManager requires ExtensionContext for first initialization",
        );
      }
  ...
  private loadState(): void {
    try {
      const stored = this.context.globalState.get<GlobalState>(
  ...
  private saveState(): void {
    try {
      this.context.globalState.update("agentgocoder.globalState", this.state);
    } catch (error) {
  ...
  // API 키 및 모델 관련 키
  private readonly API_KEY_SECRET_KEY = "agentgocoder.geminiApiKey";
  private readonly CURRENT_AI_MODEL_SECRET_KEY = "agentgocoder.currentAiModel";
  private readonly OLLAMA_SERVER_TYPE_SECRET_KEY =
    "agentgocoder.ollamaServerType";
  private readonly OLLAMA_API_URL_SECRET_KEY = "agentgocoder.ollamaApiUrl";
  private readonly OLLAMA_ENDPOINT_SECRET_KEY = "agentgocoder.ollamaEndpoint";
  private readonly OLLAMA_MODEL_SECRET_KEY = "agentgocoder.ollamaModel";
  private readonly LOCAL_OLLAMA_API_URL_SECRET_KEY =
    "agentgocoder.localOllamaApiUrl";
  private readonly LOCAL_OLLAMA_ENDPOINT_SECRET_KEY =
    "agentgocoder.localOllamaEndpoint";
  private readonly REMOTE_OLLAMA_API_URL_SECRET_KEY =
    "agentgocoder.remoteOllamaApiUrl";
  private readonly REMOTE_OLLAMA_ENDPOINT_SECRET_KEY =
    "agentgocoder.remoteOllamaEndpoint";
  private readonly REMOTE_OLLAMA_MODEL_SECRET_KEY =
  ...
  private readonly COMPACTOR_MODEL_NAME_KEY = "agentgocoder.compactorModelName";
  private readonly COMPACTOR_API_KEY_KEY = "agentgocoder.compactorApiKey";
  private readonly COMMAND_MODEL_TYPE_KEY = "agentgocoder.commandModelType";
  private readonly COMMAND_MODEL_NAME_KEY = "agentgocoder.commandModelName";
  private readonly COMMAND_API_KEY_KEY = "agentgocoder.commandApiKey";
  private readonly INTENT_MODEL_TYPE_KEY = "agentgocoder.intentModelType";
  private readonly INTENT_MODEL_NAME_KEY = "agentgocoder.intentModelName";
  private readonly INTENT_API_KEY_KEY = "agentgocoder.intentApiKey";
  private readonly ERROR_FALLBACK_MODEL_TYPE_KEY =
    "agentgocoder.errorFallbackModelType";
  private readonly ERROR_FALLBACK_MODEL_NAME_KEY =
  ...
  private readonly COMPACTOR_ADMIN_CONFIG_KEY =
    "agentgocoder.compactorAdminConfig";
  private readonly COMMAND_ADMIN_CONFIG_KEY = "agentgocoder.commandAdminConfig";
  ...
  public async getOllamaServerType(): Promise<string> {
    return (
      (await this.getSecret(this.OLLAMA_SERVER_TYPE_SECRET_KEY)) || "local"
    );
  }
  ...
  public async getOllamaEndpoint(): Promise<string> {
    return (
      (await this.getSecret(this.OLLAMA_ENDPOINT_SECRET_KEY)) || "/api/generate"
    );
  }
  ...
  public async getOllamaModel(): Promise<string> {
    return (
      (await this.getSecret(this.OLLAMA_MODEL_SECRET_KEY)) || "gemini-2.5-flash"
    );
  }
  ...
  public async getLocalOllamaApiUrl(): Promise<string> {
    return (
      (await this.getSecret(this.LOCAL_OLLAMA_API_URL_SECRET_KEY)) ||
      DEFAULT_OLLAMA_URL
    );
  }
  ...
  public async getLocalOllamaEndpoint(): Promise<string> {
    return (
      (await this.getSecret(this.LOCAL_OLLAMA_ENDPOINT_SECRET_KEY)) ||
      "/api/generate"
    );
  }
  ...
  public async getRemoteOllamaApiUrl(): Promise<string | null> {
    return (
      (await this.getSecret(this.REMOTE_OLLAMA_API_URL_SECRET_KEY)) || null
    );
  }
  ...
  public async getRemoteOllamaEndpoint(): Promise<string> {
    return (
      (await this.getSecret(this.REMOTE_OLLAMA_ENDPOINT_SECRET_KEY)) ||
      "/api/generate"
    );
  }
  ...
  public async getAiModel(): Promise<string> {
    return (await this.getSecret("agentgocoder.aiModel")) || "ollama";
  }

  public async saveAiModel(model: string): Promise<void> {
    await this.saveSecret("agentgocoder.aiModel", model);
  }

  public async getAdminModelConfig(): Promise<string | undefined> {
    return await this.getSecret("agentgocoder.adminModelConfig");
  }

  public async saveAdminModelConfig(configJson: string): Promise<void> {
    await this.saveSecret("agentgocoder.adminModelConfig", configJson);
  }
  ...
  // ===== AgentPolicy 관련 메서드들 =====
  private readonly AGENT_POLICY_STABLE_VERSION_KEY =
    "agentgocoder.agentPolicy.stableVersion";
  private readonly AGENT_POLICY_CODING_STYLE_KEY =
    "agentgocoder.agentPolicy.codingStyle";
  private readonly AGENT_POLICY_PROJECT_ARCHITECTURE_KEY =
    "agentgocoder.agentPolicy.projectArchitecture";
  private readonly AGENT_POLICY_DEPENDENCY_POLICY_KEY =
    "agentgocoder.agentPolicy.dependencyPolicy";
  private readonly AGENT_POLICY_DB_POLICY_KEY =
    "agentgocoder.agentPolicy.dbPolicy";
  ...
  public async saveAgentPolicyStableVersion(mdContent: string): Promise<void> {
    await this.context.workspaceState.update(
      this.AGENT_POLICY_STABLE_VERSION_KEY,
      mdContent,
    );
  }
  ...
  public async saveAgentPolicyCodingStyle(mdContent: string): Promise<void> {
    await this.context.workspaceState.update(
      this.AGENT_POLICY_CODING_STYLE_KEY,
      mdContent,
    );
  }
   ...
   */
  public async saveAgentPolicyProjectArchitecture(
    mdContent: string,
  ): Promise<void> {
    await this.context.workspaceState.update(
      this.AGENT_POLICY_PROJECT_ARCHITECTURE_KEY,
      mdContent,
    );
  }
   ...
   */
  public async getAgentPolicyProjectArchitecture(): Promise<
    string | undefined
  > {
    return this.context.workspaceState.get<string>(
      this.AGENT_POLICY_PROJECT_ARCHITECTURE_KEY,
   ...
   */
  public async saveAgentPolicyDependencyPolicy(
    mdContent: string,
  ): Promise<void> {
    await this.context.workspaceState.update(
      this.AGENT_POLICY_DEPENDENCY_POLICY_KEY,
      mdContent,
    );
  }
  ...
  public async saveAgentPolicyDbPolicy(mdContent: string): Promise<void> {
    await this.context.workspaceState.update(
      this.AGENT_POLICY_DB_POLICY_KEY,
      mdContent,
    );
  }
   ...
   */
  public async getCompactorModelConfig(): Promise<{
   ...
   * @returns { type: string | undefined, name: string | undefined }
   */
  public async getCommandModelConfig(): Promise<{
    type: string | undefined;
    name: string | undefined;
  }> {
    const [type, name] = await Promise.all([
   ...
   */
  public async saveCompactorModelConfig(
    type: string,
    name: string,
  ): Promise<void> {
    await Promise.all([
   ...
   */
  public async saveCommandModelConfig(
    type: string,
    name: string,
  ): Promise<void> {
    await Promise.all([
   ...
   */
  public async getIntentModelConfig(): Promise<{
    type: string | undefined;
    name: string | undefined;
  }> {
    const [type, name] = await Promise.all([
   ...
   */
  public async saveIntentModelConfig(
    type: string,
    name: string,
  ): Promise<void> {
    await Promise.all([
  ...
  public async saveErrorFallbackModelConfig(
    type: string,
    name: string,
  ): Promise<void> {
    await Promise.all([
