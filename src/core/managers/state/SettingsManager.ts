...
export class SettingsManager extends BaseManager {
  ...
  private readonly OFFLINE_CACHE_KEY = "agentgocoder.serverSettingsCache";
  private readonly DISABLED_SETTINGS_KEY =
    "agentgocoder.disabledRecommendedSettings";
  ...
  private registerVSCodeSettingsWatcher(): void {
    vscode.workspace.onDidChangeConfiguration((event) => {
      if (event.affectsConfiguration("agentgocoder")) {
        // 설정 변경 시 캐시 무효화
        ConfigurationService.invalidateCache();
        // 모든 agentgocoder 설정이 변경되었음을 알림
