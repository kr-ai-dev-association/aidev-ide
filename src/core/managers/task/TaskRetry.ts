...
export class TaskRetry {
     ...
     */
    public canRetry(task: Task, strategy?: RetryStrategy): boolean {
     ...
     */
    public calculateDelay(task: Task, strategy?: RetryStrategy): number {
        const retryStrategy = strategy || this.defaultStrategy;
        const retryCount = task.metadata?.retryCount || 0;
     ...
     * 작업을 재시도 준비 상태로 만듭니다
     */
    public prepareForRetry(task: Task, strategy?: RetryStrategy): {
        shouldRetry: boolean;
        delay: number;
    } {
     ...
     * 기본 재시도 전략을 설정합니다
     */
    public setDefaultStrategy(strategy: RetryStrategy): void {
     ...
     * 작업의 재시도 정보를 가져옵니다
     */
    public getRetryInfo(task: Task, strategy?: RetryStrategy): {
        canRetry: boolean;
        retryCount: number;
        maxRetries: number;
        nextDelay: number;
