...
export class TaskManager {
  ...
  private static PLAN_QUEUES_STORAGE_KEY = "agentgocoder.planQueues.v2";
  ...
  public enqueue(
    ...
    console.log(
      `[TaskManager] Enqueued task: ${taskId} (${type}, priority: ${task.priority})`,
    );
  ...
  public updateStatus(taskId: string, status: TaskStatus): void {
    ...
    console.log(
      `[TaskManager] Task ${taskId} status: ${oldStatus} -> ${status}`,
    );
  }
  ...
  public getStats(): TaskStats {
    ...
    const successRate =
      totalFinished > 0 ? completed.length / totalFinished : 0;

    // 평균 duration 계산
    const tasksWithDuration = allTasks.filter(
      (t) => t.result?.duration !== undefined,
    );
  ...
  private loadPlanQueues(): void {
    ...
    const stored = this.context.globalState.get<any>(
      TaskManager.PLAN_QUEUES_STORAGE_KEY,
    );
    if (
      ...
      const legacy = this.context.globalState.get<PlanItem[]>(
        "agentgocoder.planQueue",
        [],
      );
    ...
    if (!this.currentPlanQueueId) {
      this.currentPlanQueueId =
        this.planQueues.length > 0
          ? this.planQueues[this.planQueues.length - 1].id
          : undefined;
    }
  }
   ...
   */
  public enqueuePlanItems(
    items: Omit<PlanItem, "id" | "status" | "createdAt">[],
    defaultStatus: PlanItemStatus = "pending",
  ): number {
  ...
  public getPlanQueue(queueId?: string): PlanItem[] {
    const q = this.planQueues.find(
      (q) => q.id === (queueId || this.currentPlanQueueId),
    );
    return q ? [...q.items] : [];
  }
   ...
   */
  public createPlanQueue(
    title: string,
    initialItems: Omit<PlanItem, "id" | "status" | "createdAt">[] = [],
    defaultStatus: PlanItemStatus = "pending",
  ): string {
    const id =
   ...
   */
  public updatePlanItemStatusInQueue(
    queueId: string,
    id: string,
    status: PlanItemStatus,
  ): void {
  ...
  public findMatchingPlanQueue(
    ...
    for (let i = this.planQueues.length - 1; i >= 0; i--) {
      ...
      const text = (
        q.title +
        " " +
        q.items.map((it) => it.title).join(" ")
      ).toLowerCase();
      const matched = titles.some(
        (t) =>
          t.length > 0 && text.includes(t.slice(0, Math.min(20, t.length))),
      );
   ...
   */
  private addItemsToPlanQueue(
    queue: PlanQueue,
    items: Omit<PlanItem, "id" | "status" | "createdAt">[],
    defaultStatus: PlanItemStatus,
  ): number {
    const now = Date.now();
  ...
  public getNextPendingItem(): PlanItem | undefined {
    ...
    const inProgress = queue.items.find(
