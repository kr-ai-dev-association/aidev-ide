...
export class LLMManager {
  ...
  /** thinking 비활성화 여부 결정 */
  private static resolveDisableThinking(
    systemPrompt: string,
    explicit?: boolean,
  ...
  public async getCurrentModelName(): Promise<string> {
    try {
      if (this.currentModelType === AiModelType.ADMIN) {
        ...
        return (
          this.ollamaApi.getModel?.() ||
          this.ollamaApi.getCurrentModelName?.() ||
          "Ollama Model"
        );
      }
    } catch {}
    ...
    try {
      ...
      const result = await withRetry(
        ...
        (attempt, error, delayMs) => {
          console.log(
            `[LLMManager] sendMessage retry ${attempt + 1}: ${error.message.substring(0, 100)} (waiting ${delayMs}ms)`,
          );
      ...
      if (result.success && result.result !== undefined) {
        if (result.attempts > 1) {
          console.log(
            `[LLMManager] sendMessage succeeded after ${result.attempts} attempts (total delay: ${result.totalDelayMs}ms)`,
          );
        }
  ...
  public async sendMessageWithSystemPrompt(
    ...
    const disableThinking = LLMManager.resolveDisableThinking(
      systemPrompt,
    ...
    try {
      ...
      const result = await withRetry(
        ...
        (attempt, error, delayMs) => {
          console.log(
            `[LLMManager] sendMessageWithSystemPrompt retry ${attempt + 1}: ${error.message.substring(0, 100)} (waiting ${delayMs}ms)`,
          );
          if (attempt >= HIDDEN_RETRY_THRESHOLD && options?.onRetryNotify) {
            options.onRetryNotify(attempt, getRetryUserMessage(error, delayMs));
          }
      ...
      if (result.success && result.result !== undefined) {
        if (result.attempts > 1) {
          console.log(
            `[LLMManager] sendMessageWithSystemPrompt succeeded after ${result.attempts} attempts (total delay: ${result.totalDelayMs}ms)`,
          );
        }
        return result.result;
      }
      ...
      console.error(
        "[LLMManager] Failed to send message with system prompt:",
        error,
      );
  ...
  public extractResponseText(llmResponse: string): string {
    ...
    text = text.replace(
      /---\s*(?:작업 요약|Summary)\s*---[\s\S]*?---\s*(?:작업 수행 설명|Description)\s*---[\s\S]*/gi,
      "",
    );
    text = text.replace(/---\s*(?:작업 요약|Summary)\s*---[\s\S]*/gi, "");
    text = text.replace(
      /---\s*(?:작업 수행 설명|Description)\s*---[\s\S]*/gi,
      "",
    );
  ...
  private removeBashCommands(response: string): string {
    return response.replace(
      /```(?:bash|sh|shell|powershell|ps1|pwsh|cmd|batch|bat)[\s\S]*?```/gi,
      "",
    );
  }
  ...
  private removeFileDirectives(response: string): string {
    return response
      .replace(
        /(새 파일|수정 파일|삭제 파일|New file|Create file|Modified file|Update file|Modify file|Delete file|Remove file):[\s\S]*?(?=\n{2,}|$)/gi,
        "",
      )
      .trim();
  }
   ...
   */
  public static formatErrorForChat(evt: {
    time: number;
    source: string;
    message: string;
    recentLogs: any[];
    ...
    const tail =
      evt.recentLogs && evt.recentLogs.length > 0
        ? "\n\n최근 로그 (최대 10줄):\n" +
          evt.recentLogs
            .slice(-10)
            .map((l: any) => `- ${l.message || l.rawOutput || ""}`)
            .join("\n")
        : "";
    return header + tail;
  }
  ...
  public async sendMessageWithSpecificModel(
    ...
    try {
      ...
      const isAdminRouting =
        modelType === AiModelType.ADMIN ||
      ...
      console.error(
        "[LLMManager] Failed to send message with specific model:",
        error,
      );
    ...
    if (modelType !== AiModelType.OLLAMA) {
      ...
      if (adminConfigJson) {
        try {
          ...
          return await this.sendMessageWithAdminConfigSwap(
            adminConfig,
            systemPrompt,
            userParts,
            options,
  ...
  public async sendMessageWithCommandModelStreaming(
    ...
    if (!modelType) {
      return this.sendMessageWithSystemPromptStreaming(
        systemPrompt,
        userParts,
        onChunk,
        options,
      ...
      if (adminConfigJson) {
        try {
          ...
          const routingApiKey = stateManager.getCommandApiKey
            ...
            : undefined;
          if (routingApiKey) adminConfig.apiKey = routingApiKey;
          return await this.sendMessageWithAdminConfigSwapStreaming(
            adminConfig,
            systemPrompt,
            userParts,
            onChunk,
  ...
  public async sendMessageWithSystemPromptStreaming(
    ...
    const apiCall = async (): Promise<string> => {
      ...
      if (this.currentModelType === AiModelType.ADMIN) {
        response =
          await this.adminModelApi.sendMessageWithSystemPromptStreaming(
            systemPrompt,
            LLMManager.normalizeParts(userParts),
            onChunk,
            {
    ...
    try {
      ...
      const result = await withRetry(
        ...
        (attempt, error, delayMs) => {
          console.log(
            `[LLMManager] streaming retry ${attempt + 1}: ${error.message.substring(0, 100)} (waiting ${delayMs}ms)`,
          );
          if (attempt >= HIDDEN_RETRY_THRESHOLD && options?.onRetryNotify) {
            options.onRetryNotify(attempt, getRetryUserMessage(error, delayMs));
          }
      ...
      if (result.success && result.result !== undefined) {
        if (result.attempts > 1) {
          console.log(
            `[LLMManager] streaming succeeded after ${result.attempts} attempts (total delay: ${result.totalDelayMs}ms)`,
          );
        }
        return result.result;
      }
  ...
  public async generateSimpleResponse(
    ...
    const systemPrompt =
      "간결하게 응답하세요. 요청된 형식만 출력하고 추가 설명은 하지 마세요.";
