...
import { withRetry } from './LLMRetryHelper';
export class LLMManager {
    ...
    constructor(geminiApi, ollamaApi, banyaApi, initialModelType = AiModelType.OLLAMA) {
    ...
    static getInstance(geminiApi, ollamaApi, banyaApi, initialModelType) {
    ...
    async sendMessage(prompt, options) {
        ...
        try {
            ...
            // 재시도 로직으로 감싸서 호출
            const result = await withRetry(apiCall, options?.retry, signal, (attempt, error, delayMs) => {
     ...
     * @param options 요청 옵션
     */
    async sendMessageWithSpecificModel(modelType, modelName, systemPrompt, userParts, options) {
        this.currentCallController = new AbortController();
        const signal = options?.signal || this.currentCallController.signal;
        try {
            let response;
     ...
     */
    compactorStateInterface = {
        getCompactorModelType: () => Promise.resolve(undefined),
        getCompactorModelName: () => Promise.resolve(undefined),
        getCompactorApiKey: () => Promise.resolve(undefined),
    };
    /**
     * Command 모델용 StateManager 인터페이스
     */
    commandStateInterface = {
        getCommandModelType: () => Promise.resolve(undefined),
        getCommandModelName: () => Promise.resolve(undefined),
        getCommandApiKey: () => Promise.resolve(undefined),
    };
    /**
     ...
     * 설정되지 않은 경우 메인 모델 사용
     */
    async sendMessageWithCompactorModel(systemPrompt, userParts, stateManager, options) {
        ...
        return this.sendMessageWithSpecificModelAndApiKey(modelType, modelName, apiKey, systemPrompt, userParts, options);
    }
    /**
     ...
     * 설정되지 않은 경우 메인 모델 사용
     */
    async sendMessageWithCommandModel(systemPrompt, userParts, stateManager, options) {
     ...
     * 설정되지 않은 경우 메인 모델 사용
     */
    async sendMessageWithCommandModelStreaming(systemPrompt, userParts, onChunk, stateManager, options) {
        const modelType = await stateManager.getCommandModelType();
        ...
        return this.sendMessageWithSpecificModelAndApiKeyStreaming(modelType, modelName, apiKey, systemPrompt, userParts, onChunk, options);
    }
    /**
     ...
     * 설정되지 않은 경우 메인 모델 사용
     */
    async sendMessageWithIntentModel(systemPrompt, userParts, stateManager, options) {
     ...
     */
    async sendMessageWithSpecificModelAndApiKey(modelType, modelName, apiKey, systemPrompt, userParts, options) {
        ...
        if (modelType === AiModelType.GEMINI && apiKey) {
            ...
            try {
                ...
                const response = await this.sendMessageWithSpecificModel(modelType, modelName, systemPrompt, userParts, options);
                return response;
            }
            finally {
                // 원래 설정으로 복원
        ...
        // 기본: 기존 설정으로 호출
        return this.sendMessageWithSpecificModel(modelType, modelName, systemPrompt, userParts, options);
    }
    /**
     * 특정 모델 타입과 API 키로 스트리밍 메시지를 전송합니다
     * API 키가 제공되면 임시로 해당 키를 사용하고, 없으면 기존 설정 사용
     */
    async sendMessageWithSpecificModelAndApiKeyStreaming(modelType, modelName, apiKey, systemPrompt, userParts, onChunk, options) {
        // Gemini의 경우 API 키와 모델명 임시 변경
        if (modelType === AiModelType.GEMINI && apiKey) {
            const originalApiKey = this.geminiApi.getApiKey();
            const originalModelName = this.geminiApi.getModelName();
            try {
                ...
                const response = await this.geminiApi.sendMessageWithSystemPromptStreaming(systemPrompt, parts, onChunk, { signal: options?.signal });
                return response;
            }
            finally {
                // 원래 설정으로 복원
        ...
        if (modelType === AiModelType.BANYA && apiKey) {
            ...
            try {
                ...
                const parts = userParts.map(part => ({ text: part.text || '' }));
                const response = await this.banyaApi.sendMessageWithSystemPromptStreaming(systemPrompt, parts, onChunk, { signal: options?.signal });
                return response;
            }
            finally {
                // 원래 설정으로 복원
        ...
        if (modelType === AiModelType.OLLAMA) {
            ...
            const parts = userParts.map(part => ({ text: part.text || '' }));
            return this.ollamaApi.sendMessageWithSystemPromptStreaming(systemPrompt, parts, onChunk, { signal: options?.signal });
        }
        // 기본: 메인 모델 스트리밍
        return this.sendMessageWithSystemPromptStreaming(systemPrompt, userParts, onChunk, options);
    }
    ...
    async sendMessageWithSystemPromptStreaming(systemPrompt, userParts, onChunk, options) {
        ...
        const apiCall = async () => {
            ...
            if (this.currentModelType === AiModelType.GEMINI) {
                ...
                response = await this.geminiApi.sendMessageWithSystemPromptStreaming(systemPrompt, parts, onChunk, { signal });
                // Offline fallback trigger
                if (typeof response === 'string' && response.startsWith('OFFLINE:')) {
                    try {
                        await this.ollamaApi.loadSettingsFromStorage();
                    ...
                    // Ollama로 폴백 (스트리밍)
                    const ollamaParts = userParts.map(part => ({ text: part.text || '' }));
                    response = await this.ollamaApi.sendMessageWithSystemPromptStreaming(systemPrompt, ollamaParts, onChunk, { signal });
                }
            }
            else if (this.currentModelType === AiModelType.BANYA) {
                try {
                ...
                // Banya API 형식으로 변환
                const parts = userParts.map(part => ({ text: part.text || '' }));
                response = await this.banyaApi.sendMessageWithSystemPromptStreaming(systemPrompt, parts, onChunk, { signal });
            }
            else {
                try {
                    await this.ollamaApi.loadSettingsFromStorage();
                ...
                // Ollama API 형식으로 변환
                const parts = userParts.map(part => ({ text: part.text || '' }));
                response = await this.ollamaApi.sendMessageWithSystemPromptStreaming(systemPrompt, parts, onChunk, { signal });
            }
            return response;
        };
        try {
    ...
    async generateSimpleResponse(prompt, options) {
        ...
        try {
            const response = await this.sendMessageWithSystemPrompt(systemPrompt, userParts, { maxTokens: options?.maxTokens || 50 });
