...
export class IntentDetector {
    ...
    public async detectIntent(userQuery: string, options?: { modelName?: string }): Promise<IntentDetectionResult> {
        ...
        console.log('[IntentDetector] Keyword vote result:', keywordScore);

        if (confidence < 0.6) {
            try {
                const modelForIntent = options?.modelName && options.modelName.toLowerCase().includes('gemini')
                    ? undefined // Gemini는 IntentDetector(ollama 전용)에서 사용하지 않음
                    : options?.modelName;
                ...
                if (llmRaw) {
                    ...
                    console.log('[IntentDetector] Fallback intent result:', llmRaw);
                }
            } catch (error) {
                console.warn('[IntentDetector] 의도 판별 실패, 키워드 기반 결과 사용:', error);
            }
        }
        ...
        console.log('[IntentDetector] Final intent result:', result);
        return result;
    }
    ...
    private keywordVote(userQuery: string): {
        ...
        for (const [subtype, keywords] of Object.entries(this.keywordDictionary) as Array<[IntentSubtype, string[]]>) {
            ...
            if (matchedForSubtype.length > 0) {
                console.log(`[IntentDetector] Matched keywords for ${subtype}:`, matchedForSubtype);
            }
            if (score > bestScore) {
                bestScore = score;
                bestSubtype = subtype;
    ...
    private determineTaskType(userQuery: string, subtype: IntentSubtype): TaskType {
        ...
        // 1. 프로젝트 생성이 포함된 경우 - 항상 code_work
...
사용자 요청: "${userQuery}"`;
        ...
        // 현재 설정된 모델(또는 호출자가 전달한 모델)을 그대로 사용하고,
        // 비어 있는 경우에만 fallback 모델을 사용
        ...
        try {
            try {
                ...
                // 설정 로드 실패는 무시 (기존 설정으로 시도)
            }
            console.log(prompt);
            const response = await this.ollamaApi.sendMessage(prompt, {});
            console.log('[IntentDetector] raw response:', response);
