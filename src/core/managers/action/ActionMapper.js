 ...
 */
import { ActionType, Permission, FileOperationType, } from "./types";
...
export class ActionMapper {
     ...
     * 텍스트에서 액션을 추출합니다
     */
    extractActionsFromText(content) {
        ...
        console.log(`[ActionMapper] Detected ${fileCreationRanges.length} file creation ranges:`, fileCreationRanges.map((r) => `${r.start}-${r.end}`));
        // 코드 블록 추출 (파일 작성/수정)
        ...
        if (commandActions.length > 0) {
            console.log(`[ActionMapper] Extracted ${commandActions.length} command actions:`, commandActions.map((a) => a.params?.command).filter(Boolean));
        }
     ...
     */
    normalizeFileOperationDirectives(content) {
        if (!content) {
            return "";
        }
        let normalized = content;
        // 1) 볼드/이탤릭으로 둘러싸인 지시어 제거 (한국어 + 영어)
        normalized = normalized.replace(/\*\*(\s*(?:새 파일|수정 파일|삭제 파일|New file|Create file|Modified file|Update file|Modify file|Delete file|Remove file):[^\n*]+)\*\*/gi, (_, inner) => inner.trim());
        normalized = normalized.replace(/__(\s*(?:새 파일|수정 파일|삭제 파일|New file|Create file|Modified file|Update file|Modify file|Delete file|Remove file):[^\n_]+)__/gi, (_, inner) => inner.trim());
        // 2) 목록 기호(-, *) 제거 (한국어 + 영어)
        normalized = normalized.replace(/(^|\n)[\t ]*[-*]\s*((?:새 파일|수정 파일|삭제 파일|New file|Create file|Modified file|Update file|Modify file|Delete file|Remove file):\s*[^\n]+)/gi, (_, prefix, directive) => `${prefix}${directive}`);
        return normalized;
    }
    /**
     * 코드 블록에서 액션을 추출합니다
    ...
    extractCodeBlocks(content) {
        ...
        const markdownHeaderPattern = /##\s*(?:새 파일|수정 파일|New file|Create file|Modified file|Update file|Modify file):\s*[`"]?([^\r\n`"]+?)[`"]?\s*\r?\n\s*\r?\n\s*```[^\n]*\r?\n([\s\S]*?)\r?\n```/gi;
        while ((match = markdownHeaderPattern.exec(content)) !== null) {
            let filePath = match[1].trim();
            const code = match[2].trim();
            filePath = this.sanitizeFilePath(filePath);
            if (filePath && code) {
                const isDuplicate = actions.some((a) => a.type === ActionType.CODE_GENERATION &&
        ...
        const fileDirectiveCodeBlockPattern1 = /(?:##\s*)?(?:새 파일|수정 파일|New file|Create file|Modified file|Update file|Modify file):\s*[`"]?([^\r\n`"]+?)[`"]?\s*\r?\n\s*\r?\n\s*```[^\n]*\r?\n([\s\S]*?)\r?\n```/gi;
        ...
        const fileDirectiveCodeBlockPattern2 = /(?:##\s*)?(?:새 파일|수정 파일|New file|Create file|Modified file|Update file|Modify file):\s*[`"]?([^\r\n`"]+?)[`"]?\s*\r?\n\s*```[^\n]*\r?\n([\s\S]*?)\r?\n```/gi;
        ...
        const fileDirectiveCodeBlockPattern3 = /(?:##\s*)?(?:새 파일|수정 파일|New file|Create file|Modified file|Update file|Modify file):\s*`([^`]+)`\s*\r?\n\s*\r?\n\s*```[^\n]*\r?\n([\s\S]*?)\r?\n```/gi;
     ...
     */
    detectFileCreationRanges(content) {
        const ranges = [];
        // 파일 지시어 패턴 (한국어 + 영어 범용)
        const fileDirectivePattern = /(?:새 파일|수정 파일|New file|Create file|Modified file|Update file|Modify file):\s*[`"]?([^\r\n`"]+?)[`"]?/gi;
        let match = null;
        while ((match = fileDirectivePattern.exec(content)) !== null) {
            const startIndex = match.index;
            const afterDirective = content.substring(startIndex + match[0].length);
            // 다음 파일 지시어 찾기 (한국어 + 영어)
            const nextFileDirective = afterDirective.search(/(?:새 파일|수정 파일|New file|Create file|Modified file|Update file|Modify file):/i);
     ...
     */
    extractCommands(content, codeBlockActions = [], fileCreationRanges = []) {
        const actions = [];
        // 모든 코드 블록 범위 수집 (bash가 아닌 코드 블록 제외용)
        const allCodeBlockPattern = /```[\s\S]*?```/g;
        const allCodeBlockRanges = [];
        let match = null;
        ...
        while ((match = commandBlockPattern.exec(content)) !== null) {
            // 파일 생성 영역 내부에 있는지 확인
            const isInsideFileAction = fileActionRanges.some((range) => match.index >= range.start && match.index < range.end);
            ...
            for (const cmd of commands) {
                ...
                if (!/\s/.test(cleanCmd) &&
                    /\.(md|ts|tsx|js|jsx|json|css|html|py|java|go|rs|rb|php|sh|bat|ps1|sql|yml|yaml)$/i.test(cleanCmd)) {
                ...
                // 파일 지시어는 제외 (한국어 + 영어)
                if (/(?:새 파일|수정 파일|삭제 파일|New file|Create file|Modified file|Update file|Modify file|Delete file|Remove file):/i.test(cleanCmd)) {
     ...
     */
    deduplicateTerminalCommands(actions) {
        const seen = new Set();
        const result = [];
    ...
    filterInstallFlow(actions) {
        ...
        const isVersionCheck = (cmd) => /^node\s+-v\b/i.test(cmd) ||
        ...
        const isInstall = (cmd) => /(npm|yarn|pnpm)\s+(install|ci|add)\b/i.test(cmd);
        const isAuditOrList = (cmd) => /\b(npm\s+audit|npm\s+list|npm\s+outdated|yarn\s+audit|pnpm\s+audit)\b/i.test(cmd);
        const isInit = (cmd) => /\b(npm\s+init|yarn\s+init|pnpm\s+init)\b/i.test(cmd);
        for (const action of actions) {
            if (action.type !== ActionType.TERMINAL_COMMAND) {
                continue;
            }
     ...
     */
    limitDiagnosticCommands(actions) {
    ...
    extractFileOperations(content) {
        ...
        while ((match = renamePattern.exec(content)) !== null) {
            ...
            if (this.isValidFilePath(sourcePath) &&
     ...
     */
    createCodeGenerationAction(filePath, code, language) {
        return {
     ...
     */
    createTerminalCommandAction(command, cwd) {
        return {
            id: this.generateActionId(),
     ...
     */
    createFileOperationAction(operation, sourcePath, targetPath, content) {
        const permissions = [Permission.READ_FILE];
     ...
     * 신뢰도를 계산합니다
     */
    calculateConfidence(actions, content) {
        if (actions.length === 0) {
            return 0;
        }
