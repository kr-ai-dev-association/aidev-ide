  ...
  [AgentPhase.INVESTIGATION]: {
    ...
    requiredBeforeTransition: {
      ...
      condition: (context: any) => {
        ...
        const hasToolCalls =
          context.toolCallsInTurn && context.toolCallsInTurn.length > 0;
 ...
 * 상태별 복구 전략 매핑
 */
const RECOVERY_STRATEGIES: Record<
  AgentPhase,
  Record<string, RecoveryStrategy>
> = {
  [AgentPhase.INVESTIGATION]: {
    no_plan: {
      action: "retry",
      ...
      recommendation:
        "LLM에게 plan JSON 생성을 재요청하거나, 조사 도구 호출을 유도하세요",
    },
    loop_detected: {
      ...
      recommendation:
        "조사가 진전되지 않습니다. 현재까지의 정보로 작업을 종료합니다",
    },
    stale_state: {
      action: "abort",
      targetState: AgentPhase.DONE,
...
export class AgentStateManager {
  ...
  transitionTo(
    ...
    const sameTransitionCount = this.countRecentSameTransitions(
      oldState,
      newState,
    );
    ...
    if (
      contract.requiredBeforeTransition &&
      contract.requiredBeforeTransition.to === newState
    ) {
    ...
    console.log(
      `[AgentStateManager] State transition: ${oldState} → ${newState}`,
    );
    return { success: true };
  }
  ...
  private recordTransition(
    ...
    if (this.transitionHistory.length > AgentStateManager.MAX_HISTORY_SIZE) {
      this.transitionHistory = this.transitionHistory.slice(
        -AgentStateManager.MAX_HISTORY_SIZE,
      );
    }
  }
  ...
  getStateDescription(): string {
    const descriptions: Record<AgentPhase, string> = {
      [AgentPhase.INVESTIGATION]:
        "조사(Investigation) 단계: 정보 수집 및 계획 수립",
  ...
  validateFSMState(): FSMValidationResult {
    ...
    if (recentFailures.length >= 3) {
      issues.push(
        `Multiple consecutive transition failures detected: ${recentFailures.length} in last 5 attempts`,
      );
    }
    ...
    if (lastTransition && this.currentState === AgentPhase.INVESTIGATION) {
      ...
      if (Date.now() - lastTransition.timestamp > staleThreshold) {
        issues.push(
          `State appears stale: stuck in ${this.currentState} for over 5 minutes`,
        );
      }
    }
  ...
  forceReset(targetState: AgentPhase = AgentPhase.INVESTIGATION): void {
    console.warn(
      `[AgentStateManager] Force reset: ${this.currentState} → ${targetState}`,
    );
  ...
  attemptAutoRecovery(): {
    ...
    for (const issue of validation.issues) {
      ...
      if (issue.includes("loop detected")) {
        ...
        if (
          strategy &&
          strategy.action === "force_transition" &&
          strategy.targetState
        ) {
          console.warn(
            `[AgentStateManager] Auto-recovery: ${strategy.message}`,
          );
  ...
  getTransitionFailureAdvice(targetState: AgentPhase, reason: string): string {
    ...
    if (
      fromState === AgentPhase.INVESTIGATION &&
      targetState === AgentPhase.EXECUTION
    ) {
      if (
        reason.includes("Plan must exist") ||
        reason.includes("condition not met")
      ) {
        return (
          "조사 단계에서 plan JSON을 생성하거나 실행 도구를 호출해야 합니다. " +
    ...
    if (reason.includes("Invalid transition")) {
      ...
      return (
        `현재 상태 ${fromState}에서는 ${allowed.join(" 또는 ")}으로만 전환 가능합니다. ` +
    ...
    if (reason.includes("loop detected")) {
      return (
        "동일한 전환이 반복되고 있습니다. 무한 루프 방지를 위해 전환이 차단되었습니다. " +
