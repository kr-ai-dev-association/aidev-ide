...
export class ToolExecutionCoordinator {
     ...
     */
    static sendToolStartStatus(webview, call) {
        if (!webview)
            return;
        ...
        // 다른 파일 관련 도구들
        else if ([Tool.CREATE_FILE, Tool.UPDATE_FILE, Tool.REMOVE_FILE].includes(toolName)) {
     ...
     */
    static sendSingleToolResultToUI(webview, call, result) {
        if (!webview)
            return [];
     ...
     */
    static sendToolExecutionResultsToUISync(webview, calls, results) {
     ...
     */
    static async sendToolExecutionResultsToUI(webview, calls, results) {
     ...
     */
    static sendPendingChangesUpdate(webview) {
        try {
