...
(function (ErrorCategory) {
    ErrorCategory["ENVIRONMENT_MISSING"] = "environment_missing";
    ErrorCategory["SOURCE_ERRORS_CLUSTERED"] = "source_errors_clustered";
    ErrorCategory["SOURCE_ERRORS_SCATTERED"] = "source_errors_scattered";
    ErrorCategory["CONFIG_ERROR"] = "config_error";
    ErrorCategory["EXECUTION_TIMEOUT"] = "execution_timeout";
    ErrorCategory["BUILD_TIMEOUT"] = "build_timeout";
    ErrorCategory["COMMAND_NOT_FOUND"] = "command_not_found";
    ErrorCategory["SILENT_FAILURE"] = "silent_failure";
...
export class ErrorClassifier {
     ...
     */
    static classify(diagnostics, envHealth, configFiles) {
        ...
        const allInConfig = diagnostics.every(d => normalizedConfigFiles.some(cf => d.file === cf || d.file.endsWith('/' + cf)));
     ...
     */
    static classifyFromMessage(errorMessage, envHealth) {
        const fallbackEnv = envHealth || {
     ...
     */
    static classifyFromExecution(outcome, envHealth) {
        const fallbackEnv = envHealth || {
            hasManifestFile: false,
            hasDependencyDir: true,
            hasLockFile: false,
        ...
        if (outcome.errorCode === 'TIMEOUT' || outcome.errorCode === 'TIMEOUT_CONTINUE') {
            ...
            return this.buildExecutionResult(category, outcome, `${desc} (${outcome.duration}ms): ${outcome.command}`, fallbackEnv);
        }
        // 2. 명령어 미발견 (Unix exit code 127 또는 stderr 패턴)
        if (outcome.exitCode === 127 ||
            /command not found|not found/i.test(outcome.stderrSnippet)) {
            return this.buildExecutionResult(ErrorCategory.COMMAND_NOT_FOUND, outcome, `명령어를 찾을 수 없음: ${outcome.command}`, fallbackEnv);
        }
        // 3. 무출력 실패 (non-zero exit, stdout/stderr 모두 없음)
        if (outcome.exitCode !== 0 && !outcome.hasStderr && !outcome.hasStdout) {
            return this.buildExecutionResult(ErrorCategory.SILENT_FAILURE, outcome, `명령어가 코드 ${outcome.exitCode}로 종료됨. 출력 없음 (killed=${outcome.killed}, signal=${outcome.signal || 'none'})`, fallbackEnv);
        }
     ...
     */
    static groupBySourceAndCode(diagnostics) {
        const map = new Map();
     ...
     */
    static buildEnvironmentResult(diagnostics, envHealth) {
        const groups = this.groupBySourceAndCode(diagnostics);
        for (const g of groups) {
     ...
     */
    static buildResult(groups, dominantCategory, envHealth) {
     ...
     */
    static buildExecutionResult(category, outcome, hypothesis, envHealth) {
        const group = {
            category,
            source: 'execution',
     ...
     */
    static buildFingerprint(groups) {
        return groups
     ...
     */
    static getResolutionStrategy(category) {
        return RESOLUTION_STRATEGIES[category];
    }
    /**
     * 분류 결과에 대한 해결 전략 조회
     */
    static getResolutionStrategyFromResult(result) {
     ...
     * 자동 수정 가능 여부 확인
     */
    static isAutoRemediable(category) {
     ...
     */
    static buildLLMGuidance(result) {
