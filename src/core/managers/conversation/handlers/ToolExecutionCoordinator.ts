...
export class ToolExecutionCoordinator {
  ...
  public static hasSideEffects(
    ...
    return results.some(
      (res, i) =>
        res.success && sideEffectTools.includes(calls[i].name as Tool),
    );
  }
  ...
  public static trackFileChanges(
    ...
    toolCalls.forEach((call, index) => {
      ...
      const filePath =
        call.params?.path || call.params?.file_path || call.params?.target_file;
      if (!filePath) return;
      ...
      if (call.name === Tool.CREATE_FILE) {
        ...
        if (
          ...
          deletedFiles.length < AgentConfig.MAX_DELETED_FILES
        ) {
   ...
   * Tool 실행 결과 요약 생성
   */
  public static createToolResultSummary(
    turn: number,
    calls: ToolUse[],
    results: ToolResponse[],
  ): string {
        ...
        if (res.success && res.data) {
          ...
          if (res.data.files && Array.isArray(res.data.files)) {
            res.data.files.forEach(
              (file: ReadFileResultItem, index: number) => {
  ...
  public static sendToolStartStatus(
    ...
    // 다른 파일 관련 도구들
      ...
      [Tool.CREATE_FILE, Tool.UPDATE_FILE, Tool.REMOVE_FILE].includes(
        toolName as Tool,
      )
    ) {
      const filePath =
        params.path || params.file_path || params.target_file || "";
      const fileName = filePath ? pathModule.basename(filePath) : "";
      if (fileName) {
  ...
  public static sendSingleToolResultToUI(
    ...
    return ToolExecutionCoordinator.sendToolExecutionResultsToUISync(
      webview,
      [call],
      [result],
    );
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      if (res.success) {
        ...
        if (toolName === Tool.CREATE_FILE || toolName === Tool.UPDATE_FILE) {
          ...
          const content =
            res.data?.fileContent || res.fileContent || params.content || "";
          ...
          let addedLines = 0;
          const cachedChanges =
            InlineDiffManager.getInstance().getChanges(path);
          ...
          if (content) {
            ...
            const langLabelWithPath = path
              ? `${langLabel} [file:${path}]`
              : langLabel;
            const codeMarkdown = `\`\`\`${langLabelWithPath}\n${content}\n\`\`\``;
            WebviewBridge.receiveMessage(webview, "AgentGoCoder", codeMarkdown);
            collectedMessages.push({
              sender: "AgentGoCoder",
              text: codeMarkdown,
              type: "code",
        ...
        if (toolName === Tool.RUN_COMMAND || toolName === "run_terminal") {
          ...
          if (res.error?.code === "BLOCKED_BY_VALIDATOR") {
            ...
            if (combinedOutput) {
              ...
              WebviewBridge.receiveMessage(webview, "AgentGoCoder", errorMarkdown);
              collectedMessages.push({
                sender: "AgentGoCoder",
                text: errorMarkdown,
                type: "code",
          ...
          console.log(
            `[ToolExecutionCoordinator] create_file blocked (read_fail guard): ${res.message}`,
          );
        } else {
  ...
  > {
    console.log(
      `[ToolExecutionCoordinator] sendToolExecutionResultsToUI called with ${results.length} results, webview=${!!webview}`,
    );
    const collectedMessages: Array<{
      sender: "USER" | "AgentGoCoder" | "System";
      text: string;
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      console.log(
        `[ToolExecutionCoordinator] Processing result ${i}: toolName="${toolName}", success=${res.success}, isCreateOrUpdate=${toolName === Tool.CREATE_FILE || toolName === Tool.UPDATE_FILE}`,
      );

      if (res.success) {
        // 파일 생성/수정인 경우 헤더는 System 스타일로, 내용은 AgentGoCoder 스타일로 분리하여 표시
        if (toolName === Tool.CREATE_FILE || toolName === Tool.UPDATE_FILE) {
          const action = toolName === Tool.CREATE_FILE ? "Created" : "Updated";
          const icon = toolName === Tool.CREATE_FILE ? "✅" : "📝";
          ...
          if (toolName === Tool.CREATE_FILE) {
            ...
            const changes =
              inlineDiffManager.getLastAppliedChanges(absolutePath);

            if (changes && changes.length > 0) {
              console.log(
                `[ToolExecutionCoordinator] Using ${changes.length} cached changes from InlineDiffManager for ${path}`,
              );
              ...
              for (const change of changes) {
                if (change.type === "add") {
                  ...
                  const oldLineCount =
                    oldTextLines[oldTextLines.length - 1] === ""
                  ...
                  console.log(
                    `[ToolExecutionCoordinator] Modify change: oldText=${oldLineCount} lines, newText=${newLineCount} lines`,
                  );
                }
              }

              console.log(
                `[ToolExecutionCoordinator] Line count calculated: deleted=${deletedLines}, added=${addedLines}`,
              );
            } else {
              // ✅ changes가 없으면 경고만 하고 계속 진행 (라인 수 정보 없이 표시)
              console.warn(
                `[ToolExecutionCoordinator] No changes found for ${path}, skipping line count calculation. This may happen if showInlineDiff hasn't been called yet or file was just created.`,
              );
          ...
          if (content) {
            ...
            const codeMarkdown = `\`\`\`${langLabelWithPath}\n${content}\n\`\`\``;
            console.log(
              `[ToolExecutionCoordinator] Code markdown with file path: langLabel="${langLabelWithPath}"`,
            );
            console.log(
              `[ToolExecutionCoordinator] BEFORE sending code block: webview=${!!webview}, codeMarkdownLength=${codeMarkdown.length}`,
            );
        ...
        switch (toolNameStr) {
          ...
          case "ripgrep_search":
            ...
            const functionNameAtEnd = pattern.match(
            ...
            if (functionNameAtEnd && functionNameAtEnd[1]) {
              ...
              const functionNameMatch =
                pattern.match(/function\s+([a-zA-Z_$][a-zA-Z0-9_$]*)/) ||
                pattern.match(
          ...
          case "run_command":
            ...
            const output = res.data?.output || "";
            console.log(
              `[ToolExecutionCoordinator] run_command output: (${output?.length || 0} chars)`,
            );
            console.log(
              `[ToolExecutionCoordinator] run_command res.data: (${JSON.stringify(res.data || {}).length} chars)`,
            );
            if (output) {
              // 헤더 먼저 전송
              WebviewBridge.receiveMessage(webview, "System", displayMsg);
  ...
  public static sendPendingChangesUpdate(webview: vscode.Webview): void {
    try {
      ...
      console.warn(
        "[ToolExecutionCoordinator] Failed to send pending changes update:",
        error,
      );
    }
  }
