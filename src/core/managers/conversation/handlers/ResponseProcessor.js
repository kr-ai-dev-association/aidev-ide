...
export class ResponseProcessor {
     ...
     */
    async generateVerifiedSummary(originalSummary, createdFiles, modifiedFiles, workspaceRoot, systemPrompt, _accumulatedParts, abortSignal) {
     ...
     */
    async requestLLMSummary(createdFiles, modifiedFiles, __accumulatedParts, // 🔥 더 이상 사용하지 않음 (API 호환성 유지)
    abortSignal, retryCount = 0) {
        const MAX_RETRIES = 2;
        ...
        try {
            ...
            const contextParts = [{ text: fileListText || '작업 완료' }];
            console.log(`[ResponseProcessor] Requesting LLM summary (attempt ${retryCount + 1}/${MAX_RETRIES + 1}, contextParts=${contextParts.length})`);
            const verifiedSummary = await this.llmManager.sendMessageWithSystemPrompt(summaryPrompt, contextParts, { signal: abortSignal });
            // ✅ LLM이 도구 태그로 응답한 경우
     ...
     * 도구 태그 응답에서 텍스트만 추출
     */
    extractTextFromToolResponse(response) {
        // 도구 태그 제거
     ...
     */
    generateDefaultSummary(createdFiles, modifiedFiles) {
