...
import { HotLoadItemForIntent } from "../../context/prompts/phase";
...
export class ContextGatherer {
  ...
  collectMcpCustomPrompts(): string {
    try {
      ...
      console.warn(
        "[ContextGatherer] Failed to collect MCP custom prompts:",
        error,
      );
    }
    return "";
  ...
  async detectIntent(
    ...
    const intent = await detector.detectIntent(query, { hotLoadItems });
    ...
    if (intent.hotLoadMatch) {
      console.log(
        `[ContextGatherer] Hot Load matched: ${intent.hotLoadMatch.keywords} → ${intent.hotLoadMatch.command}`,
      );
    }
    return intent;
  }
  ...
  async gatherContext(
    ...
    if (options.selectedFiles && options.selectedFiles.length > 0) {
      ...
      for (const filePath of options.selectedFiles) {
        try {
          ...
          console.warn(
            `[ContextGatherer] Failed to read file ${filePath}:`,
            error,
          );
        }
      }
      ...
      try {
        ...
        if (workspaceRoot) {
          ...
          const alreadySelected = new Set(
            (options.selectedFiles || []).map((f) => f.toLowerCase()),
          );
          const newFiles = mentionedFiles.filter(
            (f) => !alreadySelected.has(f.toLowerCase()),
          );
          if (newFiles.length > 0) {
            ...
            for (const filePath of newFiles) {
              try {
                ...
                console.warn(
                  `[ContextGatherer] Failed to read auto-detected file ${filePath}:`,
                  error,
                );
              }
            }
        ...
        console.warn(
          "[ContextGatherer] Ask mode auto file detection failed:",
          error,
        );
      }
    }
    ...
    if (terminalContextContent) {
      console.log(
        "[ContextGatherer] Terminal context included in system prompt",
      );
    }
    ...
    if (diagnosticsContextContent) {
      console.log(
        "[ContextGatherer] Diagnostics context included in system prompt",
      );
    }
    ...
    try {
      ...
      console.warn(
        "[ContextGatherer] Failed to generate framework rules:",
        error,
      );
    }
    ...
    try {
      if (shouldSkipRag) {
        console.log(
          `[ContextGatherer] RAG: intent=${intent.subtype} — 코드 삭제 작업이므로 RAG 스킵`,
        );
      } else {
        const { AuthService } =
        ...
        if (authService.isLoggedIn()) {
          ...
          // RAG 소스가 등록되어 있는지 확인
          const ragSources =
            SettingsManager.getInstance().getServerSettings("rag");
          if (ragSources.length === 0) {
            console.log(
              "[ContextGatherer] RAG: 등록된 RAG 소스 없음 - 검색 스킵",
            );
            ...
            if (ragResults && ragResults.length > 0) {
              ...
              // 채팅 패널에 RAG 참조 시스템 메시지 출력
              const ragDocNames = [
                ...new Set(
                  ragResults
                    .map((r: any) => r.document_name || r.document || "")
                    .filter(Boolean),
                ),
              ].slice(0, 3);
              const ragSummary =
                ragDocNames.length > 0
                  ? `${ragDocNames.join(", ")} (${ragResults.length}개 청크)`
                  : `${ragResults.length}개 문서`;
      ...
      console.warn(
        "[ContextGatherer] RAG search failed (non-critical):",
        error,
      );
    }
    ...
    try {
      ...
      if (workspaceRoot) {
        ...
        const [branchResult, remoteResult] = await Promise.all([
          execAsync("git branch --show-current", { cwd: workspaceRoot }).catch(
            () => ({ stdout: "" }),
          ),
          execAsync("git remote get-url origin", { cwd: workspaceRoot }).catch(
            () => ({ stdout: "" }),
          ),
        ]);
    ...
    try {
      ...
      if (workspaceRoot) {
        const {
          SubProjectDetector,
        } = require("../../project/SubProjectDetector");
        subProjectStructure = SubProjectDetector.formatForPrompt(workspaceRoot);
        if (subProjectStructure) {
          console.log(
            `[ContextGatherer] SubProject structure detected (${subProjectStructure.length} chars)`,
          );
        }
      }
    } catch (error) {
      console.warn(
        "[ContextGatherer] SubProject detection failed (non-critical):",
        error,
      );
    }
    ...
    try {
      ...
      if (workspaceRoot) {
        ...
        repoMap = result.map;
        console.log(
          `[ContextGatherer] RepoMap generated: ${result.strategy} (${result.fileCount} files, ${result.symbolCount} symbols, ${result.map.length} chars)`,
        );
      }
    } catch (error) {
      console.warn(
        "[ContextGatherer] RepoMap generation failed (non-critical):",
        error,
      );
    }
