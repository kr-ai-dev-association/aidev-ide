...
export class ConversationCompactor {
    ...
    static getInstance(llmManager, config) {
     ...
     * 현재 대화가 압축이 필요한지 확인
     */
    needsCompaction(userParts, systemPrompt, maxTokens) {
     ...
     * 3. [요약] + [최근 메시지] 구조로 재구성
     */
    async compact(userParts, systemPrompt, maxTokens, abortSignal) {
     ...
     */
    async forceCompact(userParts, maxTokens) {
        // 메시지가 충분하지 않으면 압축 불가
     ...
     */
    async generateSummary(messagesToSummarize, abortSignal) {
        ...
        if (this.stateManager) {
            response = await this.llmManager.sendMessageWithCompactorModel(summarizationPrompt, userParts, this.stateManager, { signal: abortSignal });
        }
        else {
            response = await this.llmManager.sendMessageWithSystemPrompt(summarizationPrompt, userParts, { signal: abortSignal });
        }
     ...
     */
    fallbackCompaction(userParts, systemPrompt, originalTokens) {
     ...
     * 토큰 수 계산
     */
    calculateTotalTokens(userParts, systemPrompt) {
    ...
    getStats() {
        ...
        const totalSaved = this.compactionHistory.reduce((sum, h) => sum + (h.originalTokens - h.compactedTokens), 0);
        return {
     ...
     */
    async generateSummaryFromText(conversationText, abortSignal) {
        const summarizationPrompt = getCompactSummarizationPrompt();
        const userParts = [
            { text: `다음 대화를 요약해주세요:\n\n${conversationText}` },
        ];
     ...
     * 세션 요약이 필요한지 확인
     */
    needsSessionCompaction(totalTokensUsed, maxTokens) {
        if (!this.config.enabled) {
            return false;
        }
        const threshold = maxTokens * this.config.tokenThreshold;
     ...
     * 세션의 대화 히스토리를 요약
     */
    async compactSessionHistory(conversationHistory, maxTokens, abortSignal) {
...
//# sourceMappingURL=ConversationCompactor.js.map