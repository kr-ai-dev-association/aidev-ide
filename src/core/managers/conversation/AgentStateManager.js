...
(function (AgentPhase) {
    AgentPhase["INVESTIGATION"] = "investigation";
...
export class AgentStateManager {
    ...
    static MAX_SAME_TRANSITION_COUNT = 5; // 동일 전환 반복 제한
    constructor(initialState = AgentPhase.INVESTIGATION) {
     ...
     * 전환 히스토리 기록
     */
    recordTransition(from, to, success, reason, context) {
        this.transitionHistory.push({
            from,
            to,
            timestamp: Date.now(),
     ...
     * 최근 동일 전환 횟수 계산
     */
    countRecentSameTransitions(from, to) {
        let count = 0;
        // 최근 10개의 전환만 확인
    ...
    isToolAllowed(toolName) {
        ...
        if (Object.values(Tool).includes(toolName)) {
     ...
     */
    forceReset(targetState = AgentPhase.INVESTIGATION) {
  ...
  Issues: ${validation.issues.length > 0 ? validation.issues.join(', ') : 'none'}`;
     ...
     * 전환 실패 시 권고 사항 반환
     */
    getTransitionFailureAdvice(targetState, reason) {
