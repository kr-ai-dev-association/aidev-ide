...
import { AndroidStack, ReactStack, FlutterStack, SpringStack, PythonStack, } from './stackTypes';
export class StackDetector {
    /**
     * 프로젝트의 세부 스택 감지
     */
    async detectDetailedStack(projectRoot, projectType) {
     ...
     * 스택 정보를 요약 문자열로 변환
     */
    formatStackSummary(detailedStack) {
        const parts = [];
        // 프로젝트 타입
    ...
    async detectAndroidStack(projectRoot, result) {
    ...
    async detectReactStack(projectRoot, result) {
    ...
    async detectFlutterStack(projectRoot, result) {
    ...
    async detectSpringStack(projectRoot, result) {
    ...
    async detectPythonStack(projectRoot, result) {
    ...
    // ==================== Vue Stack Detection ====================
    async detectVueStack(projectRoot, result) {
        const packageJsonPath = path.join(projectRoot, 'package.json');
        if (!fs.existsSync(packageJsonPath))
            return;
        const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, 'utf8'));
    ...
    async detectNodeStack(projectRoot, result) {
        const packageJsonPath = path.join(projectRoot, 'package.json');
        if (!fs.existsSync(packageJsonPath))
            return;
        const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, 'utf8'));
    ...
    // ==================== Helper Methods ====================
    containsAny(content, patterns) {
        return patterns.some(pattern => content.includes(pattern));
    }
    extractVersion(content, patterns) {
