...
export class ProjectManager {
  ...
  public async initialize(projectRoot: string): Promise<ProjectInfo> {
    ...
    // 설정 파일 파싱
    const configFiles = await this.parseConfigFiles(
      projectRoot,
      detection.type,
    );

    // 빌드 명령어 추출
    const buildCommands = await this.extractBuildCommands(
      projectRoot,
      detection.type,
    );
    ...
    console.log(
      `[ProjectManager] Project initialized: ${projectInfo.type} (${projectInfo.framework || "none"})`,
    );
    ...
    try {
      ...
      if (localProjectType === "unknown") {
        ...
        if (keywordType && supportedTypes.includes(keywordType)) {
          console.log(
            `[ProjectManager] Keyword-based project type detection: ${keywordType}`,
          );
          return {
...
사용자 요청: "${userQuery}"`;
      ...
      if (ollamaApi) {
        response = await ollamaApi.sendMessage(projectTypePrompt, {
          signal: abortSignal,
        });
      } else {
      ...
      console.log(
        `[ProjectManager] LLM 프로젝트 타입 감지 응답 수신 (${response.length} chars)`,
      );
      ...
      if (jsonMatch) {
        try {
          ...
          if (
            result.projectType &&
            supportedTypes.includes(result.projectType)
          ) {
            const confidence =
              result.confidence || AgentConfig.MIN_PROJECT_TYPE_CONFIDENCE;
            // confidence가 임계값 미만이면 사용자 선택 필요
            const needsUserSelection =
              confidence < AgentConfig.MIN_PROJECT_TYPE_CONFIDENCE ||
              result.needsUserSelection === true;
            console.log(
              `[ProjectManager] LLM 프로젝트 타입 감지 성공: ${result.projectType} (신뢰도: ${confidence}, 사용자 선택 필요: ${needsUserSelection})`,
            );
            return {
      ...
      if (localProjectType !== "unknown") {
        console.warn(
          "[ProjectManager] LLM 프로젝트 타입 감지 실패 - 로컬 감지 결과 사용",
        );
        return {
          projectType: localProjectType,
          confidence: AgentConfig.PROJECT_TYPE_CONFIDENCE.LOCAL_HEURISTIC,
      ...
      console.warn(
        "[ProjectManager] LLM 프로젝트 타입 감지 실패 - LLM 응답을 파싱할 수 없음",
      );
      return {
        projectType: "unknown",
        confidence: 0,
  ...
  private detectProjectTypeFromKeywords(userQuery: string): string | undefined {
    ...
    const hasJava = lower.includes("java") || userQuery.includes("자바");

    if (
      hasSpring ||
      (hasJava && (lower.includes("backend") || userQuery.includes("백엔드")))
    ) {
    ...
    if (
      lower.includes("ios") ||
      userQuery.includes("아이폰") ||
      userQuery.includes("iOS")
    ) {
      return "ios";
    }
  ...
  private async buildFileTree(
    ...
    try {
      ...
      console.warn(
        `[ProjectManager] Failed to build file tree for ${dir}:`,
        error,
      );
    }
   ...
   * @returns 프로젝트 프로필
   */
  public async getProjectProfile(
    ...
    const PROFILE_KEY = "agentgocoder.projectProfile";
   ...
   * 프로젝트 프로필을 스캔합니다
   */
  public async scanProjectProfile(
    storage?: vscode.Memento,
   ...
   */
  private async scanWorkspaceForProfile(
    rootPath: string,
  ): Promise<ProjectProfile> {
    const packageJson = await this.tryReadJson(
      path.join(rootPath, "package.json"),
    );
    const pyProject = await this.tryReadToml(
    ...
    const buildGradle = await this.tryReadFile(
      path.join(rootPath, "build.gradle"),
    );
    const buildGradleKts = await this.tryReadFile(
      path.join(rootPath, "build.gradle.kts"),
    );
    const requirementsTxt = await this.tryReadFile(
    ...
    const language = this.detectLanguageForProfile({
      packageJson,
      pyProject,
      goMod,
      pomXml,
  ...
  private detectLanguageForProfile(inputs: {
    ...
    if (inputs.pomXml || inputs.buildGradle || inputs.buildGradleKts) {
      ...
      if (inputs.frameworks.length === 0) {
        inputs.frameworks.push({
          framework: "Java (Unknown Framework)",
          confidence: 0.5,
          evidence: ["Detected Java build file"],
        });
      }
      return "Java";
    }
  ...
  private populateNodeFrameworks(pkg: any, frameworks: FrameworkMatch[]): void {
    ...
    if (has("react") || has("react-dom")) {
      addFramework(
        "React",
        AgentConfig.FRAMEWORK_CONFIDENCE.MAJOR,
    ...
    if (has("@angular/core")) {
      addFramework(
        "Angular",
        AgentConfig.FRAMEWORK_CONFIDENCE.MAJOR,
        "Found @angular/core dependency",
      );
    }
    if (has("vite")) {
      addFramework(
        "Vite",
        AgentConfig.FRAMEWORK_CONFIDENCE.COMMON,
        "Found vite dependency",
      );
    }
    if (has("next")) {
      addFramework(
   ...
   * Python 프레임워크를 감지합니다
   */
  private populatePythonFrameworks(
    inputs: { pyProject?: any; requirementsTxt?: string; setupPy?: string },
    frameworks: FrameworkMatch[],
  ): void {
    ...
    const add = (framework: string, evidence: string) => {
      frameworks.push({
        framework,
        confidence: AgentConfig.FRAMEWORK_CONFIDENCE.COMMON,
        evidence: [evidence],
      });
    };

    const matched = (needle: string) =>
      dependencies.some((dep) => dep.toLowerCase().includes(needle));
    if (matched("fastapi"))
   ...
   * Java 프레임워크를 감지합니다
   */
  private populateJavaFrameworks(
    inputs: { pomXml?: string; buildGradle?: string; buildGradleKts?: string },
    frameworks: FrameworkMatch[],
  ): void {
    const add = (framework: string, evidence: string) =>
      frameworks.push({
    ...
    const detect = (content: string | undefined, needle: string) =>
  ...
  private detectEntryPoints(
    ...
    if (pkg.bin && typeof pkg.bin === "object") {
      const binValues = Object.values(pkg.bin).filter(
  ...
  public buildProfileContext(
    ...
    if (profile.frameworks.length > 0) {
      const formatted = profile.frameworks
        .map(
          (f) => `${f.framework} (신뢰도 ${(f.confidence * 100).toFixed(0)}%)`,
        )
        .join(", ");
    ...
    if (profile.entryPoints.length > 0) {
      lines.push(
        `실행 엔트리포인트: ${profile.entryPoints.slice(0, 5).join(", ")}`,
      );
    }
   ...
   * 프로젝트 파일 인벤토리를 빌드합니다
   */
  public async buildProjectInventorySection(
  ...
  public async detectDetailedStack(
    ...
    if (
      !forceRefresh &&
      this.cachedDetailedStack &&
      now - this.lastStackDetectionTime < CACHE_DURATION
    ) {
    ...
    console.log(
      `[ProjectManager] Detecting detailed stack for ${this.currentProject.type}...`,
    );
    ...
    try {
      ...
      console.log(
        `[ProjectManager] Detected ${detailedStack.stacks.length} stacks:`,
   ...
   */
  public async buildFullProjectContext(
    profile?: ProjectProfile,
    projectType?: string,
  ): Promise<string> {
    const lines: string[] = [];
    ...
    // 호환성 이슈가 있으면 추가
    if (
      this.cachedDetailedStack?.potentialIssues &&
      this.cachedDetailedStack.potentialIssues.length > 0
    ) {
      lines.push("");
