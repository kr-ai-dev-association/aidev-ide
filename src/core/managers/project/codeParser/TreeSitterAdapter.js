...
import { DefinitionType, getLanguageFromExtension, isParsableFile, } from './ICodeParserAdapter';
import { loadRequiredLanguageParsers, canParseFile } from './languageParser';
/**
...
export class TreeSitterAdapter {
    ...
    async getClassDefinition(className, searchPath) {
        const definitions = await this.parseDirectory(searchPath, { maxFiles: 100 });
        for (const file of definitions.files) {
            const classDef = file.definitions.find(def => def.name === className && def.type === DefinitionType.CLASS);
            if (classDef) {
                // 같은 파일 내에서 메서드 찾기
                const methods = file.definitions.filter(def => def.type === DefinitionType.METHOD &&
