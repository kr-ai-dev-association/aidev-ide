...
async function loadLanguage(langName: string): Promise<Parser.Language> {
  ...
  const extension = vscode.extensions.getExtension("banya.agentgocoder");
  if (!extension) {
    throw new Error("AgentGoCoder extension not found");
  }
...
export async function loadRequiredLanguageParsers(
  ...
  console.log(
    `[TreeSitter] Loading parsers for ${filesToParse.length} files...`,
  );
  ...
  for (const ext of extensionsToLoad) {
    try {
      ...
      switch (ext) {
        ...
        default:
          console.log(
            `[LanguageParser] Unsupported language extension: ${ext}`,
          );
          continue;
      }
      ...
      console.error(
        `[LanguageParser] Failed to load parser for ${ext}:`,
        error,
      );
    }
  }
...
export async function loadLanguageParser(
  ext: string,
): Promise<{ parser: Parser; query: Parser.Query } | null> {
