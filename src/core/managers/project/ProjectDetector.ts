...
export class ProjectDetector {
   ...
   */
  public static async detectPythonRuntime(
    ...
    if (
      ...
      (fs.existsSync(path.join(workspaceRoot, "uv.lock")) ||
        fs.existsSync(path.join(workspaceRoot, ".python-version")))
    ) {
      try {
        execSync("uv run python --version", {
          cwd: workspaceRoot,
          timeout: 5000,
          stdio: "pipe",
        });
        ...
        console.log(
          "[ProjectDetector] Python runtime detected: uv run python (uv project)",
        );
    ...
    console.log(
      "[ProjectDetector] Python runtime not found, using default: python3",
    );
    return "python3";
  }
  ...
  private static readonly EXPLICIT_BUILD_FILES: Array<{
    ...
    // ⚠️ Android 프로젝트 (Gradle 기반) - Spring Boot보다 먼저 체크해야 함
    {
      ...
      detector: (projectRoot: string) => {
        ...
        return (
          fs.existsSync(path.join(projectRoot, "app", "build.gradle")) ||
  ...
  private async detectByFilesAsync(projectRoot: string): Promise<{
    ...
    try {
      ...
      for (const rule of ProjectDetector.EXPLICIT_BUILD_FILES) {
        ...
        if (foundFile) {
          console.log(
            `[ProjectDetector] Detected ${rule.type} by file: ${foundFile}`,
          );
          return {
            type: rule.type,
      ...
      try {
        ...
        const csprojFiles = files.filter(
          (f) =>
            f.endsWith(".csproj") ||
            f.endsWith(".sln") ||
            f.endsWith(".fsproj"),
  ...
  public async getValidationCommand(
    ...
    if (fs.existsSync(path.join(projectRoot, ".pre-commit-config.yaml"))) {
      if (allFiles.length > 0) {
        return {
          command: `pre-commit run --files ${allFiles.join(" ")}`,
          description: "Pre-commit 훅 실행",
        };
      }
      return {
        command: "pre-commit run --all-files",
        description: "Pre-commit 훅 실행 (전체)",
      };
    }
    ...
    if (fs.existsSync(path.join(projectRoot, "Justfile"))) {
      return {
        ...
        description: "Justfile 확인 (실행 전 확인)",
      };
    }
    ...
    if (fs.existsSync(path.join(projectRoot, "Taskfile.yml"))) {
      return {
        command: "task lint || task build",
        description: "Taskfile 실행",
      };
    }
    ...
    if (fs.existsSync(path.join(projectRoot, "nx.json"))) {
      return {
        ...
        description: "Nx Affected Lint",
      };
    }
    if (fs.existsSync(path.join(projectRoot, "turbo.json"))) {
      return {
        command: "npx turbo run lint --filter=...",
        description: "TurboRepo Lint",
      };
    }
    ...
    if (fs.existsSync(path.join(projectRoot, ".github", "workflows"))) {
      try {
        const workflowFiles = fs
          .readdirSync(path.join(projectRoot, ".github", "workflows"))
    ...
    switch (projectType) {
      ...
      case ProjectType.NODE:
        ...
        const hasTypeScript =
          createdFiles.some((f) => f.endsWith(".ts") || f.endsWith(".tsx")) ||
        ...
        if (hasTypeScript) {
          ...
          if (fs.existsSync(path.join(projectRoot, "package.json"))) {
            ...
            if (this.hasScript(projectRoot, "build")) {
              return {
                command: `npx tsc --noEmit && ${pm} run build`,
                description: "TypeScript 타입 검사 + 빌드 검증",
              };
            }
        ...
        if (fs.existsSync(path.join(projectRoot, "package.json"))) {
          ...
          if (this.hasScript(projectRoot, "validate"))
            return {
              command: `${pm} run validate`,
              description: "Validate 스크립트",
            };
        ...
        return {
          command: `npm run build --dry-run ${nullDev} || echo "No build script"`,
          description: "Node.js 빌드 검사",
        };
      ...
      case ProjectType.ANDROID:
        ...
        if (
          ...
          return {
            command: `${gradlewAndroid} assembleDebug`,
            description: "Android 디버그 빌드",
          };
        }
        ...
        return {
          ...
          description: "Android 디버그 빌드 (Gradle)",
        };
      ...
      case ProjectType.RUST:
        ...
        if (
          ...
          return {
            ...
            description: "Cargo Clippy (warnings as errors)",
          };
        }
      ...
      case ProjectType.PHP:
        ...
        if (fs.existsSync(path.join(projectRoot, "artisan"))) {
          return {
            ...
            description: "Laravel Route Check",
          };
        }
        ...
        if (phpFiles.length > 0) {
          ...
          const relativePath = path.isAbsolute(firstPhpFile)
            ...
            : firstPhpFile;
          return {
            command: `php -l ${relativePath}`,
            description: "PHP 문법 검사",
          };
        }
        return null;
      ...
      case ProjectType.CSHARP: {
        ...
        if (hasTestProject) {
          return {
            ...
            description: ".NET 테스트 실행",
          };
        }
        if (slnFiles.length > 0) {
          return {
            command: "dotnet build --no-restore",
            description: ".NET 솔루션 빌드",
          };
        }
      ...
      case ProjectType.RUBY:
        ...
        if (rubyFiles.length > 0) {
          ...
          const relativePath = path.isAbsolute(firstRubyFile)
            ...
            : firstRubyFile;
          return {
            command: `ruby -c ${relativePath}`,
            description: "Ruby 문법 검사",
          };
        }
        return null;
      ...
      case ProjectType.KOTLIN: {
        ...
        if (
          ...
          return {
            command: `${gradlewKt} compileKotlin`,
            description: "Kotlin Gradle 컴파일",
          };
        }
        return {
          command: "gradle compileKotlin",
          description: "Kotlin 컴파일 검사",
        };
      }
      ...
      default:
        ...
        if (
          ...
          allFiles.some((f) => f.endsWith("Dockerfile"))
        ) {
          return {
        ...
        if (allFiles.some((f) => f.endsWith("docker-compose.yml"))) {
          return {
            ...
            description: "Docker Compose Config Check",
          };
        }

        // --- Kubernetes / Helm ---
        if (
          (extensions.has(".yaml") || extensions.has(".yml")) &&
        ...
        if (extensions.has(".scala")) {
          ...
          return {
            command: 'scalac -version && echo "Scala files detected"',
            description: "Scala Check",
          };
        }
        ...
        if (extensions.has(".kt") || extensions.has(".kts")) {
          if (fs.existsSync(path.join(projectRoot, "build.gradle.kts"))) {
            ...
            return {
              command: `${gradlewKt} compileKotlin`,
              description: "Kotlin Gradle Compile",
            };
          }
          return {
            command: 'kotlinc -version && echo "Kotlin files detected"',
            description: "Kotlin Check",
          };
        }
        ...
        if (extensions.has(".hs")) {
          if (fs.existsSync(path.join(projectRoot, "stack.yaml"))) {
            return {
              ...
              description: "Haskell Stack Check",
            };
          }
          if (fs.existsSync(path.join(projectRoot, "cabal.project"))) {
            return {
              command: "cabal build --dry-run",
              description: "Haskell Cabal Check",
            };
          }
        }
        ...
        if (extensions.has(".ml") || extensions.has(".mli")) {
          if (fs.existsSync(path.join(projectRoot, "dune-project"))) {
            return {
              command: "dune build --dry-run",
              description: "OCaml Dune Build",
            };
          }
        }
        ...
        if (extensions.has(".yaml") || extensions.has(".yml")) {
          ...
          if (yamlFiles.length > 0) {
            ...
            return {
              command: `yamllint ${relativePath} || yq eval ${relativePath} ${yamlNull}`,
        ...
        if (extensions.has(".toml")) {
          ...
          if (tomlFiles.length > 0) {
            ...
            const relativePath = path.isAbsolute(firstFile)
              ...
              : firstFile;
            return {
              command: `taplo format --check ${relativePath} || echo "TOML file exists"`,
        ...
        if (extensions.has(".sql")) {
          ...
          if (sqlFiles.length > 0) {
            ...
            return {
              command: `sqlfluff lint ${relativePath} || sql-lint ${relativePath}`,
              description: "SQL Lint",
            };
          }
        }
        ...
        if (extensions.has(".graphql") || extensions.has(".gql")) {
          const gqlFiles = allFiles.filter(
            (f) => f.endsWith(".graphql") || f.endsWith(".gql"),
          );
          if (gqlFiles.length > 0) {
            const fileList = gqlFiles
              .map((f) =>
                path.isAbsolute(f) ? path.relative(projectRoot, f) : f,
              )
              .join(" ");
            return {
              command: `graphql-schema-linter ${fileList}`,
              description: "GraphQL Schema Lint",
            };
          }
        }
        ...
        if (extensions.has(".proto")) {
          ...
          if (protoFiles.length > 0) {
            ...
            const protoNull =
              process.platform === "win32" ? "nul" : "/dev/null";
            return {
              command: `protoc --descriptor_set_out=${protoNull} ${relativePath}`,
        ...
        if (jsonFile) {
          const relativePath = path.isAbsolute(jsonFile)
            ...
            : jsonFile;
          return {
            command: `node -e "require('fs').readFileSync('${relativePath}', 'utf8'); JSON.parse(require('fs').readFileSync('${relativePath}', 'utf8'))"`,
  ...
  public async getNextValidationCandidate(
    ...
    // 프로젝트 타입별 후보 목록 생성
    const candidates = await this.getValidationCandidates(
      projectType,
      projectRoot,
      allFiles,
    );
    ...
    for (const candidate of candidates) {
      const isExcluded = excludedCommands.some(
        (excluded) =>
          candidate.command.includes(excluded) ||
          excluded.includes(candidate.command),
      );
      if (!isExcluded) {
        console.log(
          `[ProjectDetector] Next validation candidate: ${candidate.command} (excluded ${excludedCommands.length} commands)`,
        );
        return candidate;
      }
    }
    ...
    console.log(
      `[ProjectDetector] No more validation candidates (all ${candidates.length} excluded)`,
    );
    return null;
  }
      ...
      case ProjectType.FASTAPI: {
        ...
        if (pythonFiles.length > 0) {
          if (
            ...
            candidates.push({
              command: `ruff check ${relativePaths}`,
   ...
   */
  private async getServerBuildTestOverride(
    modifiedFiles?: string[],
  ): Promise<{ command: string; description: string } | null> {
    try {
      ...
      if (serverConfigs && serverConfigs.length > 0) {
        const requiredConfig = serverConfigs.find(
          (c) => c.enforcement === "required",
        );
        if (requiredConfig && requiredConfig.value) {
          const cmd =
            typeof requiredConfig.value === "string"
              ? requiredConfig.value
              : requiredConfig.value.command;
          const desc =
            typeof requiredConfig.value === "object"
              ? requiredConfig.value.description ||
                `Server config: ${requiredConfig.key}`
              : `Server config: ${requiredConfig.key}`;

          if (cmd) {
            console.log(
              `[ProjectDetector] Using required server build/test config: ${requiredConfig.key}`,
            );
      ...
      if (personalConfigs && personalConfigs.length > 0) {
        ...
        const cmd =
          typeof personal.value === "string"
            ? personal.value
            : personal.value?.command;
        if (cmd) {
          console.log(
            `[ProjectDetector] Using personal build/test config: ${personal.key}`,
          );
          return {
            command: cmd,
            description: personal.description || `Personal: ${personal.key}`,
          };
        }
      }
      ...
      if (serverConfigs && serverConfigs.length > 0) {
        const recommendedConfigs = serverConfigs.filter(
          (c) => c.enforcement === "recommended" || c.enforcement === "preset",
        );
        for (const recommendedConfig of recommendedConfigs) {
          ...
          if (
            settingsManager.isSettingDisabled(
              "build_test",
              recommendedConfig.key,
            )
          ) {
            console.log(
              `[ProjectDetector] Skipping disabled recommended config: ${recommendedConfig.key}`,
            );
            continue;
          }
          ...
          const configLang =
            typeof recommendedConfig.value === "object"
              ? recommendedConfig.value.language
              : undefined;
          if (configLang && modifiedFiles && modifiedFiles.length > 0) {
            if (!this.isLanguageMatchingFiles(configLang, modifiedFiles)) {
              console.log(
                `[ProjectDetector] Skipping recommended config (language mismatch): ${recommendedConfig.key} (${configLang}) vs files: ${modifiedFiles.slice(0, 3).join(", ")}`,
              );
              continue;
            }
          }
          ...
          const cmd =
            typeof recommendedConfig.value === "string"
              ? recommendedConfig.value
              : recommendedConfig.value.command;
          const desc =
            typeof recommendedConfig.value === "object"
              ? recommendedConfig.value.description ||
                `Server recommended: ${recommendedConfig.key}`
              : `Server recommended: ${recommendedConfig.key}`;

          if (cmd) {
            console.log(
              `[ProjectDetector] Using recommended server build/test config: ${recommendedConfig.key}`,
            );
            return { command: cmd, description: desc };
          }
        }
      ...
      console.warn(
        "[ProjectDetector] Failed to get server build/test configs (falling back to auto-detect):",
        error,
      );
      return null;
    }
...
JSON 형식으로 응답하세요:
{
  ...
  "reasoning": "build.gradle과 app 폴더가 존재하므로 Android 프로젝트입니다."
      ...
      if (jsonMatch) {
        ...
        try {
          ...
          console.warn(
            "[ProjectDetector] JSON parse failed, trying code block extraction",
          );
  ...
  public getFormatterCommand(
    ...
    switch (projectType) {
      ...
      case ProjectType.PHP:
        ...
        if (
          fs.existsSync(path.join(projectRoot, "pint.json")) ||
      ...
      case ProjectType.C_CPP:
        // ✅ C / C++
        const cppFiles = allFiles.filter(
          (f) =>
            f.endsWith(".c") ||
            f.endsWith(".cpp") ||
            f.endsWith(".cc") ||
   ...
   */
  public static checkEnvironmentHealth(
    workspaceRoot: string,
  ): EnvironmentHealth {
    ...
    for (const eco of ECOSYSTEM_MAP) {
      ...
      if (fs.existsSync(manifestPath)) {
        ...
        if (eco.depDir) {
          result.hasDependencyDir = fs.existsSync(
            path.join(workspaceRoot, eco.depDir),
          );
        } else {
        ...
        result.needsInstall =
          result.hasManifestFile && !result.hasDependencyDir;
