...
export class ConfigParser {
     ...
     * 설정 파일을 파싱합니다
     */
    public async parseConfig(configPath: string): Promise<ConfigFile | null> {
        try {
            const fileName = path.basename(configPath);
            const content = fs.readFileSync(configPath, 'utf8');

            let parsed: any = {};
            let type: 'package' | 'build' | 'config' | 'env' = 'config';
     ...
     */
    public extractBuildCommands(packageJson: any): BuildCommands {
     ...
     */
    public extractDependencies(packageJson: any): Dependency[] {
     ...
     */
    public extractMetadata(packageJson: any): ProjectMetadata {
        return {
     ...
     * go.mod 파일을 파싱합니다
     */
    private parseGoMod(content: string): any {
        const result: any = {};
        const lines = content.split('\n');
