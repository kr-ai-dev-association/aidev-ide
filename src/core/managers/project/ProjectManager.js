...
사용자 요청: "${userQuery}"`;
    ...
    populateJavaFrameworks(inputs, frameworks) {
        const add = (framework, evidence) => frameworks.push({ framework, confidence: AgentConfig.FRAMEWORK_CONFIDENCE.COMMON, evidence: [evidence] });
    ...
    async detectDetailedStack(forceRefresh = false) {
        ...
        try {
            const detailedStack = await this.stackDetector.detectDetailedStack(this.projectRoot, this.currentProject.type);
            // 캐시 저장
            this.cachedDetailedStack = detailedStack;
            this.lastStackDetectionTime = now;
            console.log(`[ProjectManager] Detected ${detailedStack.stacks.length} stacks:`, detailedStack.stacks.map(s => s.name).join(', '));
