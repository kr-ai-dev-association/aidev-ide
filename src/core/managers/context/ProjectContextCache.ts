...
export class ProjectContextCache {
  ...
  public async cacheFile(filePath: string, ttl?: number): Promise<void> {
    try {
      ...
      console.error(
        `[ProjectContextCache] Failed to cache file ${filePath}:`,
        error,
      );
    }
  }
  ...
  public cacheProjectStructure(
    ...
    console.log(
      `[ProjectContextCache] Cached project structure: ${projectRoot}`,
    );
  }
  ...
  public async preloadPriorityFiles(projectRoot: string): Promise<void> {
    console.log(
      `[ProjectContextCache] Preloading priority files for ${projectRoot}`,
    );
  ...
  public invalidateByPattern(pattern: RegExp): number {
    ...
    console.log(
      `[ProjectContextCache] Invalidated ${count} cache entries by pattern`,
    );
    return count;
  }
  ...
  private evictLRU(requiredSpace: number): void {
    ...
    console.log(
      `[ProjectContextCache] Evicted LRU entries, freed ${freedSpace} bytes`,
    );
  }
  ...
  private cleanupExpired(): void {
    ...
    if (removed > 0) {
      console.log(
        `[ProjectContextCache] Cleaned up ${removed} expired entries`,
      );
    }
  }
  ...
  private startCleanupTimer(): void {
    ...
    console.log(
      `[ProjectContextCache] Cleanup timer started (interval: ${this.options.cleanupInterval}ms)`,
    );
  }
  ...
  private watchFile(filePath: string): void {
    ...
    watcher.onDidChange(() => {
      console.log(
        `[ProjectContextCache] File changed, invalidating cache: ${filePath}`,
      );
      this.invalidate(cacheKey);
    });
    ...
    watcher.onDidDelete(() => {
      console.log(
        `[ProjectContextCache] File deleted, invalidating cache: ${filePath}`,
      );
      this.invalidate(cacheKey);
    });
  ...
  private async saveCacheToDisk(): Promise<void> {
    try {
      ...
      await this.context.globalState.update(
        "agentgocoder.projectContextCache",
        {
          cache: cacheData,
          stats: this.stats,
        },
      ...
      console.error(
        "[ProjectContextCache] Failed to save cache to disk:",
        error,
      );
    }
  }
  ...
  private async loadCacheFromDisk(): Promise<void> {
    try {
      ...
      if (stored) {
        ...
        console.log(
          `[ProjectContextCache] Loaded ${this.cache.size} cache entries from disk`,
        );
      }
    } catch (error) {
      console.error(
        "[ProjectContextCache] Failed to load cache from disk:",
        error,
      );
    }
  }
  ...
  private async loadOptions(): Promise<void> {
    const stored = this.context.globalState.get<Partial<CacheOptions>>(
      "agentgocoder.projectContextCacheOptions",
    );
    if (stored) {
