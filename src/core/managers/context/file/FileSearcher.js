...
export class FileSearcher {
    ...
    async findRipgrep() {
        try {
            ...
            const vscodeRipgrep = path.join(vscode.env.appRoot, 'resources', 'app', 'node_modules', 'vscode-ripgrep', 'bin', process.platform === 'win32' ? 'rg.exe' : 'rg');
            try {
     ...
     * 정규식으로 파일 검색
     */
    async searchFiles(pattern, searchPath, options) {
     ...
     * ripgrep을 사용한 검색
     */
    async searchFilesWithRipgrep(pattern, searchPath, options) {
                ...
                try {
                    ...
                    if (parsed.type === 'match') {
                        if (currentResult && currentFile) {
                            ...
                            fileMap.get(currentFile).push(currentResult);
                        }
     ...
     */
    async searchFilesNative(pattern, searchPath, options) {
        const results = [];
     ...
     * 검색할 파일 목록 가져오기
     */
    async getFilesToSearch(searchPath, options) {
        const files = [];
     ...
     * 특정 파일에서 검색
     */
    async searchInFile(filePath, pattern, contextLines) {
        try {
     ...
     * 검색 결과 하이라이트
     */
    formatResults(results, projectRoot) {
        if (results.length === 0) {
