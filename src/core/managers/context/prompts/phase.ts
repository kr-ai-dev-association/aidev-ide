...
export function getIntentPrompt(
  ...
  // Hot Load section (highest priority — semantic match against registered items)
  const hotLoadSection = hotLoadItems && hotLoadItems.length > 0
    ? `
**⚡ HOT LOAD (check FIRST, highest priority):**
Check whether any registered Hot Load item below semantically matches the user request.
If one matches, include its ID in "hotLoadMatchId".

Registered Hot Load items:
${hotLoadItems.map(item => `- [ID: ${item.id}] keywords: "${item.keywords}" | description: "${item.description}" | command: \`${item.command}\``).join('\n')}

**Hot Load matching criteria:**
- Match by the **intent / semantic similarity** of the request to the Hot Load keywords/description, not exact keyword equality.
- Example: "create an agent" -> matches a "create agent" Hot Load item.
`
    : '';
...
- **When a Hot Load item matches**: requiresPlan is always **false** (Hot Load runs immediately)
...
- "Create an agent" (Hot Load match) -> hotLoadMatchId: 1, subtype: "execution_run", requiresPlan: false
...
**Note: If a Hot Load item matches, you MUST include its ID in hotLoadMatchId. requiredSkillKeys must include keys of registered skills relevant to the user request; if none are relevant, return an empty array [].**
