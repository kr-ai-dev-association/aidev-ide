 ...
 * 도구 호출 형식 프롬프트
 */
export function getToolCallingFormatPrompt() {
 ...
 * 도구 스펙 프롬프트 생성
 */
export function getToolSpecPrompt(spec) {
    ...
    if (spec.name === Tool.CREATE_FILE) {
        ...
        for (const param of spec.parameters) {
            if (param.required) {
                let exampleValue = '...';
                if (param.name === 'path')
                    exampleValue = 'src/example.ts';
                else if (param.name === 'command')
                    exampleValue = 'npm install';
                else if (param.name === 'pattern')
                    exampleValue = 'TODO';
 ...
 * 작업 흐름 가이드라인 프롬프트
 */
export function getWorkflowGuidelinePrompt() {
 ...
 * 중요 규칙 프롬프트
 */
export function getImportantRulesPrompt() {
    let prompt = '### 중요 규칙\n\n';
 ...
 * 전체 도구 프롬프트 섹션 생성
 */
export function buildToolPromptSection(specs) {
