...
**의존성 파일 수정 후 설치 필수:**
- package.json, requirements.txt, pyproject.toml, go.mod, Cargo.toml, Gemfile, pom.xml, build.gradle 등 의존성 파일을 수정한 경우
- 반드시 해당 언어의 패키지 설치 명령을 실행하세요 (package.json → npm install, requirements.txt → pip install -r requirements.txt 등)
- 의존성 파일 수정 후 설치 없이 다음 작업으로 넘어가지 마세요`;
}
