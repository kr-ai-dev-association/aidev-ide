...
// ==================== Common Rules ====================
export function getXmlToolRules(): string {
  return `**XML 도구 호출 규칙:**
...
**글로벌 핵심 규칙**
- **실행 중심 응답**: "해야 한다", "조사하겠습니다"라는 설명만 하지 마세요. 작업 진행 중이라면 반드시 최소 하나 이상의 XML 도구 호출이 포함되어야 합니다.
- **실패로부터 학습**: 도구 실행이 실패했다면 동일한 파라미터로 재시도하기 전에 반드시 실패 원인을 분석하세요.
- **작업 완료 및 요약**: 모든 작업 완료 후 최종 결과를 한국어로 상세히 요약하여 답변하세요.
- **행동과 설명 동시 제공**: 도구 호출 시 해당 작업 설명을 함께 제공하세요.
- **도구 호출 누락 금지**: 파일 조사 필요 시 즉시 <list_files>나 <read_file> 호출, 계획 수립 완료 시 즉시 <plan> 태그 출력하세요.
- **마크다운 코드 블록 금지**: 모든 코드는 반드시 XML 도구 내부에만 있어야 합니다.
- **다중 도구 호출 필수**: 필요한 파일이 여러 개라면 한 번에 모든 도구를 호출하세요. 여러 <read_file>, <create_file> 태그를 동시에 출력 가능합니다.
- **이미 읽은 파일 중복 읽기 금지**: 대화 기록에서 이미 읽은 파일은 다시 호출하지 마세요.
- **Multi-Tool 허용 조건**:
  - Read-only 묶음 허용, Write-only 묶음 허용
  - Read A + Update A 절대 금지 (반드시 턴 분리)
...
- **파일 일괄 수정 금지 (Bulk File Replacement Prohibition)**: 여러 파일에서 동일한 텍스트를 찾아 일괄 수정해야 할 때는 **절대 \`find + sed -i\`, \`perl -i\`, \`xargs sed\` 같은 쉘 명령어를 사용하지 마세요.** 이러한 명령어는 플랫폼 의존성, escape 지옥, 결과 추적 불가 등의 문제가 있습니다. **올바른 플로우**: 1) \`ripgrep_search\`로 패턴 검색하여 파일 목록과 라인 번호 확인, 2) 각 파일에 대해 \`read_file\`로 내용 확인, 3) \`update_file\`로 SEARCH/REPLACE 블록을 사용하여 수정. 이렇게 하면 라인 단위 변경, diff 미리보기, 실패 파일만 재시도, 롤백 가능 등의 이점이 있습니다.
