...
export class PromptComposer {
  ...
  private static resolveIncludes(
    ...
    return content.replace(includePattern, (match, includePath) => {
      ...
      if (processedPaths.has(normalizedPath)) {
        console.warn(
          `[PromptComposer] Circular @include detected: ${normalizedPath}`,
        );
        return "";
      }
      ...
      try {
        ...
        return PromptComposer.resolveIncludes(
          included,
  ...
  public static addSkillReference(
    skillKey: string,
    source: "local" | "server",
  ): void {
    ...
    // 중복 방지
    if (
      !PromptComposer._lastReferences.some(
        (r) => r.type === refType && r.name === skillKey,
      )
    ) {
      PromptComposer._lastReferences.push({
        type: refType,
        name: skillKey,
        source,
      });
  ...
  public static getLastIncludedServerRuleKeys(): {
    key: string;
    title: string;
  }[] {
  ...
  public static getSkillDescriptions(): {
    key: string;
    description: string;
    source: string;
  }[] {
    return Array.from(PromptComposer._skillRegistry.values()).map((s) => ({
      key: s.key,
   ...
   */
  public static loadGlobalRulesWithKeys(): {
    text: string;
    ruleKeys: Set<string>;
  } {
    ...
    try {
      const globalDir = path.join(
        PromptComposer._globalRulesDir,
        "global-rules",
      );
      ...
      if (fs.existsSync(globalDir) && fs.statSync(globalDir).isDirectory()) {
        ...
        for (const file of files) {
          ...
          try {
            ...
            rawContent = PromptComposer.resolveIncludes(
              rawContent,
            ...
            if (rulePaths && rulePaths.length > 0) {
              const hasMatch = Array.from(
                PromptComposer._touchedFilePaths,
              ).some((touchedFile) => {
              ...
              if (!hasMatch) {
                console.log(
                  `[PromptComposer] Conditional rule skipped (no matching files): ${filePath}, paths: ${rulePaths.join(", ")}`,
                );
                continue;
              }
            }
            ...
            const { type, description, body } =
              PromptComposer.parseFrontmatter(rawContent);
            const fileKey = file.replace(/\.(md|markdown)$/, "");
            if (type === "skill" && description) {
              skillEntries.push({
                key: `global-rules--${fileKey}`,
                description,
                content: body,
                source: "local",
              });
            ...
            console.warn(
              `[PromptComposer] 글로벌 규칙 파일 읽기 실패: ${filePath}`,
              e,
            );
          }
        }
      ...
      if (skillEntries.length > 0) {
        console.log(
          `[PromptComposer] 글로벌 Skills(조건부) 등록: ${skillEntries.length}개`,
        );
      }
...
${rules.join("\n\n---\n\n")}`,
   ...
   */
  public static loadAgentRulesWithKeys(): {
    text: string;
    ruleKeys: Set<string>;
  } {
    const ruleKeys = new Set<string>();
    ...
    try {
      ...
      const ruleCategories = [
        {
          dir: "stable-version",
          legacyFile: "stable-version.md",
          title: "Version Management Rules",
        },
        {
          dir: "coding-style",
          legacyFile: "coding-style.md",
          title: "Coding Style Rules",
        },
        {
          dir: "project-architecture",
          legacyFile: "project-architecture.md",
          title: "Project Architecture Rules",
        },
        {
          dir: "dependency-policy",
          legacyFile: "dependency-policy.md",
          title: "Dependency Policy Rules",
        },
        {
          dir: "db-policy",
          legacyFile: "db-policy.md",
          title: "Database Policy Rules",
        },
      ];
          ...
          try {
            ...
            for (const file of files) {
              ...
              try {
                ...
                console.warn(
                  `[PromptComposer] Failed to read ${filePath}:`,
                  error,
                );
              }
            }
            ...
            console.warn(
              `[PromptComposer] Failed to read directory ${categoryDir}:`,
              error,
            );
          }
        }
        ...
        if (
          ...
          if (
            ...
            try {
              ...
              console.warn(
                `[PromptComposer] Failed to read ${category.legacyFile}:`,
                error,
              );
            }
          }
      ...
      if (localSkillEntries.length > 0) {
        console.log(
          `[PromptComposer] 로컬 Skills(조건부) 등록: ${localSkillEntries.length}개 — ${localSkillEntries.map((s) => s.key).join(", ")}`,
        );
      }
      ...
      console.log(
        `[PromptComposer] 로컬 Rules 로드: ${rules.length}개 카테고리, 키: [${[...ruleKeys].join(", ")}]`,
      );
...
${rules.join("\n\n---\n\n")}`,
  ...
  public static loadServerPromptTemplates(localRuleKeys: Set<string>): {
    text: string;
    ...
    try {
      ...
      console.log(
        `[PromptComposer] 서버 Skills(dev_rules) 로드: ${rules.length}개 — ${rules.map((r: { key: string; enforcement: string; title?: string; skill_type?: string }) => `[${r.enforcement}/${r.skill_type || "rule"}] ${r.title || r.key}`).join(", ")}`,
      );
      ...
      const filteredRules = rules.filter(
        (r: {
          ...
          if (r.skill_type === "skill" && r.skill_description) {
            ...
            console.log(
              `[PromptComposer] 서버 Skill(조건부) 등록: ${r.title || r.key}`,
            );
      ...
      const formattedRules = filteredRules
        .map(
          (r: {
            key: string;
            content: string;
...
`;
    ...
    const terminalCommandRules =
      taskType === "execution_work" ? base.getTerminalCommandRules() : "";
...
`
    ...
    const { text: agentRulesRaw, ruleKeys: localRuleKeys } =
      this.loadAgentRulesWithKeys();
    const { text: serverPromptTemplates, overrideKeys } =
      this.loadServerPromptTemplates(localRuleKeys);
    ...
    if (activeSkillKeys && activeSkillKeys.length > 0) {
      for (const skillKey of activeSkillKeys) {
        ...
        if (entry) {
          const skillType =
            entry.source === "server" ? "server_skill" : "local_skill";
          references.push({
            type: skillType,
            name: entry.key,
            source: entry.source,
    ...
    for (const prev of prevNonRuleRefs) {
      if (
        !references.some((r) => r.type === prev.type && r.name === prev.name)
      ) {
    ...
    const ragSection = ragContext
      ? `## Reference Documents (RAG) - Use with Priority
...
**Code that ignores Skills is not allowed.**`
    ...
    if (hotLoadPrompt) {
      PromptComposer._essentialRules.push({
        key: "hotload",
        content: hotLoadPrompt,
        source: "hotload",
        precedence: RulePrecedence.HOTLOAD,
...
- The ONLY exceptions are: code, file paths, technical identifiers, CLI commands.`;
    PromptComposer._essentialRules.push({
      ...
      content: koreanRule,
      source: "base",
      precedence: RulePrecedence.BASE_PROMPT,
      enforcement: "required",
      essential: true,
    ...
    if (hotLoadPrompt)
      addRuleLog(10, "hotload", "HotLoad Rules", estimateTokens(hotLoadPrompt));
    if (memoryContext)
      addRuleLog(8, "memory", "Memory Context", estimateTokens(memoryContext));
    if (globalRulesRaw)
      addRuleLog(5, "global", "Global Rules", estimateTokens(globalRulesRaw));
    if (agentRules)
      addRuleLog(7, "local", "Agent Rules", estimateTokens(agentRules));
    if (serverPromptTemplates)
      addRuleLog(
        9,
        "server",
        "Server Rules",
        estimateTokens(serverPromptTemplates),
      );
    if (activeSkillsSection)
      addRuleLog(
        ...
        "skill",
        "Active Skills",
        estimateTokens(activeSkillsSection),
      );
    if (frameworkRulesSection)
      addRuleLog(
        3,
        "framework",
        "Framework Rules",
        estimateTokens(frameworkRulesSection),
      );
    if (ragSection)
      addRuleLog(2, "rag", "RAG Context", estimateTokens(ragSection));
    if (mcpCustomPrompts)
      addRuleLog(4, "mcp", "MCP Prompts", estimateTokens(mcpCustomPrompts));

    const totalRuleTokens =
      ruleLog.length > 0
        ? estimateTokens(parts.filter(Boolean).join("\n\n"))
        : 0;
    if (ruleLog.length > 0) {
      console.log(
        `[PromptComposer] Rules loaded: ${ruleLog.length} sections, ${totalRuleTokens} tokens\n${ruleLog.join("\n")}`,
      );
    }
    ...
    const modelLimits =
      MODEL_TOKEN_LIMITS[modelType] || MODEL_TOKEN_LIMITS[AiModelType.OLLAMA];
    const tokenBudget = Math.floor(modelLimits.maxInputTokens * 0.3);

    if (totalTokens > tokenBudget) {
      console.log(
        `[PromptComposer] System prompt exceeds token budget: ${totalTokens} > ${tokenBudget}. Trimming low-priority sections.`,
      );
    ...
    if (currentHash !== PromptComposer._lastPromptHash) {
      ...
      console.log(
        "[PromptComposer] System prompt changed — ToolSpec cache cleared",
      );
    }
