...
export class PromptComposer {
  ...
  public static async ensureServerSettingsSynced(): Promise<void> {
    // No-op in standalone (no server to sync with)
  }
...
`
    ...
    // Skills load: local(.agent/rules) only — standalone has no server
    const { text: agentRulesRaw, ruleKeys: localRuleKeys } =
      this.loadAgentRulesWithKeys();
    const serverPromptTemplates = "";
    const overrideKeys = new Set<string>();

    // (server override removal — not applicable in standalone)
