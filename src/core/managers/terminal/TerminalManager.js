...
export class TerminalManager {
    ...
    shellIntegrationHistory = new Map();
    constructor() {
    ...
    registerShellIntegrationHandlers() {
        ...
        if (onDidStartExecution && onDidEndExecution) {
            ...
            // 명령어 실행 시작 감지
            const startDisposable = onDidStartExecution.call(windowAny, (event) => {
            ...
            // 명령어 실행 종료 감지
            const endDisposable = onDidEndExecution.call(windowAny, (event) => {
                const terminal = event.terminal;
                const execution = event.execution;
                const terminalName = terminal.name;
    ...
    async executeFileOpFromToken(token) {
        try {
            ...
            try {
                ...
                errorManager.captureError(ErrorSource.TERMINAL, message, {
                    command: 'file-op',
                    cwd: errorPath
                }).catch(() => { });
            }
    ...
    async handleInteractiveCommand(command, projectRoot) {
        ...
        try {
            ...
            const runPromise = this.executionManager.runCommandCapture(finalCommand, { cwd, shell: selectShellForCapture(finalCommand) || true }, (data) => {
                ...
                try {
                    const errorManager = ErrorManager.getInstance();
                    errorManager.captureError(ErrorSource.TERMINAL, decoded, {
            ...
            const result = await this.executionManager.runCommandCapture(this.normalizeEncodedPowerShellCommand(cleanCommand), { cwd, shell: true }, (chunk) => {
                ...
                chunk.split(/\r?\n/).forEach(line => {
                    ...
                    try {
                        const errorManager = ErrorManager.getInstance();
                        errorManager.captureError(ErrorSource.TERMINAL, cleaned, {
    ...
    async tryAutoFixWithCore(failedCommand, errorOutput, cwd, terminalName) {
        try {
            ...
            const parsedError = await errorManager.captureError(ErrorSource.TERMINAL, errorOutput, {
