...
export class TerminalManager {
  ...
  private corruptedPowerShellScripts: Map<
    string,
    { decoded: string; originalCommand: string }
  > = new Map();
   ...
   */
  public setErrorCorrectionServices(
    llmApiClient: any,
   ...
   */
  private async getCorrectedCommand(
    failedCommand: string,
    errorOutput: string,
    cwd: string,
  ): Promise<CorrectedCommandResult | null> {
    if (!this.llmApiClient || !this.currentWebview) {
      console.log(
        "[TerminalManager] LLM API 클라이언트 또는 웹뷰가 설정되지 않음",
      );
      return null;
    }
    ...
    try {
      ...
      const cmdHash = Buffer.from(failedCommand)
        .toString("base64")
        .substring(0, 32);
      const corruptedScript = this.corruptedPowerShellScripts.get(cmdHash);
      if (corruptedScript) {
        console.log(
          "[TerminalManager] 손상된 PowerShell 스크립트 감지, LLM에게 복구 요청",
        );
        const repaired = await this.repairCorruptedPowerShellScript(
          corruptedScript.decoded,
          errorOutput,
        );
        if (repaired) {
          ...
          console.log(
            "[TerminalManager] LLM 스크립트 복구 실패, 일반 오류 수정 시도",
          );
      ...
      try {
        if (projectRoot && fs.existsSync(projectRoot)) {
          ...
          for (const file of files) {
            ...
            if (file.isDirectory()) {
              if (
                /^(src|target|build|lib|resources|config|test|tests|node_modules)$/i.test(
                  name,
                )
              ) {
              ...
              if (
                /\.(xml|gradle|properties|json|yml|yaml|java|kt|py|js|ts|tsx|jsx|md|txt|sh|bat|cmd|ps1|toml|mod)$/i.test(
                  name,
                )
              ) {
                importantFiles.push(name);
              } else if (
                /^(pom|build|package|requirements|setup|Makefile|Dockerfile|Cargo|Gemfile|composer|go)$/i.test(
                  name,
                )
              ) {
                importantFiles.push(name);
          ...
          for (const configFileName of relevantConfigFiles) {
            try {
              ...
              if (fs.existsSync(configPath)) {
                ...
                const configPreview =
                  configContent.length > 2000
                    ? configContent.substring(0, 2000) + "\n... (중간 생략) ..."
                    : configContent;
                const lang =
                  configFileName === "pom.xml"
                    ? "xml"
                    : configFileName.includes("gradle")
                      ? "gradle"
      ...
      const isWindows =
        ...
        this.userOS.toLowerCase().includes("windows") ||
        this.userOS.toLowerCase() === "win32";
...
15. **중요: 쉘 스크립트 생성 조건 및 오류 수정:**
    ...
    - 함수 정의(\`func() { ... }\`), 여러 줄 변수 설정, if/for/while 루프가 포함된 스크립트에서 "syntax error: unexpected end of file" 같은 오류가 발생하면, 스크립트를 파일로 생성하도록 수정하세요`;
      ...
      const osSpecificGuidelines = isWindows
        ? windowsGuidelines
        : unixGuidelines;
      ...
      if (errorOutputForPrompt.length > 50000) {
        ...
        const endPart = errorOutputForPrompt.substring(
          errorOutputForPrompt.length - 40000,
        );
      ...
      const hasCompilationError =
        /(Compilation failure|BUILD FAILURE|build failed|compile error|syntax error|cannot find symbol|cannot find module|Module not found|unmappable character|encoding error)/i.test(
          cleanedErrorOutput,
        );
...
파일 작업이 필요 없는 경우 fileOperations는 빈 배열 []로 설정하세요.`;
      ...
      console.log(
        `[TerminalManager] 컴파일 오류 감지 여부: ${hasCompilationError}`,
      );
      console.log(
        `[TerminalManager] 오류 출력 길이: ${cleanedErrorOutput.length} chars`,
      );

      // ErrorManager를 통해 LLM 호출
      const errorManager = ErrorManager.getInstance();
      const response = await errorManager.sendMessageForErrorCorrection(
        ...
        for (const jsonStr of jsonMatches) {
          try {
            ...
            const hasFileOps =
              ...
              Array.isArray(result.fileOperations) &&
              result.fileOperations.length > 0;

            console.log(
              `[TerminalManager] JSON 파싱 결과: hasValidCommand=${hasValidCommand}, hasFileOps=${hasFileOps}, correctedCommand=${result.correctedCommand}, fileOperations.length=${result.fileOperations?.length || 0}`,
            );

            if (
              hasValidCommand ||
              (hasFileOps &&
                (!result.correctedCommand || result.correctedCommand === null))
            ) {
              if (hasValidCommand) {
                console.log(
                  `[TerminalManager] LLM 오류 수정 성공: ${result.correctedCommand} (신뢰도: ${result.confidence})`,
                );
              } else {
                console.log(
                  `[TerminalManager] LLM 파일 수정 제안: ${hasFileOps ? result.fileOperations.length : 0}개 파일 작업`,
                );
              }
                ...
                for (const op of result.fileOperations) {
                  ...
                  if (
                    ...
                    if (/\.(cmd|bat|sh|ps1)$/i.test(pathLower)) {
                      console.log(
                        `[TerminalManager] 스크립트 파일 작업 거부: ${op.path}`,
                      );
                      continue;
                    }
                    ...
                    if (invalidChars.test(normalizedPath)) {
                      console.log(
                        `[TerminalManager] 경로에 허용되지 않는 문자가 포함되어 거부: ${normalizedPath}`,
                      );
                      continue;
                    }
                    ...
                    if (normalizedPath.length > 260) {
                      console.log(
                        `[TerminalManager] 경로가 너무 길어서 거부: ${normalizedPath.length}자`,
                      );
                      continue;
                    }
                    ...
                    const altB64 =
                      op.contentBase64 || op.content_b64 || op.contentB64;
                    try {
                      if (
                        enc === "base64" &&
                        typeof contentToUse === "string"
                      ) {
                        contentToUse = Buffer.from(
                          contentToUse,
                          "base64",
                        ).toString("utf8");
                      } else if (typeof altB64 === "string" && !contentToUse) {
                        contentToUse = Buffer.from(altB64, "base64").toString(
                          "utf8",
                        );
                      }
                    } catch {
                    ...
                    console.log(
                      `[TerminalManager] 파일 작업 추가: ${finalOpType} ${normalizedPath}`,
                    );
                  }
                }
              }
            ...
            console.warn(
              "[TerminalManager] JSON 파싱 실패, 다음 JSON 시도:",
              parseError,
            );
            continue;
          }
      ...
      const keyMatch = fenceStripped.match(
        /"correctedCommand"\s*:\s*"([^"]*(?:\\.[^"]*)*)"/i,
      );
      if (keyMatch && keyMatch[1]) {
        ...
        if (cmd && cmd.length >= 4 && !cmd.match(/^(\\|""|''|```)/)) {
          console.log(
            "[TerminalManager] Fallback correctedCommand extracted via regex",
          );
          return {
      ...
      try {
        ...
        if (hasPomPath) {
          ...
          if (projectEnd !== -1) {
            if (xmlDeclStart !== -1 && xmlDeclStart < projectEnd) {
              xmlContent = fenceStripped.substring(
                xmlDeclStart,
                projectEnd + "</project>".length,
              );
          ...
          if (xmlContent && xmlContent.length > 0) {
            console.log(
              "[TerminalManager] Fallback: Extracted pom.xml content via regex",
            );
            return {
        ...
        console.warn(
          "[TerminalManager] Fallback fileOperations extraction failed:",
          e,
        );
      }

      // Fallback: 순수 명령문으로만 온 경우
      const singleLine = fenceStripped
        .split("\n")
        .map((s) => s.trim())
        .filter(Boolean)
        .join(" ");
   ...
   */
  private async repairCorruptedPowerShellScript(
    corruptedScript: string,
    errorOutput: string,
  ): Promise<string | null> {
    if (!this.llmApiClient) {
      console.log(
        "[TerminalManager] LLM API 클라이언트가 없어 스크립트 복구 불가",
      );
      return null;
    }
...
JSON 형식으로 응답해주세요:
{
  ...
  "confidence": 0.9}`;
      ...
      console.log(
        "[TerminalManager] LLM에게 손상된 PowerShell 스크립트 복구 요청",
      );
      // ErrorManager를 통해 LLM 호출
      const errorManager = ErrorManager.getInstance();
      const response = await errorManager.sendMessageForErrorCorrection(
        ...
        for (const jsonStr of jsonMatches) {
          try {
            ...
            if (result.correctedCommand && result.confidence > 0.7) {
              console.log(
                `[TerminalManager] LLM이 스크립트 복구 완료 (신뢰도: ${result.confidence})`,
              );
      ...
      for (const pattern of cmdPatterns) {
        ...
        if (cmdMatch && cmdMatch[1]) {
          ...
          if (cmd && cmd.length > 10 && cmd !== "\\" && cmd !== "\\\\") {
            console.log(
              `[TerminalManager] LLM 스크립트 복구 완료 (fallback, 길이: ${cmd.length})`,
            );
            return cmd;
          }
        }
...
---
*이 보고서는 agentgocoder의 자동 오류 수정 시스템에 의해 생성되었습니다.*`;
      ...
      console.log(
        "[TerminalManager] 오류 수정 종합 설명을 채팅창에 전송했습니다.",
      );
    } catch (error) {
  ...
  private normalizeEncodedPowerShellCommand(cmd: string): string {
    try {
      ...
      if (!isValidDecode) {
        console.log(
          "[TerminalManager] Decoded script validation failed, storing for LLM repair",
        );
        const cmdHash = Buffer.from(cmd).toString("base64").substring(0, 32);
        this.corruptedPowerShellScripts.set(cmdHash, {
          decoded,
          originalCommand: cmd,
        });
