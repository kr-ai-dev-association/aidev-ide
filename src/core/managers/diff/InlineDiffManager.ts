...
export class InlineDiffManager {
  ...
  private documentCache: Map<
    string,
    { document: vscode.TextDocument; version: number; timestamp: number }
  > = new Map(); // Document 캐싱
  ...
  private setupEditorChangeListener(): void {
    ...
    this.visibleEditorsDisposable = vscode.window.onDidChangeVisibleTextEditors(
      (editors) => {
  ...
  private isDuplicateChange(
    ...
    for (const existing of existingChanges) {
      // 1. Offset 기반 비교
      if (
        newChange.startOffset === existing.startOffset &&
      ...
      // 2. 텍스트 내용 비교 (정확히 일치)
      if (
        newChange.oldText === existing.oldText &&
      ...
      const normalizedNewOld = this.normalizeTextForComparison(
        newChange.oldText,
      );
      const normalizedNewNew = this.normalizeTextForComparison(
        newChange.newText,
      );
      const normalizedExistingOld = this.normalizeTextForComparison(
        existing.oldText,
      );
      const normalizedExistingNew = this.normalizeTextForComparison(
        existing.newText,
      );

      if (
        normalizedNewOld === normalizedExistingOld &&
      ...
      if (
        normalizedNewNew === normalizedExistingNew &&
        normalizedNewNew.length > 10
      ) {
        // 너무 짧은 코드는 제외 (false positive 방지)
        return true;
      }
        ...
        setTimeout(() => {
          ...
          if (actualVersion < expectedVersion) {
            ...
            for (const docChange of contentChanges) {
              ...
              const docChangeStartOffset = e.document.offsetAt(
                docChangeRange.start,
              );
              const docChangeEndOffset = e.document.offsetAt(
                docChangeRange.end,
              );
              ...
              for (const aiChange of currentChanges) {
                ...
                if (!isDirty) {
                  ...
                  if (
                    !(
                      docChangeEndOffset <= changeStartOffset ||
                      docChangeStartOffset >= changeEndOffset
                    )
                  ) {
                    isDirty = true;
                  }
                ...
                if (!isDirty && aiChange.newText) {
                  // offset 기반으로 현재 range 계산
                  try {
                    const changeCurrentRange = this.getCurrentRange(
                      aiChange,
                      e.document,
                    );
                    ...
                    if (
                      ...
                      aiChange.range = new vscode.Range(
                        ...
                        e.document.lineAt(Math.max(0, e.document.lineCount - 1))
                          .text.length,
                      );
                ...
                if (!isDirty && aiChange.type === "delete") {
                  ...
                  const checkOffset = Math.max(
                    0,
                    Math.min(aiChange.startOffset, currentDocumentText.length),
                  );
                  const checkText = currentDocumentText.substring(
                    checkOffset,
                    Math.min(
                      currentDocumentText.length,
                      checkOffset + aiChange.oldText.length,
                    ),
                  );
                ...
                if (
                  ...
                  const lastLineLen =
                    e.document.lineCount > 0
                      ? e.document.lineAt(lastLine).text.length
                      : 0;
  ...
  public invalidateFile(
    ...
    this.turnCheckpointStack = this.turnCheckpointStack.filter(
      (tc) => tc.fileSnapshots.size > 0,
    );
  ...
  public markFormatterFinished(filePath: string): void {
    ...
    if (this.shadow.has(filePath)) {
      try {
        ...
        /* 파일 읽기 실패 시 무시 (formatter가 파일 삭제했을 경우 등) */
      }
    }
  }
    ...
    return this.withFileLock(filePath, async () => {
      ...
      if (isNewFile) {
        try {
          ...
          console.error(
            "[InlineDiffManager] Failed to create new file:",
            error,
          );
          vscode.window.showErrorMessage(`파일 생성에 실패했습니다: ${error}`);
      ...
      if (!editor) {
        try {
          ...
          vscode.window.showErrorMessage(`파일 열기에 실패했습니다: ${error}`);
          return;
        }
      } else {
      ...
      // ✅ STEP 4: Diff 계산
      // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
      const newChanges = this.analyzeChanges(
        checkpointBeforeContent,
        afterContent,
      );
      ...
      const existingPendingChanges = (existingChanges || []).filter(
        (c) => c.status === "pending",
      );
      const existingProcessedChanges = (existingChanges || []).filter(
      ...
      for (const newChange of contentFilteredChanges) {
        ...
        if (currentDocument) {
          ...
          isDuplicate = this.isDuplicateChange(
            newChange,
      ...
      const updatedExistingPendingChanges = existingPendingChanges.map(
        (existingChange) => {
          ...
          // range + offset 업데이트
          const updatedChange =
            lineOffset !== 0 ? { ...existingChange } : { ...existingChange };
          if (lineOffset !== 0) {
            updatedChange.range = new vscode.Range(
              new vscode.Position(
                existingChange.range.start.line + lineOffset,
                existingChange.range.start.character,
              ),
              new vscode.Position(
                existingChange.range.end.line + lineOffset,
                existingChange.range.end.character,
              ),
            );
          ...
          if (startLine >= 0 && startLine < afterLineStartOffsets.length) {
            updatedChange.startOffset =
              afterLineStartOffsets[startLine] +
              updatedChange.range.start.character;
          }
          if (endLine >= 0 && endLine < afterLineStartOffsets.length) {
            updatedChange.endOffset =
              afterLineStartOffsets[endLine] +
              updatedChange.range.end.character;
          }
      ...
      await this.applyDecorationsWithRetry(
        filePath,
        allChanges,
        afterContent,
        isNewFile,
  ...
  public analyzeChanges(
    ...
    for (let i = 0; i < diffs.length; i++) {
      ...
      if (part.removed) {
        ...
        const startOffset =
          lineStartOffsets[startLine] ||
          (startLine > 0
            ? lineStartOffsets[lineStartOffsets.length - 1] +
              newLines[newLines.length - 1].length
            : 0);
        const endOffset =
          lineStartOffsets[endLine + 1] ||
          (endLine + 1 < lineStartOffsets.length
            ? lineStartOffsets[endLine + 1]
            : newContent.length);
        ...
        if (pendingDelete) {
          ...
          changes.push({
            ...
            filePath: "",
            range: new vscode.Range(
              pendingDelete.startLine,
              0,
              pendingDelete.endLine,
              Number.MAX_SAFE_INTEGER,
            ),
  ...
  public getCurrentRange(
    ...
    try {
      ...
      if (
        ...
        change.endOffset > document.getText().length
      ) {
        return change.range;
      }
  ...
  private applyDecorationsToEditor(
    ...
    for (const change of pendingChanges) {
      try {
        ...
        if (
          currentRange.start.line < 0 ||
          currentRange.end.line >= editor.document.lineCount
        ) {
          continue;
        }
  ...
  private async applyDecorationsWithRetry(
    ...
    if (!currentEditor) {
      ...
      if (retryCount < maxRetries) {
        console.log(
          `[InlineDiffManager] Editor not ready for ${filePath}, retrying (${retryCount + 1}/${maxRetries})...`,
        );
        await new Promise((resolve) => setTimeout(resolve, retryDelay));
        return this.applyDecorationsWithRetry(
          filePath,
          allChanges,
          expectedContent,
          isNewFile,
      ...
      console.warn(
        `[InlineDiffManager] Editor not available after ${maxRetries} retries for ${filePath}`,
      );
      const { DiffCodeLensProvider } = require("./DiffCodeLensProvider");
      DiffCodeLensProvider.getInstance().refresh();
      return;
  ...
  public async acceptChange(filePath: string, changeId: string): Promise<void> {
    ...
    if (checkpoint && change.checkpointId === checkpoint.id) {
      ...
      const changesAfterThisCheckpoint = changes.filter(
        (c) =>
          c.status === "pending" &&
      ...
      if (changesAfterThisCheckpoint.length === 0) {
        ...
        const checkpointChanges = changes.filter(
          (c) => c.checkpointId === checkpoint.id,
        );
        const allProcessed = checkpointChanges.every(
          (c) => c.status === "accepted" || c.status === "rejected",
        );
    ...
    setTimeout(() => {
      ...
      const remainingPending = currentChanges.filter(
        (c) => c.status === "pending",
      );
  ...
  public async rejectChange(filePath: string, changeId: string): Promise<void> {
    ...
    if (change.type === "add") {
      ...
      edit.insert(
        editor.document.uri,
        insertPosition,
        change.oldText + (change.oldText.endsWith("\n") ? "" : "\n"),
      );
    }
    ...
    setTimeout(() => {
      ...
      const pendingChanges = currentChanges.filter(
        (c) => c.status === "pending",
      );
      ...
      if (pendingChanges.length === 0) {
        ...
        const checkpoint = this.checkpoints.get(filePath);
        if (checkpoint) {
          const allAccepted = currentChanges.every(
            (c) => c.status === "accepted",
          );
          const allRejected = currentChanges.every(
            (c) => c.status === "rejected",
          );
          if (allAccepted) {
            checkpoint.status = "accepted";
  ...
  private recalculateOffsetsAfterReject(
    ...
    if (rejectedChange.type === "delete") {
      ...
      for (const sibling of changes) {
        if (sibling.id === rejectedChange.id || sibling.status !== "pending")
          continue;
        if (sibling.newText) {
    ...
    if (rejectedChange.type === "add") {
      ...
      // newText → oldText 교체
      offsetDelta =
        rejectedChange.oldText.length - rejectedChange.newText.length;
    }
   ...
   */
  private recalculateAllOffsetsFromDocument(
    filePath: string,
    document: vscode.TextDocument,
  ): void {
    const changes = this.pendingChanges.get(filePath);
   ...
   */
  private pushTurnSnapshot(
    filePath: string,
    beforeContent: string,
    conversationTurnId: string,
  ): void {
    if (!conversationTurnId || conversationTurnId === "legacy") {
      return;
    }

    let tc = this.turnCheckpointStack.find(
      (t) => t.conversationTurnId === conversationTurnId,
    );
    if (!tc) {
      tc = {
  ...
  private async restoreFileContent(
    ...
    if (content === "") {
      try {
        ...
        console.log(
          `[InlineDiffManager] Deleted new file on undo: ${path.basename(filePath)}`,
        );
      } catch {
        /* 이미 삭제됨 */
      }
  ...
  private async savePersistedState(): Promise<void> {
    ...
    try {
      ...
      for (const [filePath, changes] of this.pendingChanges.entries()) {
        ...
        pendingEntries.push([
          filePath,
          changes.map((c) => this.serializeChange(c)),
        ]);
        ...
        if (sh !== undefined) {
          shadowEntries.push([filePath, sh]);
        }
        const dk = this.disk.get(filePath);
        if (dk !== undefined) {
          diskEntries.push([filePath, dk]);
        }
        ...
        try {
          ...
          if (hash) {
            hashEntries.push([filePath, hash]);
          }
        }
      }
      ...
      const serializedTurnStack: SerializableTurnCheckpoint[] =
        this.turnCheckpointStack.map((tc) => ({
          id: tc.id,
  ...
  private async loadPersistedState(): Promise<void> {
    ...
    try {
      const state =
        this.extensionContext.globalState.get<PersistedDiffState>(
          "inlineDiffState_v2",
        );
      ...
      if (Date.now() - state.savedAt > MAX_AGE) {
        console.log(
          "[InlineDiffManager] Persisted state expired (>24h), discarding",
        );
        await this.extensionContext.globalState.update(
          "inlineDiffState_v2",
          undefined,
      ...
      for (const [filePath, serializedChanges] of state.pendingChanges) {
        ...
        if (savedHash) {
          try {
            ...
            if (currentHash !== savedHash) {
              console.log(
                `[InlineDiffManager] File hash mismatch for ${filePath} (saved=${savedHash.substring(0, 8)}..., current=${currentHash.substring(0, 8)}...), discarding`,
              );
              continue;
            }
          } catch {
            console.log(
              `[InlineDiffManager] Cannot read file ${filePath}, discarding`,
            );
            continue;
          }
        }
        ...
        // 복원
        const changes = serializedChanges.map((sc) =>
          this.deserializeChange(sc),
        );
        this.pendingChanges.set(filePath, changes);
        if (savedHash) {
          this.storedFileHashes.set(filePath, savedHash);
        }
        ...
        if (shEntry) {
          this.shadow.set(filePath, shEntry[1]);
        }
        const dkEntry = state.disk.find(([fp]) => fp === filePath);
        if (dkEntry) {
          this.disk.set(filePath, dkEntry[1]);
        }
      ...
      if (state.turnCheckpointStack && state.turnCheckpointStack.length > 0) {
        ...
        console.log(
          `[InlineDiffManager] Restored turn checkpoint stack: ${this.turnCheckpointStack.length} entries`,
        );
      }

      if (restoredFiles > 0) {
        console.log(
          `[InlineDiffManager] Restored persisted state: ${restoredFiles} files`,
        );
  ...
  private scheduleDecorationRestore(): void {
    ...
    this.restorationDisposable = vscode.window.onDidChangeVisibleTextEditors(
      (editors) => {
        for (const editor of editors) {
          const filePath = editor.document.uri.fsPath;
          const changes = this.pendingChanges.get(filePath);
  ...
  public async acceptChangesByTurn(conversationTurnId: string): Promise<void> {
    console.log(
      `[InlineDiffManager] acceptChangesByTurn: ${conversationTurnId}`,
    );
    ...
    for (const [filePath, changes] of this.pendingChanges.entries()) {
      const turnChanges = changes.filter(
        (c) =>
          c.conversationTurnId === conversationTurnId && c.status === "pending",
      );
    ...
    if (idx !== -1) {
      ...
      console.log(
        `[InlineDiffManager] Removed TurnCheckpoint for ${conversationTurnId.substring(0, 8)}, stack size: ${this.turnCheckpointStack.length}`,
      );
    }
  ...
  public async rejectChangesByTurn(conversationTurnId: string): Promise<void> {
    ...
    if (targetIndex === -1) {
      console.warn(
        `[InlineDiffManager] No TurnCheckpoint for ${conversationTurnId}, legacy fallback`,
      );
    ...
    const turnIdsToUndo = new Set(
      turnsToUndo.map((tc) => tc.conversationTurnId),
    );
    console.log(
      `[InlineDiffManager] Cascade undo: ${turnsToUndo.length} turn(s) from index ${targetIndex}`,
    );
    ...
    for (const [, changes] of this.pendingChanges) {
      if (
        changes.some(
          (c) =>
            turnIdsToUndo.has(c.conversationTurnId) && c.status === "pending",
        )
      ) {
        hasPending = true;
        break;
    ...
    for (const [fp] of filesToRestore) {
      ...
      const remaining = changes.filter(
        (c) => !turnIdsToUndo.has(c.conversationTurnId),
      );
      if (remaining.length === 0) {
        ...
        if (restored === "") {
          ...
          try {
            ...
            console.log(
              `[InlineDiffManager] Deleted newly created file on undo: ${fp}`,
            );
          } catch (delErr) {
            console.warn(
              `[InlineDiffManager] Failed to delete file on undo: ${fp}`,
              delErr,
            );
          }
        } else {
    ...
    this._onPendingChangedEmitter.fire();
    console.log(
      `[InlineDiffManager] Cascade undo complete. Stack size: ${this.turnCheckpointStack.length}`,
    );
  }
   ...
   */
  private async rejectChangesByTurnLegacy(
    conversationTurnId: string,
  ): Promise<void> {
    console.log(
      `[InlineDiffManager] rejectChangesByTurnLegacy: ${conversationTurnId}`,
    );
  ...
  public getPendingChangesByTurn(): Array<{
    ...
    for (const [filePath, changes] of this.pendingChanges.entries()) {
      for (const change of changes) {
        if (change.status !== "pending" || !change.conversationTurnId) {
          continue;
        }
    ...
    for (const [turnId, fileMap] of turnMap.entries()) {
      ...
      const cascadeUndoTurnCount =
        stackIndex !== -1 ? this.turnCheckpointStack.length - stackIndex : 1;
