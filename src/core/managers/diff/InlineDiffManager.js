...
export class InlineDiffManager {
    ...
    formatterRunning = new Set(); // filePath -> formatter 실행 중 플래그 (diff 보호용)
    formatterJustFinished = new Set(); // filePath -> formatter 방금 종료됨 플래그 (다음 document 변경 1회만 무시)
    lastAppliedChanges = new Map(); // filePath -> 마지막으로 적용된 changes (ToolExecutionCoordinator용 캐시)
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    ...
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    shadow = new Map(); // filePath -> 작업 중인 가상 문서 (누적 상태)
    disk = new Map(); // filePath -> 마지막 Accept된 상태 (또는 원본)
    addedDecoration;
    deletedDecoration;
    ...
    fileSystemWatcher;
    documentCache = new Map(); // Document 캐싱
    constructor() {
        // 추가된 라인 (초록색)
     ...
     */
    isDuplicateChange(newChange, existingChanges, document) {
     ...
     */
    calculateOverlapPercentage(range1, range2) {
        ...
        const totalLines = Math.max(range1.end.line - range1.start.line + 1, range2.end.line - range2.start.line + 1);
        if (totalLines === 0)
            return 0;
                        ...
                        for (const aiChange of currentChanges) {
                            ...
                            if (!isDirty && aiChange.newText) {
                                ...
                                try {
                                    ...
                                    if (isNewFileCheckpoint &&
                                        aiChange.type === 'add' &&
                                        aiChange.startOffset === 0) {
                                        aiChange.newText = currentDocumentText;
                                        aiChange.range = new vscode.Range(0, 0, e.document.lineCount - 1, e.document.lineAt(Math.max(0, e.document.lineCount - 1)).text.length);
    ...
    getCheckpointBeforeContent(filePath) {
        const checkpoint = this.checkpoints.get(filePath);
        if (checkpoint) {
    ...
    applyDecorationsToEditor(editor, changes) {
        ...
        const addedRanges = [];
        const deletedDecorations = []; // ✅ decoration.before 사용
     ...
     */
    async applyDecorationsWithRetry(filePath, allChanges, expectedContent, isNewFile, retryCount = 0, providedEditor) {
        ...
        let currentEditor = providedEditor || vscode.window.visibleTextEditors.find(e => e.document.uri.fsPath === filePath);
        if (!currentEditor) {
    ...
    async acceptAllChanges(filePath) {
        ...
        // 파일이 열려있지 않으면 먼저 열기
        let editors = vscode.window.visibleTextEditors.filter(e => e.document.uri.fsPath === absolutePath);
        if (editors.length === 0) {
            try {
     ...
     */
    getLastAppliedChanges(filePath) {
