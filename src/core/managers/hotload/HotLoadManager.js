...
export class HotLoadManager {
     ...
     * Hot Load 항목 추가
     */
    async addHotLoad(keywords, description, command, completionCondition, maxRetries, onFailure) {
     ...
     * Hot Load 항목 수정
     */
    async updateHotLoad(id, keywords, description, command, completionCondition, maxRetries, onFailure) {
        await this.initialize();
     ...
     */
    async matchKeywordWithLLM(userQuery, llmManager) {
        await this.initialize();
...
- 설명이나 추가 텍스트 없이 숫자만 응답`;
     ...
     */
    async executeWithRetry(item, workspaceRoot, webview) {
        const maxAttempts = (item.maxRetries || 0) + 1; // 최초 1회 + 재시도 횟수
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                if (attempt > 1) {
                    console.log(`[HotLoadManager] Retry ${attempt - 1}/${item.maxRetries} for: ${item.command}`);
                    // 재시도 전 1초 대기
...
//# sourceMappingURL=HotLoadManager.js.map