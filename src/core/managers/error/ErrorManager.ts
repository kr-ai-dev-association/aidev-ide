...
export class ErrorManager {
  ...
  public async captureError(
    ...
    if (this.executionManager) {
      ...
      if (errorInfo) {
        ...
        const parsedError = this.convertErrorInfoToParsedError(
          errorInfo,
          output,
          source,
          metadata,
   ...
   * 터미널 에러 이벤트를 발행합니다
   */
  private fireTerminalErrorEvent(
    parsedError: ParsedError,
    metadata?: any,
  ): void {
    const terminalName =
      metadata?.terminalName || metadata?.source || "unknown";
        ...
        level:
          log.error.severity === ErrorSeverity.CRITICAL
            ...
            : log.error.severity === ErrorSeverity.HIGH
              ? "error"
              : log.error.severity === ErrorSeverity.MEDIUM
                ? "warn"
                : "info",
        source: "terminal",
