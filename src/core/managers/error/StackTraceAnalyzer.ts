...
export class StackTraceAnalyzer {
    ...
    public analyze(stackTrace: StackTrace): {
        ...
        // 사용자 코드와 라이브러리 코드 분리
        const userCodeFrames: StackFrame[] = [];
        const libraryFrames: StackFrame[] = [];
     ...
     * 사용자 코드인지 확인합니다
     */
    private isUserCode(frame: StackFrame): boolean {
        if (!frame.file) {
            return false;
        }
     ...
     */
    public extractErrorLocation(stackTrace: StackTrace): ErrorLocation | undefined {
     ...
     */
    public simplify(stackTrace: StackTrace, maxFrames: number = 10): StackTrace {
        const analysis = this.analyze(stackTrace);
     ...
     * 스택 트레이스의 깊이를 가져옵니다
     */
    public getDepth(stackTrace: StackTrace): number {
     ...
     */
    public findFrameByFile(stackTrace: StackTrace, fileName: string): StackFrame | undefined {
        return stackTrace.frames.find(frame => 
            frame.file && frame.file.includes(fileName)
        );
    }
     ...
     */
    public findFrameByFunction(stackTrace: StackTrace, functionName: string): StackFrame | undefined {
        return stackTrace.frames.find(frame => 
            frame.function && frame.function.includes(functionName)
        );
    }
}
