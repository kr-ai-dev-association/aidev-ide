...
export class AutoFix {
     ...
     */
    async getHeuristicFix(error, context) {
        ...
        if ((output.includes('esbuild') && output.includes('SyntaxError')) ||
        ...
        if ((output.includes('ENOTEMPTY') && output.includes('node_modules')) ||
        ...
        if ((output.includes('Must use import to load ES Module') || output.includes('Cannot use import statement')) &&
     ...
     */
    async executeCorrectedCommand(command, context) {
     ...
     */
    isValidCommand(command, context) {
        if (!command)
            return false;
