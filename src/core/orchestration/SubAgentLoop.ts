...
export class SubAgentLoop {
  ...
  async run(): Promise<AgentLoopResult> {
    ...
    const isNativeUserEnabled =
      await SettingsManager.getInstance().isNativeToolCallingEnabled();
    ...
    const nativeTools = isNativeAdmin
      ? ToolSpecBuilder.buildOpenAIToolsConfig(allowedTools, true)
      : undefined;
    ...
    while (turnCount < effectiveMaxTurns) {
      ...
      if (turnCount > 0 && conversationParts.length > 10) {
        const totalChars = conversationParts.reduce(
          ...
          if (conversationParts.length > keepFirst + keepLast + 2) {
            ...
            for (const p of middle) {
              ...
              const fileMatches = text.match(
                /(?:read_file|create_file|update_file|remove_file)[:\s]+([^\n\]]+)/g,
              );
              if (fileMatches)
                fileMatches.forEach((m) => middleFiles.add(m.substring(0, 60)));
            }
            const summary = `[Previous turns summary] ${middle.length} messages compacted. Files worked on: ${[...middleFiles].join(", ") || "(no tool calls)"}`;
            conversationParts.splice(keepFirst, middle.length, {
              text: summary,
            });
            console.log(
              `[SubAgentLoop:${this.subtask.id}] Context compacted: ${middle.length} messages → 1 summary (${totalChars} → ${conversationParts.reduce((s, p) => s + (p.text?.length || 0), 0)} chars)`,
            );
          }
        }
      }
      ...
      if (
        ...
        AgentConfig.SUB_AGENT_TOTAL_TIMEOUT
      ) {
        errors.push(
          `Total execution time exceeded (${AgentConfig.SUB_AGENT_TOTAL_TIMEOUT / 1000}s)`,
        );
        break;
      }
      ...
      try {
          ...
          if (this.useStreaming) {
            ...
            const executeStreamingCreate = (
              ...
              streamingCreatePromise = streamingCreatePromise.then(async () => {
                ...
                if (needsApproval && this.callbacks?.onToolApprovalRequired) {
                  const approved =
                    await this.callbacks.onToolApprovalRequired(capturedCall);
                  if (!approved) {
                    return;
                  }
                }
                ...
                if (streamResults[0]?.success && capturedCall.params.path) {
                  ...
                  if (
                    !createdFiles.includes(streamPath) &&
                    !modifiedFiles.includes(streamPath)
                  ) {
            ...
            const onNativeToolComplete = (
              ...
              if (
                ...
                streamingHandledPaths.has(p)
              ) {
                return;
              }
              const capturedCall: ToolUse = {
                name: toolName,
                params: { ...args },
              };
            ...
            const onChunk = (chunk: string) => {
              ...
              if (!this.callbacks?.onStreamingStatus) {
                return;
              }
              ...
              if (
                !lastReportedTool &&
                streamBuffer.length % 500 < chunk.length
              ) {
                const tokens = estimateTokens(streamBuffer);
                this.callbacks.onStreamingStatus(
            ...
            const timeoutId = setTimeout(
              ...
              AgentConfig.SUB_AGENT_LLM_CALL_TIMEOUT,
            );
            ...
            try {
              ...
              if (timeoutController.signal.aborted) {
                ...
                consecutiveFailures++;
                errors.push(
                  `LLM call timeout (${AgentConfig.SUB_AGENT_LLM_CALL_TIMEOUT / 1000}s)`,
                );
                if (consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) {
                  errors.push(
                    "Consecutive failure limit exceeded, aborting agent",
                  );
                  break;
                }
        ...
        if (maxTokensReached) {
          ...
          console.warn(
            `[SubAgentLoop:${this.subtask.id}] MAX_TOKENS detected — will inject continuation prompt`,
          );
        }
        ...
        if (!trimmed) {
          consecutiveFailures++;
          console.warn(
            `[SubAgentLoop:${this.subtask.id}] Empty/thinking-only response (${consecutiveFailures}/${MAX_CONSECUTIVE_FAILURES})`,
          );
          if (consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) {
            errors.push(
              "The LLM only repeated thinking without producing any tool calls or text responses. The model may not have understood the task instructions.",
            );
            break;
          }
        ...
        const strippedForParse = response
          .replace(THINKING_TAG_REGEX, "")
          .trim();
        const toolCalls = ToolParser.parseCodeBlockFormat(
          strippedForParse,
          parseWarnings,
        );
        ...
        if (executableCalls.length === 0 && doneCall) {
          ...
          const hasExhaustedRetries =
            failedUpdateFilePaths.length > 0 &&
            failedUpdateFilePaths.every((fp) => {
              const count = (this as any)._fileFailureCounts?.get(fp) || 0;
              return count >= MAX_SAME_FILE_UPDATE_FAILURES;
            });
          const blockDone =
            failedUpdateFilePaths.length > 0 &&
            !hasExecutedWriteTools &&
            !hasExhaustedRetries &&
            (status !== "already_done" || this.isRepairAgent);
          if (blockDone) {
            const paths = failedUpdateFilePaths.join(", ");
            console.warn(
              `[SubAgentLoop:${this.subtask.id}] Standalone __done__ rejected: unresolved update_file failures for: ${paths}`,
            );
            conversationParts.push({
          ...
          if (hasExhaustedRetries) {
            console.warn(
              `[SubAgentLoop:${this.subtask.id}] __done__ allowed despite failures — max retries (${MAX_SAME_FILE_UPDATE_FAILURES}) exhausted for: ${failedUpdateFilePaths.join(", ")}`,
            );
          }

          console.log(
            `[SubAgentLoop:${this.subtask.id}] __done__ signal: status=${status}, summary=${summary.substring(0, 100)}`,
          );
          if (
            this.subtask.toolPermission === "full" &&
            !hasExecutedWriteTools &&
            status !== "already_done"
          ) {
            warnings.push(
              `Completed without file modifications -- the model declared task completion with __done__(${status}).`,
            );
          }
        ...
        if (this.isAgentMode && !doneCall && executableCalls.length === 0) {
          console.log(
            `[SubAgentLoop:${this.subtask.id}] AGENT mode: Text-only response — completing.`,
          );
          completedNormally = true;
        ...
        if (executableCalls.length === 0) {
          ...
          const unknownToolWarnings = parseWarnings.filter((w) =>
          ...
          if (unknownToolWarnings.length > 0) {
            ...
            console.warn(
              `[SubAgentLoop:${this.subtask.id}] Unknown tool names: ${unknownNames}. Re-prompting.`,
            );
            conversationParts.push({ text: response });
            conversationParts.push({
          ...
          consecutiveFailures++;
          const reason =
            needsWrite && hasExecutedTools
              ? "read-only tools only, no write operations"
              : "no prior tool execution";
          console.warn(
            `[SubAgentLoop:${this.subtask.id}] No tool calls (${reason}) — treating as failure (${consecutiveFailures}/${MAX_CONSECUTIVE_FAILURES})`,
          );
          if (consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) {
            errors.push(
              needsWrite && hasExecutedTools
        ...
        const uniqueCalls = allowedCalls.filter((call) => {
          ...
          if (seenInTurn.has(key)) {
            console.log(
              `[SubAgentLoop:${this.subtask.id}] Same-turn duplicate removed: ${call.name} ${call.params.path || ""}`,
            );
            return false;
          }
          seenInTurn.add(key);
          ...
          if (
            ...
            console.log(
              `[SubAgentLoop:${this.subtask.id}] Skipping streaming-pre-executed create_file: ${call.params.path}`,
            );
            return false;
          }
          ...
          if (call.name === "read_file" && call.params.path) {
            ...
            if (
              !this.disableReadDedup &&
              alreadyReadFiles.has(call.params.path)
            ) {
              console.log(
                `[SubAgentLoop:${this.subtask.id}] Cross-turn duplicate read skipped: ${call.params.path}`,
              );
          ...
          if (call.name === "stat_file" && call.params.path) {
            ...
            if (alreadyStattedFiles.has(call.params.path)) {
              console.log(
                `[SubAgentLoop:${this.subtask.id}] Cross-turn duplicate stat skipped: ${call.params.path}`,
              );
          ...
          if (call.name === "update_file" && call.params.path) {
            ...
            if (createFilesInTurn.has(call.params.path)) {
              console.log(
                `[SubAgentLoop:${this.subtask.id}] Skipped update_file after create_file in same turn: ${call.params.path}`,
              );
              skippedUpdateFiles.push({
        ...
        for (let ci = 0; ci < callsToExecute.length; ci++) {
          ...
          if (call.name === "update_file" && call.params.path) {
            ...
            if (failCount >= MAX_SAME_FILE_UPDATE_FAILURES) {
              console.warn(
                `[SubAgentLoop:${this.subtask.id}] Skipping update_file for ${fp} — ${failCount} failures exceeded limit (${MAX_SAME_FILE_UPDATE_FAILURES})`,
              );
        ...
        const results =
          filteredCallsToExecute.length > 0
        ...
        for (const path of streamingCreatedPaths) {
          if (
            allowedCalls.some(
              (c) => c.name === "create_file" && c.params.path === path,
            )
          ) {
            uniqueCalls.push({
              name: "create_file",
            ...
            results.push({
              success: true,
              message: `[Created during streaming] File ${path} was created immediately during streaming.`,
            });
          }
        }
        ...
        for (const skippedPath of skippedReadFiles) {
          uniqueCalls.push({
            name: "read_file",
            params: { path: skippedPath },
          } as ToolUse);
          // Include cached file content so LLM can proceed to next step
          let cachedContent = "";
          try {
            const absPath = path.resolve(
              this.toolContext.workspaceRoot || this.toolContext.projectRoot,
              skippedPath,
            );
          ...
          if (callResult?.success) {
            ...
            if (!READ_ONLY_TOOLS.has(callItem.name)) {
              ...
              if (callItem.name === "update_file" && callItem.params.path) {
                failedUpdateFilePaths = failedUpdateFilePaths.filter(
                  (p) => p !== callItem.params.path,
                );
              }
            }
            ...
            !callResult?.success &&
            callItem.name === "update_file" &&
            callItem.params.path
          ) {
            ...
            if (!(this as any)._fileFailureCounts)
              (this as any)._fileFailureCounts = new Map<string, number>();
            ...
            console.warn(
              `[SubAgentLoop:${this.subtask.id}] update_file FAILED for: ${fp} (attempt ${prevCount + 1}/${MAX_SAME_FILE_UPDATE_FAILURES})`,
            );
          }
        }
        ...
        if (this.subtask.toolPermission === "full" && !this.isAgentMode) {
          if (hasWriteToolInThisTurn) {
            ...
            if (consecutiveReadOnlyTurns >= MAX_READONLY_CONSECUTIVE_TURNS) {
              console.log(
                `[SubAgentLoop:${this.subtask.id}] ${consecutiveReadOnlyTurns} consecutive read-only turns — nudging write phase`,
              );
              conversationParts.push({ text: response });
              const toolResultsText = this.formatToolResults(
                uniqueCalls,
                results,
              );
          ...
          if (result?.success && result.filePath) {
            ...
            if (call.name === "remove_file") {
              ...
              const alreadyTracked =
                createdFiles.includes(result.filePath) ||
                modifiedFiles.includes(result.filePath);
              if (!alreadyTracked) {
        ...
        if (
          hasWriteToolInThisTurn &&
          (createdFiles.length > 0 || modifiedFiles.length > 0)
        ) {
          try {
            const { TestRunner } =
              await import("../managers/conversation/handlers/TestRunner");
            const workspaceRoot =
            ...
            if (diagnosticErrors.length > 0) {
              ...
              console.log(
                `[SubAgentLoop:${this.subtask.id}] Inline diagnostics: ${diagnosticErrors.length} errors detected`,
              );
            }
          } catch (e) {
            console.warn(
              `[SubAgentLoop:${this.subtask.id}] Inline diagnostics check failed:`,
              e,
            );
          }
        }
        ...
        const updateFailedPaths = uniqueCalls
          ...
          .filter(
            ({ call, result }) =>
              call.name === "update_file" && !result?.success,
          )
        ...
        if (updateFailedPaths.length > 0) {
          ...
          for (const filePath of updateFailedPaths) {
            try {
              ...
              contentParts.push(
                `--- ${filePath} (current actual file content) ---\n${content}\n---`,
              );
            } catch {
              /* ignore read failure */
            }
          ...
          if (contentParts.length > 0) {
            ...
            console.log(
              `[SubAgentLoop:${this.subtask.id}] Auto-injected current content for ${updateFailedPaths.length} failed update_file(s)`,
            );
          }
        }
        if (maxTokensReached) {
          conversationParts.push({
            text: "[System] The previous response was truncated due to max_tokens. If there are any truncated tool calls or file content, output them again completely from the beginning. Continue with the task.",
          });
        }
        ...
        if (doneCall) {
          ...
          if (status !== "already_done" || this.isRepairAgent) {
            ...
            const hasAccumulatedFailures =
              failedUpdateFilePaths.length > 0 && !hasExecutedWriteTools;
            ...
            if (
              (currentTurnUpdateFailures.length > 0 ||
                hasAccumulatedFailures) &&
              !allRetriesExhausted
            ) {
              const failedPaths =
                currentTurnUpdateFailures.length > 0
                  ? currentTurnUpdateFailures
                      .map(({ call }) => call.params.path || "?")
                      .join(", ")
                  : failedUpdateFilePaths.join(", ");
              console.warn(
                `[SubAgentLoop:${this.subtask.id}] __done__ rejected: unresolved update_file failures for: ${failedPaths}`,
              );
              conversationParts.push({
            ...
            if (allRetriesExhausted) {
              console.warn(
                `[SubAgentLoop:${this.subtask.id}] __done__ allowed despite failures — max retries exhausted`,
              );
            }
          }
    ...
    if (!completedNormally && hasWrittenFiles) {
      warnings.push(
        `Agent did not terminate normally but file writing was completed (${createdFiles.length} created, ${modifiedFiles.length} modified). Falling back to system verification.`,
      );
    ...
    const completionSummary = [
      ...
      lastResponse
        ? `Summary: ${SubAgentLoop.extractHeadTail(lastResponse.replace(THINKING_TAG_REGEX, "").trim(), 800, 400)}`
        : null,
    ]
      .filter(Boolean)
      .join("\n");
...
${
  nativeMode
    ? `## Tool Call Examples (API Function Call format)
...
**Important: Only call tools via API function calls. Do not output { "tool": ... } JSON in text. Always call __done__ when the task is finished.**`
    : `## Tool Call Examples (you must follow this format)
...
**Important: To use a tool, output the JSON format above directly in your response. Output only JSON without explanatory text. Do not put tool calls inside thinking. Always call the __done__ tool when the task is finished.**`
  ...
  private getAllowedTools(): Tool[] {
    const exclude = (tools: Tool[]) =>
      tools.filter((t) => !SubAgentLoop.MAIN_LOOP_ONLY_TOOLS.has(t));
  ...
  private filterByPermission(toolCalls: ToolUse[]): ToolUse[] {
    ...
    return toolCalls.filter((call) => {
      ...
      if (
        this.subtask.toolPermission === "read-only-with-commands" &&
        call.name === "run_command"
      ) {
        return true;
      }
      console.warn(
        `[SubAgentLoop:${this.subtask.id}] Blocked tool ${call.name} (permission: ${this.subtask.toolPermission})`,
      );
      return false;
    });
  }
   ...
   */
  private static extractHeadTail(
    text: string,
    head: number,
    tail: number,
  ): string {
  ...
  private parseStreamingToolStatus(content: string): string | null {
    ...
    const jsonPattern =
      /^\s*\{\s*"tool"\s*:\s*"(\w+)"[^}]*"(?:path|filePath)"\s*:\s*"([^"]+)"/gm;
    ...
    const cmdPattern =
      /^\s*\{\s*"tool"\s*:\s*"run_command"[^}]*"command"\s*:\s*"([^"]+)"/gm;
    ...
    if (lastMatch && lastMatch.tool === "run_command") {
      const cmd =
        lastMatch.file.length > 30
          ? lastMatch.file.substring(0, 30) + "..."
          : lastMatch.file;
      return `명령 준비 중: ${cmd}`;
    }
  ...
  private buildRecoveryNudge(failureCount: number): string {
    ...
    return (
      `[System] Warning: ${failureCount} consecutive failures. You must output one of the following:\n` +
