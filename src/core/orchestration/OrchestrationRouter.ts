...
export class OrchestrationRouter {
  ...
  static async route(options: RouteOptions): Promise<void> {
    const orchestrationEnabled =
      ConfigurationService.get<boolean>("orchestration", false) ?? false;
    ...
    if (
      ...
      options.promptType === PromptType.PLAN ||
      options.promptType === PromptType.GENERAL_ASK ||
      options.promptType === PromptType.AGENT
    ) {
    ...
    try {
      ...
      try {
        const { HotLoadManager } =
          await import("../managers/hotload/HotLoadManager");
      ...
      // B방식 (순수 분리): TaskSplitter 에 rule 을 주입하지 않는다.
      // TaskSplitter 는 "뭘 나눌지" 만 결정 + subtask.description 에 stack/tech 언급 금지 (system prompt 로 강제).
      // 실제 stack 선택은 SubAgent 단계에서 rules 보고 결정.
      ...
      console.log(
        `[OrchestrationRouter] Splitting into ${splitResult.subtasks.length} subtasks: ${splitResult.reasoning}`,
      );

      // Separate independent/dependent tasks
      const independentTasks = splitResult.subtasks.filter(
        (st) => st.dependencies.length === 0,
      );
      const dependentTasks = splitResult.subtasks.filter(
        (st) => st.dependencies.length > 0,
      );
      ...
      if (skillDescriptions.length > 0) {
        try {
          ...
          if (candidateSkillKeys.length > 0) {
            console.log(
              `[OrchestrationRouter] candidateSkillKeys from IntentDetector: ${candidateSkillKeys.join(", ")}`,
            );
          }
        } catch (e) {
          console.warn(
            "[OrchestrationRouter] Failed to detect candidateSkillKeys:",
            e,
          );
        }
      }
      ...
      try {
        const gathered = await OrchestrationRouter.gatherRulesContext(
          options.userQuery,
          candidateSkillKeys,
        );
        ...
        if (rulesContext) {
          console.log(
            `[OrchestrationRouter] Rules context gathered (${rulesContext.length} chars)`,
          );
        }
        ...
        console.warn(
          "[OrchestrationRouter] Failed to gather rules context:",
          e,
        );
      }
      ...
      if (poolErrors.length > 0) {
        console.warn(
          `[OrchestrationRouter] ${poolErrors.length} agent(s) failed during parallel execution`,
        );
      ...
      if (dependentTasks.length > 0 && !options.abortSignal?.aborted) {
        const completedIds = new Set(
          results.filter((r) => r.success).map((r) => r.subtaskId),
        );
        ...
        for (const subtask of dependentTasks) {
          ...
          if (!depsOk) {
            const missing = subtask.dependencies.filter(
              (d) => !completedIds.has(d),
            );
            console.warn(
              `[OrchestrationRouter] Skipping ${subtask.id}: dependencies not met (${missing.join(", ")})`,
            );
            ...
            results.push({
              ...
              deletedFiles: [],
              errors: [
                `Prerequisite tasks incomplete: ${missing.map((d) => d.replace(/^task-/, "agent ")).join(", ")}`,
              ],
              warnings: [],
              completionSummary: "",
              turnCount: 0,
          ...
          const enrichedSubtask =
            await OrchestrationRouter.enrichWithPriorResults(
              subtask,
              results,
      ...
      if (!options.abortSignal?.aborted) {
        const failedResults = results.filter(
          (r) => !r.success && r.turnCount > 0,
        );
        if (failedResults.length > 0) {
          console.log(
            `[OrchestrationRouter] ${failedResults.length} subtask(s) failed, retrying...`,
          );
          ...
          for (const failedResult of failedResults) {
            ...
            const originalSubtask = splitResult.subtasks.find(
              (st) => st.id === failedResult.subtaskId,
            );
            if (!originalSubtask) {
              continue;
            }
                ...
                ? `\n## Other agents' completed work\n${successResults
                    .map((r) => {
                      const cleaned = r.response
                        ...
                        .trim();
                      return cleaned
                        ? `- ${r.subtaskId}: ${cleaned.substring(0, 200)}`
                        : "";
                    })
                    .filter(Boolean)
                    .join("\n")}\n`
            ...
            const failedIdx = results.findIndex(
              (r) => r.subtaskId === failedResult.subtaskId,
            );
            if (failedIdx !== -1 && retryResult.success) {
              results[failedIdx] = {
                ...retryResult,
                subtaskId: failedResult.subtaskId,
              };
              console.log(
                `[OrchestrationRouter] Retry succeeded for ${failedResult.subtaskId}`,
              );
            } else if (failedIdx !== -1) {
              ...
              console.warn(
                `[OrchestrationRouter] Retry also failed for ${failedResult.subtaskId}`,
              );
            }
          }
        }
      ...
      if (!options.abortSignal?.aborted) {
        ...
        for (const skipped of skippedResults) {
          ...
          const originalSubtask = splitResult.subtasks.find(
            (st) => st.id === skipped.subtaskId,
          );
          if (!originalSubtask) {
            continue;
          }

          const depsOk = originalSubtask.dependencies.every((dep) =>
            nowCompletedIds.has(dep),
          );
          if (!depsOk) {
            continue;
          }
          ...
          console.log(
            `[OrchestrationRouter] Dependencies now met for ${skipped.subtaskId}, executing...`,
          );
          ...
          const skippedIdx = results.findIndex(
            (r) => r.subtaskId === skipped.subtaskId,
          );
          if (skippedIdx !== -1) {
      ...
      try {
        ...
        console.error(
          "[OrchestrationRouter] Post-merge validation threw:",
          validationError,
        );
      }
      ...
      try {
        ...
        console.warn(
          "[OrchestrationRouter] Failed to generate unified summary:",
          e,
        );
      }
      ...
      const summaryMessage = OrchestrationRouter.formatMergedResult(
        merged,
        validationResult,
        unifiedSummary,
      );
      ...
      if (collectedReferences.length > 0) {
        console.log(
          `[OrchestrationRouter] Sending ${collectedReferences.length}개 references to webview`,
        );
        WebviewBridge.sendReferenceInfo(webview, {
          items: collectedReferences,
        });
      }
      ...
      try {
        const { InlineDiffManager } =
          await import("../managers/diff/InlineDiffManager");
        const turnStats =
          InlineDiffManager.getInstance().getPendingChangesByTurn();
      ...
      await OrchestrationRouter.saveToHistory(
        options,
        merged,
        collectedUIMessages,
        startTime,
      ...
      if (promptSuggestionEnabled) {
        try {
          const { PromptSuggestionService } =
            await import("../managers/suggestion/PromptSuggestionService");
      ...
      console.error(
        "[OrchestrationRouter] Orchestration failed, falling back to single loop:",
        error,
      );
      WebviewBridge.hideLoading(webview);
  ...
  private static async runAgent(
    ...
    rulesContext?: string,
    agentOptions?: {
      disableReadDedup?: boolean;
      isRepairAgent?: boolean;
      isAgentMode?: boolean;
    },
    ...
    const agentToolContext: ToolExecutionContext = {
      ...toolContext,
      conversationTurnId: agentTurnId,
    };
    const settingsMgr = options.extensionContext
      ? SettingsManager.getInstance(options.extensionContext)
      : null;
    const useStreaming = settingsMgr
      ? await settingsMgr.isStreamingEnabled()
      : false;
    const thinkingEnabled = settingsMgr
      ? await settingsMgr.isThinkingEnabled()
      : true;

    // Tool approval settings (same pending behavior as single agent)
    const isAutoToolEnabled = settingsMgr
      ? await settingsMgr.isAutoToolExecutionEnabled()
      : true;
    const isAutoCommandEnabled = settingsMgr
      ? await settingsMgr.isAutoExecuteCommandsEnabled()
      : true;
    const isAutoUpdateEnabled = settingsMgr
      ? await settingsMgr.isAutoUpdateEnabled()
      : true;
    const isAutoDeleteEnabled = settingsMgr
      ? await settingsMgr.isAutoDeleteFilesEnabled()
      : true;

    callbacks.onToolApprovalRequired = async (
      call: ToolUse,
    ): Promise<boolean> => {
      ...
      WebviewBridge.receiveMessage(
        webview,
        "System",
        `[Pending] ${toolLabel}${detailDisplay} - User approval required`,
      );
    ...
    const mergedAgentOptions = {
      ...agentOptions,
      isAgentMode:
        agentOptions?.isAgentMode ?? options.promptType === PromptType.AGENT,
    };
  ...
  private static async saveToHistory(
    ...
    try {
      ...
      console.log(
        `[OrchestrationRouter] Saved orchestration result to session history`,
      );
    } catch (e) {
      console.warn(
        "[OrchestrationRouter] Failed to save to session history:",
        e,
      );
    }
  }
  ...
  private static async runPostMergeValidation(
    ...
    for (let attempt = 0; attempt <= MAX_REPAIR_RETRIES; attempt++) {
      ...
      if (testResult.success) {
        ...
        console.log(
          `[OrchestrationRouter] Validation passed (attempt ${attempt})`,
        );
      ...
      // Validation failed
      console.log(
        `[OrchestrationRouter] Validation failed (attempt ${attempt}): ${testResult.errorMessage?.substring(0, 100).replace(/\n/g, " ")}`,
      );
      ...
      if (
        !ErrorClassifier.isRetryable(classification.dominantCategory) &&
        !isAgentMode
      ) {
        const strategy = ErrorClassifier.getResolutionStrategy(
          classification.dominantCategory,
        );
        WebviewBridge.sendProcessingStatus(
          webview,
          "review",
      ...
      for (const f of repairResult.createdFiles) {
        if (!allCreatedFiles.includes(f)) {
          allCreatedFiles.push(f);
        }
      }
      for (const f of repairResult.modifiedFiles) {
        if (!allModifiedFiles.includes(f)) {
          allModifiedFiles.push(f);
        }
      }
    }
   ...
   */
  private static async enrichWithPriorResults(
    subtask: SubTask,
    priorResults: AgentLoopResult[],
    workspaceRoot: string,
  ): Promise<SubTask> {
    const depResults = priorResults.filter((r) =>
    ...
    const allAlreadyDone = depResults.every(
      (r) => r.doneStatus === "already_done",
    );
    ...
    for (const r of depResults) {
      ...
      if (r.doneStatus === "already_done") {
        context.push(
          `**Status: already implemented (already_done)** -- This task was completed without additional work because the existing code already fully implements it.`,
        );
      }
      ...
      if (allFiles.length > 0) {
        ...
        for (const filePath of allFiles) {
          ...
          try {
            ...
            context.push(
              `- ${filePath} (read failed -- needs verification via read_file)`,
            );
          }
        }
      }
      ...
      if (r.deletedFiles.length > 0) {
        context.push(
          `**Deleted files (${r.deletedFiles.length}):** ${r.deletedFiles.join(", ")}`,
        );
      }
    }
    ...
    if (allAlreadyDone) {
      ...
      context.push(
        `**Briefly check the required files and if no additional work is needed, complete with __done__(already_done).**`,
      );
    } else {
      context.push(
      ...
      context.push(
        `**WARNING: The files listed above already exist on disk. If modifications are needed, you must use update_file. Do not overwrite with create_file.**`,
      );
    }
    ...
    try {
      ...
      if (userQuery) {
        const mentionedPaths =
          OrchestrationRouter.extractPathsFromQuery(userQuery);
        if (mentionedPaths.length > 0) {
          lines.push(
            `\nTarget paths mentioned by user: ${mentionedPaths.join(", ")}`,
          );
          lines.push(
            `WARNING: All file creation/modification must be performed relative to the paths above.`,
          );
        }
      }
   ...
   */
  private static async gatherRulesContext(
    userQuery: string,
    candidateSkillKeys?: string[],
  ): Promise<{ text: string; references: ReferenceItem[] }> {
    const parts: string[] = [];
    ...
    try {
      const { text: localRules, ruleKeys } =
        PromptComposer.loadAgentRulesWithKeys();
      const { text: serverRules } =
        PromptComposer.loadServerPromptTemplates(ruleKeys);
      if (localRules) {
        parts.push(localRules);
      }
      if (serverRules) {
        parts.push(serverRules);
      }
      ...
      if (candidateSkillKeys && candidateSkillKeys.length > 0) {
        for (const skillKey of candidateSkillKeys) {
          const skill = PromptComposer.getSkillDescriptions().find(
            (s) => s.key === skillKey,
          );
          if (skill) {
            const refType =
              skill.source === "server" ? "server_skill" : "local_skill";
            references.push({
              type: refType,
              name: skill.key,
              source: skill.source || "server",
            });
          }
        }
      }
      ...
      if (skillDescriptions.length > 0) {
        const candidateHint =
          candidateSkillKeys && candidateSkillKeys.length > 0
    ...
    try {
      const { getErrorCorrectionGuide } =
        await import("../managers/context/prompts/base");
      parts.push(getErrorCorrectionGuide());
    } catch (e) {
      console.warn(
        "[OrchestrationRouter] Failed to load error correction guide:",
        e,
      );
    }
    ...
    try {
      ...
      const mcpServers =
        MCPManager.getInstance().getServers() as McpServerInfo[];
    ...
    try {
      ...
      if (auth.isLoggedIn() && userQuery) {
        ...
        try {
          const { SettingsManager: RagSettingsMgr } =
            await import("../managers/state/SettingsManager");
          ragProjectId =
            RagSettingsMgr.getInstance()?.context?.globalState?.get<string>(
              "codepilot.projectId",
            ) || undefined;
        } catch {}
        console.log(
          `[OrchestrationRouter] RAG: 검색 시작 (query: "${userQuery.substring(0, 50)}...", orgId: ${orgId}, projectId: ${ragProjectId || "none"})`,
        );
        // RAG 소스별 similarity_threshold/top_k 반영 (admin에서 설정)
        const { SettingsManager: OrchRagSettingsMgr } =
          await import("../managers/state/SettingsManager");
        const { topK: ragTopK, thresholdBySourceId } =
          OrchRagSettingsMgr.getInstance().getRagSearchParams();
        const ragRaw = (await CodePilotApiClient.getInstance().searchRag(
          userQuery,
          orgId || undefined,
          undefined,
          ragTopK,
          ragProjectId || undefined,
        )) as RagResult[] | { data?: RagResult[]; results?: RagResult[] };
        const ragResults: RagResult[] = Array.isArray(ragRaw)
          ? ragRaw
          : (ragRaw as { data?: RagResult[]; results?: RagResult[] }).data ||
            (ragRaw as { data?: RagResult[]; results?: RagResult[] }).results ||
            [];
        // source_id별 threshold 적용 (없으면 0.8 fallback)
        const filteredRag = ragResults.filter((r) => {
          if (!r.similarity) return true;
          const threshold =
            thresholdBySourceId.get(String((r as any).source_id)) ?? 0.8;
          return r.similarity >= threshold;
        });
        if (filteredRag.length > 0) {
          console.log(
            `[OrchestrationRouter] RAG: ${filteredRag.length}/${ragResults.length}개 문서 청크 포함 (per-source threshold)`,
          );
          const ragText = filteredRag
            ...
            .join("\n\n---\n\n");
          parts.push(
            `## Reference Documents (RAG) -- Must be used with priority\n${ragText}`,
          );
        ...
        console.log(
          `[OrchestrationRouter] RAG: 스킵 (isLoggedIn=${auth.isLoggedIn()}, hasQuery=${!!userQuery})`,
        );
      }
    } catch (e) {
...
Based on the above, write a unified completion response to deliver to the user.`;
    ...
    const response = await llm.sendMessageWithSystemPrompt(systemPrompt, [
      { text: userMessage },
    ]);
  ...
  private static formatMergedResult(
    ...
    if (validation?.validated && !validation.testPassed) {
      ...
      if (validation.repairAttempts > 0) {
        lines.push(
          `> Unresolved after ${validation.repairAttempts} auto-repair attempt(s).`,
        );
      }
    }
    ...
    if (result.errors.length > 0) {
      ...
      result.errors.forEach((e) =>
        lines.push(`- ${e.replace(/\[task-(\d+)\]/g, "[agent $1]")}`),
      );
    }
