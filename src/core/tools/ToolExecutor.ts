...
export class ToolExecutor {
  ...
  async executeTool(
    ...
    // A2: PreToolUse 검증
    const validation = await PreToolUseValidator.validate(
      toolUse,
      context.projectRoot,
    );
    if (!validation.allowed) {
      console.warn(
        `[ToolExecutor] Tool blocked by PreToolUseValidator: ${toolUse.name} - ${validation.reason}`,
      );
      return {
        success: false,
        message: validation.reason || "Tool execution blocked",
        error: {
          code: "BLOCKED_BY_VALIDATOR",
          message: validation.reason || "Blocked",
        },
      };
    }
    ...
    if (validation.severity === "warning") {
      console.warn(
        `[ToolExecutor] Tool warning: ${toolUse.name} - ${validation.reason}`,
      );
    }
    ...
    if (!handler) {
      return {
        ...
        error: {
          code: "UNKNOWN_TOOL",
          message: `Tool ${toolUse.name} is not registered`,
        },
      };
    }
    ...
    try {
      ...
      // 도구 실행 메트릭 기록
      usageMetrics.recordToolExecution(
        toolUse.name,
        executionTime,
        result.success,
      );
      console.log(
        `[ToolExecutor] Tool ${toolUse.name} completed: ${result.success ? "success" : "failed"} (${executionTime}ms)`,
      );
      ...
      console.error(
        `[ToolExecutor] Tool execution failed: ${toolUse.name} (${executionTime}ms)`,
        error,
      );
  ...
  private async executeToolsSequential(
    ...
    for (let i = 0; i < toolUses.length; i++) {
      if (abortSignal?.aborted) {
        console.log(
          `[ToolExecutor] Aborted before tool ${i + 1}/${toolUses.length}`,
        );
        break;
      }
      ...
      // run_command 실패해도 배치 계속 — 모델이 tool_result로 판단. 중단은 abort만.
    }
  ...
  private async executeToolsParallel(
    ...
    console.log(
      `[ToolExecutor] Parallel mode: ${readBatch.length} read-only, ${writeBatch.length} write tools`,
    );

    // Phase 1: 읽기 도구 병렬 실행 (부분 실패 허용 — Promise.allSettled)
    if (readBatch.length > 0) {
      const settled = await Promise.allSettled(
        readBatch.map(async ({ toolUse, idx }) => {
    ...
    for (const { toolUse, idx } of writeBatch) {
      ...
      // run_command 실패해도 쓰기 배치 계속 — 모델이 tool_result로 판단. 중단은 abort만.
    }

    // undefined 슬롯을 에러 응답으로 채워 인덱스 매핑 보존 (abort 시 일부 미실행)
    return results.map(
      (r, i) =>
        r ?? {
          success: false,
          ...
          error: {
            code: "SKIPPED",
            message: "Skipped due to prior failure in write batch",
          },
        },
    );
  }
