...
export class ToolParser {
     ...
     * { "tool": "read_file", "path": "src/file.ts" }
     */
    static parseCodeBlockFormat(content, warnings) {
        const toolCalls = [];
        // JSON + CODE 블록 패턴 찾기
    ...
    static isValidToolName(name) {
        // 빌트인 도구 (fast path)
        if (Object.values(Tool).includes(name)) {
            return true;
        }
     ...
     */
    static parseToolCallsUnified(content, _nativeResponse, _provider, warnings) {
        // CODE 블록 형식 파싱
     ...
     * 도구 호출 파싱 (CODE 블록 형식)
     */
    static parseToolCalls(content, warnings) {
     ...
     */
    static parseInvestigationDone(content) {
        try {
