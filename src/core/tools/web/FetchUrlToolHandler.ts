...
export class FetchUrlToolHandler implements IToolHandler {
  ...
  async execute(
    ...
    try {
      ...
      // Untrusted content wrapping: 외부 웹 컨텐츠를 LLM이 "지시문"이 아닌 "참고 데이터"로 해석하도록 유도.
      // 공격자가 페이지에 심은 "이전 지시 무시" 같은 prompt injection 완화. Skills/Rules/RAG 등 trusted 경로는 래핑하지 않음.
      const safeUrl = String(url).replace(/"/g, "&quot;");
      const wrapped =
        `<untrusted_content source="fetch_url" url="${safeUrl}" length="${content.length}"${truncated ? ' truncated="true"' : ""}>\n` +
        result +
        `\n</untrusted_content>`;
      ...
      const req = protocol.get(
        ...
        (res) => {
          ...
          if (
            ...
            FetchUrlToolHandler.fetchContent(res.headers.location)
              .then(resolve)
              .catch(reject);
            return;
          }
  ...
  public static extractTextFromHtml(html: string): string {
    ...
    $(
      'script, style, nav, header, footer, aside, iframe, noscript, svg, form, button, [role="navigation"], [role="banner"], [role="contentinfo"]',
    ).remove();
