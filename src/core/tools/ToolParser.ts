...
export class ToolParser {
  ...
  private static extractFileContent(
    ...
    if (codeStartIdx === -1) {
      ...
      if (toolName === Tool.CREATE_FILE || toolName === Tool.UPDATE_FILE) {
        ...
        if (
          ...
          warnings?.push(
            `[ToolParser] Fallback: content extracted via SEARCH/REPLACE markers, length=${trimmed.length}`,
          );
          return trimmed;
        }
      }
    ...
    warnings?.push(
      `[ToolParser] Fallback: missing </file_content> closing tag, length=${extracted.length}`,
    );
    return extracted;
  }
  ...
  static parseCodeBlockFormat(content: string, warnings?: string[]): ToolUse[] {
    ...
    for (const candidate of candidates) {
      ...
      try {
        ...
        if (
          candidate.json.includes('"tool"') ||
          candidate.json.includes("'tool'")
        ) {
          warnings?.push(
            `JSON parse failed: ${candidate.json.substring(0, 80)}...`,
          );
        }
        continue;
      }
    ...
    for (let i = 0; i < toolCandidates.length; i++) {
      ...
      if (parsed.path !== undefined) {
        ...
        if (pathList.length > 0) {
          ...
          for (let j = 1; j < pathList.length; j++) {
            toolCalls.push({
              name: Tool.READ_FILE,
              params: { path: pathList[j] },
              partial: false,
            });
          }
          console.log(
            `[ToolParser] read_file paths expanded: ${pathList.length} files`,
          );
        }
      }
      ...
      if (!validation.valid) {
        warnings?.push(
          validation.message || `${toolName}: required parameter missing`,
        );
    ...
    if (deduped.length < toolCalls.length) {
      console.log(
        `[ToolParser] Deduplicated: ${toolCalls.length} → ${deduped.length} tool calls`,
      );
    }
    ...
    const capped =
      deduped.length > MAX_TOOL_CALLS_PER_RESPONSE
        ? deduped.slice(0, MAX_TOOL_CALLS_PER_RESPONSE)
        : deduped;
    if (capped.length < deduped.length) {
      console.warn(
        `[ToolParser] Tool call cap applied: ${deduped.length} → ${capped.length} (max ${MAX_TOOL_CALLS_PER_RESPONSE})`,
      );
    }

    console.log(
      `[ToolParser] parseCodeBlockFormat result: ${capped.length} tool calls found`,
      capped.map((c) => c.name),
    );
    return capped;
  }
   ...
   */
  private static validateToolParams(
    toolName: string,
    ...
    if (toolName === Tool.CREATE_FILE) {
      ...
      if (!params.content && params.content !== "") {
        ...
        return {
          valid: false,
          message: `create_file is missing content (path=${params.path})`,
        };
      }
    }
  ...
  static parseToolCallsWithResult(content: string): ToolParseResult {
    ...
    const tools = this.parseToolCallsUnified(
      content,
      undefined,
      undefined,
      warnings,
   ...
   */
  static parsePlanItems(content: string): Array<{
    title: string;
    detail?: string;
    ...
    try {
      ...
      while ((match = jsonBlockPattern.exec(content)) !== null) {
        try {
          parseItems(JSON.parse(match[1].trim()));
        } catch {
          /* 무시 */
        }
      }
      ...
      if (items.length === 0) {
        ...
        if (directJsonStr) {
          try {
            parseItems(JSON.parse(directJsonStr));
          } catch {
            /* 무시 */
          }
        }
