...
export class UpdateFileToolHandler implements IToolHandler {
    ...
    async execute(toolUse: ToolUse, context: ToolExecutionContext): Promise<ToolResponse> {
        ...
        const replacementsToApply: Array<{ start: number; end: number; replace: string }> = [];
        ...
        for (const replacement of replacements) {
            ...
            if (matchResult) {
                ...
                console.log(`[UpdateFileToolHandler] Failed SEARCH block (lines: ${replacement.search.split('\n').length}):\n[START]\n${replacement.search}\n[END]`);

                let errorMessage = `파일(${filePath})에서 SEARCH 블록의 내용을 찾을 수 없습니다.\n\n`;
                errorMessage += `**실패 원인:**\n`;
                errorMessage += `1. SEARCH 블록의 내용이 실제 파일 내용과 정확히 일치하지 않습니다.\n`;
                errorMessage += `2. 파일 내용이 LLM이 기억하는 상태와 달라졌을 수 있습니다.\n\n`;

                errorMessage += `**해결 방법:**\n`;
                errorMessage += `1. **강력 권장**: \`update_file\`이 계속 실패하면 즉시 **\`create_file\`로 파일 전체를 덮어쓰세요.**\n`;
                errorMessage += `2. **고유 패턴 사용**: SEARCH 블록을 더 작고 고유한(unique) 내용으로 지정하세요.\n\n`;
        ...
        for (let i = 0; i < replacementsToApply.length - 1; i++) {
            if (replacementsToApply[i].end > replacementsToApply[i + 1].start) {
                return {
                    ...
                    error: { code: 'OVERLAPPING_REPLACEMENTS', message: 'Replacements overlap.' }
                };
            }
        }
    ...
    private structuralFallbackMatch(
        ...
        const normalizedSearch = searchContent.replace(normalizePattern, ' ').trim();
