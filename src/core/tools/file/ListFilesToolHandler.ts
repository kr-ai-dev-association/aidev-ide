...
export class ListFilesToolHandler implements IToolHandler {
  ...
  async execute(
    ...
    if (!path.isAbsolute(dirPath) && !fsSync.existsSync(absolutePath)) {
      const fallback = SubProjectDetector.resolveWithFallback(
        context.projectRoot,
        dirPath,
      );
      if (fallback && fsSync.statSync(fallback).isDirectory()) {
        console.log(
          `[ListFilesToolHandler] SubProject fallback: ${dirPath} → ${path.relative(context.projectRoot, fallback)}`,
        );
    ...
    if (
      !absolutePath.startsWith(context.projectRoot) &&
      absolutePath !== context.projectRoot
    ) {
      console.warn(
        `[ListFilesToolHandler] External path blocked: ${absolutePath}. Using project root instead.`,
      );
      absolutePath = context.projectRoot;
    }
    ...
    try {
      ...
      const adminExcludePatterns = SettingsManager.getInstance()
        .getServerExcludePatterns()
        .map((e) => e.pattern)
  ...
  private async listFiles(
    ...
    const isTargetingIgnored =
      this.DEFAULT_IGNORE_DIRECTORIES.includes(targetDirName) ||
    ...
    async function traverse(currentPath: string) {
      try {
        ...
        for (const entry of entries) {
          ...
          // 4. Check admin exclude_patterns (server-managed glob list)
          if (!isTargetingIgnored) {
            if (
              self.DEFAULT_IGNORE_DIRECTORIES.includes(entryName) ||
            ...
            if (
              ...
              self.matchesGitignore(entryName, relativePath, gitignorePatterns)
            ) {
              continue;
            }
            if (
              adminExcludePatterns.length > 0 &&
              self.matchesGitignore(
                entryName,
                relativePath,
                adminExcludePatterns,
              )
        ...
        if (error.code === "EACCES") {
          console.warn(
            `[ListFilesToolHandler] Skipping protected directory: ${currentPath}`,
          );
          return;
        }
        throw error;
