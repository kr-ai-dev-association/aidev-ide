...
export class MCPToolHandler {
    ...
    toolInfo;
    mcpManager;
    constructor(serverId, serverName, toolInfo) {
     ...
     * 사용자 승인 요청
     */
    async requestApproval(webview) {
        const message = `MCP 도구 "${this.mcpToolName}" 실행을 허용하시겠습니까?\n\n설명: ${this.toolInfo.description || '(설명 없음)'}`;
        const result = await vscode.window.showWarningMessage(message, { modal: true }, '허용 (이후 자동 실행)', '이번만 허용', '거부');
...
//# sourceMappingURL=MCPToolHandler.js.map