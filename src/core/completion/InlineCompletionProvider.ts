...
export class InlineCompletionProvider implements vscode.InlineCompletionItemProvider {
    ...
    constructor(
        ...
        this.outputChannel = vscode.window.createOutputChannel('CodePilot Completion');
    }
    ...
    async provideInlineCompletionItems(
        ...
        this.outputChannel.appendLine(`[Completion] ${filename} (${language}) line ${position.line + 1}`);
        ...
        try {
            ...
            this.outputChannel.appendLine(`[Completion] OK: "${cleaned.slice(0, 60).replace(/\n/g, '↵')}${cleaned.length > 60 ? '...' : ''}"`);
    ...
    private buildPrefix(doc: vscode.TextDocument, pos: vscode.Position, maxLines: number): string {
        const startLine = Math.max(0, pos.line - maxLines);
        return doc.getText(new vscode.Range(new vscode.Position(startLine, 0), pos));
    }

    private buildSuffix(doc: vscode.TextDocument, pos: vscode.Position, maxLines: number): string {
