 ...
 */
export function openSettingsPanel(extensionUri, context, viewColumn, configurationService, notificationService, geminiApi, // GeminiApi 추가
licenseService, // LicenseService 추가
ollamaApi, // OllamaApi 추가
llmService, // LlmService 추가
terminalMonitorService, // TerminalMonitorService 추가
chatViewProvider) {
    const settingsManager = SettingsManager.getInstance(context);
    const panel = createAndSetupWebviewPanel(extensionUri, context, "settings", "CODEPILOT Settings", "settings", viewColumn, async (data, panel) => {
        ...
        switch (data.command) {
            case "getCurrentSettings": {
                try {
                    ...
                    const autoDeleteFilesEnabled = await settingsManager.isAutoDeleteFilesEnabled();
                    const autoExecuteCommandsEnabled = await settingsManager.isAutoExecuteCommandsEnabled();
                    const autoToolExecutionEnabled = await settingsManager.isAutoToolExecutionEnabled();
                    const streamingEnabled = await settingsManager.isStreamingEnabled();
                    // 채팅 테마 설정 로드
                    const config = vscode.workspace.getConfiguration('codepilot');
                    const chatTheme = config.get('chatTheme') || 'dark';
                    // 확장 버전 로드 (context에서)
            ...
            case "getRoutingOllamaModels": {
                ...
                try {
                    ...
                    console.error("[PanelManager] Failed to get routing Ollama models:", e?.message || String(e));
                    safePostMessage(panel, {
                        command: "routingOllamaModels",
                        models: [],
                        error: e?.message || String(e),
            ...
            case "saveCompactorModel": // Compactor 모델 저장
                try {
                    ...
                    if (compactorType) {
                        ...
                        const typeLabel = { ollama: "Ollama", gemini: "Google Gemini", banya: "Banya" }[compactorType] || compactorType;
                        const modelInfo = compactorModelName ? ` (${compactorModelName})` : "";
                        notificationService.showInfoMessage(`CODEPILOT: Compactor 모델이 ${typeLabel}${modelInfo}로 설정되었습니다.`);
                    }
                    else {
                        safePostMessage(panel, {
                    ...
                    notificationService.showErrorMessage(`Compactor 모델 저장 오류: ${error.message}`);
                }
                break;
            case "saveCompactorApiKey": // Compactor API 키 저장
                try {
                    ...
                    if (compactorApiKey) {
                        ...
                        const typeLabel = { gemini: "Gemini", banya: "Banya" }[compactorApiType] || "";
                        notificationService.showInfoMessage(`CODEPILOT: Compactor ${typeLabel} API 키가 저장되었습니다.`);
                    }
                    else {
                        safePostMessage(panel, {
                    ...
                    notificationService.showErrorMessage(`Compactor API 키 저장 오류: ${error.message}`);
                }
                break;
            case "clearCompactorModel": // Compactor 모델 초기화
                try {
                    ...
                    notificationService.showInfoMessage("CODEPILOT: Compactor 모델이 초기화되었습니다. 메인 모델이 사용됩니다.");
                }
                catch (error) {
                    safePostMessage(panel, {
            ...
            case "saveCommandModel": // Command 모델 저장
                try {
                    ...
                    if (commandType) {
                        ...
                        const typeLabel = { ollama: "Ollama", gemini: "Google Gemini", banya: "Banya" }[commandType] || commandType;
                        const modelInfo = commandModelName ? ` (${commandModelName})` : "";
                        notificationService.showInfoMessage(`CODEPILOT: Command 모델이 ${typeLabel}${modelInfo}로 설정되었습니다.`);
                    }
                    else {
                        safePostMessage(panel, {
                    ...
                    notificationService.showErrorMessage(`Command 모델 저장 오류: ${error.message}`);
                }
                break;
            case "saveCommandApiKey": // Command API 키 저장
                try {
                    ...
                    if (commandApiKey) {
                        ...
                        const typeLabel = { gemini: "Gemini", banya: "Banya" }[commandApiType] || "";
                        notificationService.showInfoMessage(`CODEPILOT: Command ${typeLabel} API 키가 저장되었습니다.`);
                    }
                    else {
                        safePostMessage(panel, {
                    ...
                    notificationService.showErrorMessage(`Command API 키 저장 오류: ${error.message}`);
                }
                break;
            case "clearCommandModel": // Command 모델 초기화
                try {
                    ...
                    notificationService.showInfoMessage("CODEPILOT: Command 모델이 초기화되었습니다. 메인 모델이 사용됩니다.");
                }
                catch (error) {
                    safePostMessage(panel, {
            ...
            case "saveIntentModel": // Intent 모델 저장
                try {
                    ...
                    if (intentType) {
                        ...
                        const typeLabel = { ollama: "Ollama", gemini: "Google Gemini", banya: "Banya" }[intentType] || intentType;
                        const modelInfo = intentModelName ? ` (${intentModelName})` : "";
                        notificationService.showInfoMessage(`CODEPILOT: Intent 모델이 ${typeLabel}${modelInfo}로 설정되었습니다.`);
                    }
                    else {
                        safePostMessage(panel, {
                    ...
                    notificationService.showErrorMessage(`Intent 모델 저장 오류: ${error.message}`);
                }
                break;
            case "saveIntentApiKey": // Intent API 키 저장
                try {
                    ...
                    if (intentApiKey) {
                        ...
                        const typeLabel = { gemini: "Gemini", banya: "Banya" }[intentApiType] || "";
                        notificationService.showInfoMessage(`CODEPILOT: Intent ${typeLabel} API 키가 저장되었습니다.`);
                    }
                    else {
                        safePostMessage(panel, {
                    ...
                    notificationService.showErrorMessage(`Intent API 키 저장 오류: ${error.message}`);
                }
                break;
            case "clearIntentModel": // Intent 모델 초기화
                try {
                    ...
                    notificationService.showInfoMessage("CODEPILOT: Intent 모델이 초기화되었습니다. 메인 모델이 사용됩니다.");
                }
                catch (error) {
                    safePostMessage(panel, {
            ...
            case "saveBanyaApiKey": // Banya API 키 저장 케이스 추가
                ...
                if (banyaApiKeyToSave && typeof banyaApiKeyToSave === "string") {
                    try {
                        ...
                        notificationService.showInfoMessage("CODEPILOT: Banya API Key saved successfully.");
                    }
                    catch (error) {
                        safePostMessage(panel, {
                        ...
                        notificationService.showErrorMessage(`Error saving Banya API Key: ${error.message}`);
                    }
                }
                else {
                    safePostMessage(panel, {
            ...
            case "saveBanyaModel": // Banya 모델 저장 케이스 추가
                ...
                if (banyaModelToSave && typeof banyaModelToSave === "string") {
                    try {
                        ...
                        notificationService.showInfoMessage(`CODEPILOT: Banya Model saved as ${banyaModelToSave}.`);
                    }
                    catch (error) {
                        safePostMessage(panel, {
                        ...
                        notificationService.showErrorMessage(`Error saving Banya Model: ${error.message}`);
                    }
                }
                else {
                    safePostMessage(panel, {
            ...
            case "setAutoUpdateEnabled": // 자동 파일 업데이트 설정 저장 케이스 추가 (toggles.js에서 호출)
                ...
                if (typeof autoUpdateEnabledToSet === "boolean") {
                    try {
                        ...
                        console.log(`[PanelManager] Auto Update 설정 저장됨: ${autoUpdateEnabledToSet}`);
                    }
                    catch (error) {
                        safePostMessage(panel, {
                        ...
                        notificationService.showErrorMessage(`Error setting Auto Update: ${error.message}`);
                    }
                }
                else {
                    safePostMessage(panel, {
            ...
            case "setAutoDeleteFilesEnabled": // 자동 파일 삭제 설정 저장 케이스 추가
                ...
                if (typeof autoDeleteFilesEnabledToSet === "boolean") {
                    try {
                        ...
                        console.log(`[PanelManager] Auto Delete Files 설정 저장됨: ${autoDeleteFilesEnabledToSet}`);
                    }
                    catch (error) {
                        safePostMessage(panel, {
                        ...
                        notificationService.showErrorMessage(`Error setting Auto Delete Files: ${error.message}`);
                    }
                }
                else {
                    safePostMessage(panel, {
                    ...
                    notificationService.showErrorMessage("Invalid Auto Delete Files setting provided.");
                }
                break;
            ...
            case "setAutoToolExecutionEnabled": // 도구 자동 실행 설정 저장 케이스 추가
                ...
                if (typeof autoToolExecutionEnabledToSet === "boolean") {
                    try {
                        await settingsManager.updateAutoToolExecutionEnabled(autoToolExecutionEnabledToSet);
                        safePostMessage(panel, {
                            command: "autoToolExecutionEnabledSet",
                        });
                        console.log(`[PanelManager] Auto Tool Execution 설정 저장됨: ${autoToolExecutionEnabledToSet}`);
                    }
                    catch (error) {
                        safePostMessage(panel, {
                        ...
                        notificationService.showErrorMessage(`Error setting Auto Tool Execution: ${error.message}`);
                    }
                }
                else {
                    safePostMessage(panel, {
                    ...
                    notificationService.showErrorMessage("Invalid Auto Tool Execution setting provided.");
                }
                break;
            case "setStreamingEnabled": // 스트리밍 설정 저장 케이스 추가
                ...
                if (typeof streamingEnabledToSet === "boolean") {
                    try {
                        await settingsManager.updateStreamingEnabled(streamingEnabledToSet);
                        safePostMessage(panel, {
                            command: "streamingEnabledSet",
                        });
                        console.log(`[PanelManager] Streaming 설정 저장됨: ${streamingEnabledToSet}`);
                        // 알림 제거 (v9.4.1) - 사용자 요청
                    }
                    catch (error) {
                        safePostMessage(panel, {
                        ...
                        notificationService.showErrorMessage(`Error setting Streaming: ${error.message}`);
                    }
                }
                else {
                    safePostMessage(panel, {
                    ...
                    notificationService.showErrorMessage("Invalid Streaming setting provided.");
                }
                break;
                    ...
                    try {
                        ...
                        if (chatViewProvider && typeof chatViewProvider.postMessageToWebview === 'function') {
                            ...
                            if (aiModelToSave === 'gemini') {
                                ...
                                const config = vscode.workspace.getConfiguration('codepilot');
                                modelName = config.get('banyaModel') || 'Banya-Solar:100b';
                            }
            ...
            case "addAgentPolicyFile": // 카테고리에 파일 추가
                try {
                    ...
                    notificationService.showInfoMessage(`CODEPILOT: ${safeFileName} saved to .agent/rules/${category}/`);
                }
                catch (error) {
                    safePostMessage(panel, {
                    ...
                    notificationService.showErrorMessage(`Error adding Agent Policy file: ${error.message}`);
                }
                break;
            case "deleteAgentPolicyFile": // 카테고리에서 특정 파일 삭제
                try {
                    ...
                    notificationService.showInfoMessage(`CODEPILOT: ${targetFileName} deleted`);
                }
                catch (error) {
                    ...
                    notificationService.showErrorMessage(`Error deleting Agent Policy file: ${error.message}`);
                }
                break;
            ...
            case "getChatTheme": // 채팅 테마 로드
                try {
                    const config = vscode.workspace.getConfiguration('codepilot');
                    const theme = config.get('chatTheme') || 'dark';
                    safePostMessage(panel, {
                        command: 'chatTheme',
                        theme: theme
                    });
                ...
                try {
                    ...
                    if (enabled) {
                        ...
                        if (!existingServers.find(s => s.id === toggleServerId)) {
                            ...
                            const serverConfig = allServers.find((s) => s.id === toggleServerId);
                            if (serverConfig) {
            ...
            case "addHotLoad":
                try {
                    ...
                    const addCondition = data.conditionType && data.conditionType !== 'none'
                        ...
                        : undefined;
                    const id = await hotLoadManager.addHotLoad(data.keywords, data.description, data.commandStr, addCondition, data.maxRetries ? parseInt(data.maxRetries, 10) : undefined, data.onFailure || undefined);
                    safePostMessage(panel, {
                        command: "hotLoadAdded",
                        id: id,
                    });
                    notificationService.showInfoMessage("CODEPILOT: Hot Load 항목이 추가되었습니다.");
                }
                catch (error) {
            ...
            case "updateHotLoad":
                try {
                    ...
                    const updateCondition = data.conditionType && data.conditionType !== 'none'
                        ? { type: data.conditionType, value: data.conditionValue || '' }
                        : undefined;
                    await hotLoadManager.updateHotLoad(data.id, data.keywords, data.description, data.commandStr, updateCondition, data.maxRetries ? parseInt(data.maxRetries, 10) : undefined, data.onFailure || undefined);
                    safePostMessage(panel, { command: "hotLoadUpdated" });
                    notificationService.showInfoMessage("CODEPILOT: Hot Load 항목이 수정되었습니다.");
                }
                catch (error) {
            ...
            case "deleteHotLoad":
                try {
                    ...
                    notificationService.showInfoMessage("CODEPILOT: Hot Load 항목이 삭제되었습니다.");
                }
                catch (error) {
            ...
            case "getContextExclusions":
                try {
                    const customExclusions = context.globalState.get('contextExclusionPatterns', []);
                    const disabledExclusions = context.globalState.get('contextExclusionDisabled', []);
            ...
            case "addContextExclusion":
                try {
                    ...
                    const currentPatterns = context.globalState.get('contextExclusionPatterns', []);
            ...
            case "deleteContextExclusion":
                try {
                    ...
                    const existingPatterns = context.globalState.get('contextExclusionPatterns', []);
            ...
            case "disableDefaultExclusion":
                try {
                    ...
                    const currentDisabled = context.globalState.get('contextExclusionDisabled', []);
            ...
            case "enableDefaultExclusion":
                try {
                    ...
                    const disabledList = context.globalState.get('contextExclusionDisabled', []);
            ...
            case "getSecurityRules":
                try {
                    const { DEFAULT_BLOCKED_COMMANDS, DEFAULT_PROTECTED_FILES, updateCustomBlockedCommands, updateCustomProtectedFiles, updateDisabledBlockedCommands, updateDisabledProtectedFiles, } = await import('../tools/PreToolUseValidator');
                    const customCommands = context.globalState.get('securityBlockedCommands', []);
                    const customFiles = context.globalState.get('securityProtectedFiles', []);
                    const disabledCommands = context.globalState.get('securityDisabledBlockedCommands', []);
                    const disabledFiles = context.globalState.get('securityDisabledProtectedFiles', []);
                    // 캐시 업데이트
            ...
            case "addBlockedCommand":
                try {
                    ...
                    const currentCmds = context.globalState.get('securityBlockedCommands', []);
            ...
            case "deleteBlockedCommand":
                try {
                    ...
                    const existingCmds = context.globalState.get('securityBlockedCommands', []);
            ...
            case "addProtectedFile":
                try {
                    ...
                    const currentFiles = context.globalState.get('securityProtectedFiles', []);
            ...
            case "deleteProtectedFile":
                try {
                    ...
                    const existingFiles = context.globalState.get('securityProtectedFiles', []);
            ...
            case "disableBlockedCommand":
                try {
                    ...
                    const currentDisabledCmds = context.globalState.get('securityDisabledBlockedCommands', []);
            ...
            case "enableBlockedCommand":
                try {
                    ...
                    const disabledCmdList = context.globalState.get('securityDisabledBlockedCommands', []);
            ...
            case "disableProtectedFile":
                try {
                    ...
                    const currentDisabledFiles = context.globalState.get('securityDisabledProtectedFiles', []);
            ...
            case "enableProtectedFile":
                try {
                    ...
                    const disabledFileList = context.globalState.get('securityDisabledProtectedFiles', []);
 ...
 * Plan Queue 패널을 엽니다.
 */
export function openPlanPanel(extensionUri, context) {
    // 🆕 core TaskManager 사용
    const taskManager = TaskManager.getInstance(context);
    const panel = createAndSetupWebviewPanel(extensionUri, context, "plan", "CODEPILOT Plan Queue", "plan", vscode.ViewColumn.Two, async (data, panel) => {
        switch (data.command) {
            case "planQueueLoad": {
                try {
