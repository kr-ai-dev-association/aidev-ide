...
const GLOBAL_STATE_KEY = "agentgocoder.userModels";
const SECRET_KEY_PREFIX = "agentgocoder.userModelApiKey.";
      ...
      switch (data.command) {
        ...
        case "updateUserModel": {
          ...
          notificationService.showInfoMessage(
            `AgentGoCoder: 사용자 모델 저장됨 - ${incoming.name}`,
          );
          return true;
        }
        ...
        case "selectUserModel": {
          ...
          notificationService.showInfoMessage(
            `AgentGoCoder: 활성 모델 - ${config.model}`,
          );
          return true;
        }
