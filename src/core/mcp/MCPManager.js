...
export class MCPManager {
    ...
    async migrateLegacySettings() {
        if (!this.context)
            return;
        const legacySettings = this.context.globalState.get('mcp.settings');
    ...
    async loadSettings() {
        ...
        const approvedTools = await stateManager.getMcpApprovedTools();
        this.settings = {
            servers,
            approvedTools,
            enabled: true
    ...
    async removeServer(id) {
        ...
        await this.saveSettings();
        // 관련 승인 도구도 삭제
        this.settings.approvedTools = this.settings.approvedTools.filter(t => t.serverId !== id);
        await this.saveSettings();
     ...
     * 도구가 승인되었는지 확인
     */
    isToolApproved(serverId, toolName) {
        return this.settings.approvedTools.some(t => t.serverId === serverId && t.toolName === toolName);
    }
    /**
     * 도구 승인
     */
    async approveTool(serverId, toolName) {
        if (this.isToolApproved(serverId, toolName))
            return;
     ...
     * 도구 승인 취소
     */
    async revokeTool(serverId, toolName) {
        this.settings.approvedTools = this.settings.approvedTools.filter(t => !(t.serverId === serverId && t.toolName === toolName));
        await this.saveSettings();
    }
    /**
     * 모든 승인된 도구 목록
