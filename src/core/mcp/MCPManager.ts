...
export class MCPManager {
  ...
  async initialize(context: vscode.ExtensionContext): Promise<void> {
    ...
    console.log(
      `[MCPManager] Initialized with ${this.settings.servers.length} servers`,
    );
  ...
  private async autoConnectEnabledServers(): Promise<void> {
    ...
    console.log(
      `[MCPManager] autoConnect: ${allServers.length} servers (${allServers.map((s) => `${s.id}:enabled=${s.enabled}`).join(", ")})`,
    );
    for (const server of allServers) {
      if (server.enabled) {
        this.connectToServer(server.id).catch(async (err) => {
          ...
          console.error(
            `[MCPManager] Auto-connect failed for ${server.name}:`,
            errorMessage,
          );
          ...
          import("../../services/error/ErrorReportingService")
            .then(({ ErrorReportingService }) => {
              ErrorReportingService.getInstance().reportMCPError(
                server.id,
                server.name,
  ...
  private async migrateLegacySettings(): Promise<void> {
    ...
    const legacySettings =
      this.context.globalState.get<MCPSettings>("mcp.settings");
    if (legacySettings && legacySettings.servers.length > 0) {
      const stateManager = this.getStateManager();
      const existingServers = await stateManager.getMcpServers();
      if (existingServers.length === 0) {
        console.log(
          "[MCPManager] Migrating from globalState to StateManager...",
        );
        await stateManager.saveMcpServers(legacySettings.servers);
        await stateManager.saveMcpApprovedTools(
          legacySettings.approvedTools || [],
        );
        // 레거시 데이터 정리
  ...
  private async loadSettings(): Promise<void> {
    ...
    const approvedTools =
      (await stateManager.getMcpApprovedTools()) as ApprovedMCPTool[];
  ...
  /**
   * 서버에서 받은 admin MCP 설정을 강제로 다시 로드.
   * SettingsManager.syncServerSettings() 후 호출되어 adminServers 배열을
   * 최신 캐시로 갱신함. (이전엔 init 1회만 로드되어 admin에서 추가/수정한
   * MCP 가 IDE 갱신 버튼으로도 안 보이던 회귀를 수정)
   */
  public async reloadAdminServers(): Promise<void> {
    await this.mergeServerMCPConfigs();
  }
    ...
    try {
      ...
      for (const serverConfig of serverConfigs) {
        ...
        const isDisabled =
          ...
          settingsManager.isSettingDisabled("mcp_server", serverConfig.key);
        console.log(
          `[MCPManager] Admin server "${serverConfig.key}": enforcement=${enforcement}, isDisabled=${isDisabled}, enabled=${enforcement === "required" ? true : !isDisabled}`,
        );

        // 개인 목록에서 동일 ID 제거 (이전 병합 마이그레이션)
        const personalIdx = this.settings.servers.findIndex(
          (s) => s.id === serverId,
        );
        if (personalIdx !== -1) {
      ...
      console.log(
        `[MCPManager] Loaded ${this.adminServers.length} admin MCP configs`,
      );
    } catch (error) {
      console.warn(
        "[MCPManager] Failed to load admin MCP configs (falling back to local-only):",
        error,
      );
    }
  }
  ...
  private findServer(
    serverId: string,
  ): MCPServerConfig | AdminMCPServer | undefined {
    return (
      this.settings.servers.find((s) => s.id === serverId) ||
   ...
   */
  async addServer(
    config: MCPServerConfig | Omit<MCPServerConfig, "id">,
  ...
  async toggleAdminServer(
    ...
    try {
      ...
      const key = serverId.startsWith("server_")
        ? serverId.substring(7)
        : serverId;
      console.log(
        `[MCPManager] toggleAdminServer: serverId="${serverId}", key="${key}", disabled=${!enabled}`,
      );
      ...
      console.log(
        `[MCPManager] toggleAdminServer: disabled state saved successfully`,
      );
    } catch (err) {
      console.error(
        "[MCPManager] Failed to save admin server disabled state:",
        err,
      );
    }
  ...
  async callTool(
    ...
    if (!client || client.status !== "connected") {
      console.log(
        `[MCPManager] Client not connected for ${serverId}, attempting reconnect...`,
      );
      try {
        await this.connectToServer(serverId);
        ...
        console.error(
          `[MCPManager] Reconnect failed for ${serverId}: ${errorMsg}`,
        );
        return {
          success: false,
          content: [],
