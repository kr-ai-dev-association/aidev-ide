import { IFrameworkAdapter } from './IFrameworkAdapter';
...
export class FrameworkAdapterFactory {
    private static adapterCache = new Map<string, IFrameworkAdapter>();

    /**
     * 프로젝트 경로에서 기술 스택을 감지하고 어댑터 반환
     */
    static async detectAndCreate(projectPath: string): Promise<IFrameworkAdapter | null> {
        // 캐시 확인
        ...
        // 기술 스택 감지
        const adapter = await this.detectFramework(projectPath);
     ...
     * 특정 기술 스택 어댑터 생성
     */
    static create(stackId: string, options?: any): IFrameworkAdapter | null {
        switch (stackId) {
            case 'typescript':
            ...
            default:
                console.warn(`[FrameworkAdapterFactory] Unknown stack ID: ${stackId}`);
                return null;
        }
    }
     ...
     * 프로젝트 타입 감지
     */
    private static async detectFramework(projectPath: string): Promise<IFrameworkAdapter | null> {
        // 우선순위대로 감지
        const detectors = [
        ...
        for (const detector of detectors) {
            try {
                ...
                if (isMatch) {
                    console.log(`[FrameworkAdapterFactory] Detected tech stack for ${projectPath}`);
                    return await detector.create();
                }
            } catch (error) {
                console.error(`[FrameworkAdapterFactory] Error detecting tech stack:`, error);
            }
        }

        console.warn(`[FrameworkAdapterFactory] Could not detect tech stack for ${projectPath}`);
        return null;
    }
