...
export class AskViewProvider implements vscode.WebviewViewProvider {
  public static readonly viewType = "agentgocoder.askView"; // 새로운 뷰 타입
  ...
  public resolveWebviewView(
    ...
    webviewView.webview.onDidReceiveMessage(async (data: any) => {
      switch (data.command) {
        ...
        case "clearHistory":
          console.log(
            "[Extension Host] Clearing conversation history for Ask tab",
          );
          try {
  ...
  private async executeBashCommands(commands: string[]): Promise<void> {
    try {
      console.log(
        "[AskViewProvider] executeBashCommands called with:",
        commands,
      );
      ...
      if (terminalCwd) {
        console.log(
          "[AskViewProvider] Using workspace root for terminal:",
          terminalCwd,
        );
      } else {
      ...
      console.log(
        "[AskViewProvider] Creating new terminal with shell:",
        shellPath,
        "cwd:",
        terminalCwd,
      );
      ...
      console.log(
        `[AskViewProvider] Submitted script as single block (${commands.length} logical lines)`,
      );
    } catch (error) {
   ...
   */
  public processGeneralAskResponse(
    llmResponse: string,
    ...
    if (this.hasBashCommands(cleanedResponse)) {
      const warningMsg =
        "ASK 탭에서는 터미널 명령어를 실행할 수 없습니다. CODE 탭을 사용해주세요.";
    ...
    if (this.hasFileDirectives(cleanedResponse)) {
      const warningMsg =
        "ASK 탭에서는 파일 생성, 수정, 삭제를 할 수 없습니다. CODE 탭을 사용해주세요.";
      safePostMessage(webview, {
        command: "receiveMessage",
        sender: "AgentGoCoder",
        text: warningMsg,
  ...
  private removeBashCommands(response: string): string {
    ...
    return response.replace(
      /```(?:bash|sh|shell|powershell|pwsh|cmd|batch|bat)[\s\S]*?```/g,
      "",
    );
  }
  ...
  private hasBashCommands(response: string): boolean {
    return /```(?:bash|sh|shell|powershell|pwsh|cmd|batch|bat)[\s\S]*?```/i.test(
      response,
    );
  }
  ...
  private hasFileDirectives(response: string): boolean {
    return /(?:새 파일|수정 파일|삭제 파일|New file|Create file|Modified file|Update file|Modify file|Delete file|Remove file):/i.test(
      response,
    );
  }
