...
import { PromptType, LicenseService } from '../../services';
import { TerminalManager, ConversationService, TaskManager, ExecutionManager, StateManager, SessionManager } from '../../core';
...
export class ChatViewProvider {
    ...
    constructor(extensionUri, context, openSettingsPanel, configurationService, notificationService, gitRepositoryService, geminiApi, ollamaApi, banyaApi) {
...
[실행 시 주의할 점들]`;
                    ...
                    try {
                        ...
                        if (editor) {
                            ...
                            if (firstModifiedLine !== null && firstModifiedLine >= 0) {
                                ...
                                editor.revealRange(new vscode.Range(firstLine, firstLine), vscode.TextEditorRevealType.InCenter);
                            }
                        }
                    }
                    catch (error) {
                ...
                case 'openDiff':
                    try {
                        ...
                        if (!this.diffDocumentProvider) {
                            ...
                            this.context.subscriptions.push(vscode.workspace.registerTextDocumentContentProvider('codepilot-diff', this.diffDocumentProvider));
                        }
                        // Diff 에디터 열기
    ...
    async sendFileList(webview) {
        try {
            ...
            const excludeDirs = getAllExclusionPaths()
                .filter((excludedPath) => !excludedPath.includes('*') && !excludedPath.includes('/')) // 와일드카드와 경로 구분자 제외
                .join(',');
            const excludePattern = `**/{${excludeDirs}}/**`;
            // 파일 검색 (최대 500개로 제한)
            const files = await vscode.workspace.findFiles(new vscode.RelativePattern(searchRoot, '**/*'), excludePattern, 500);
    ...
    async sendTerminalList(webview) {
     ...
     */
    async sendActiveTerminalContext(webview) {
        try {
            ...
            if (!terminalContent) {
                ...
                if (shellHistory.length > 0) {
                    ...
                    const terminalHistory = history.getAll().filter((entry) => entry.sessionName === terminalName).slice(-10);
     ...
     */
    async sendDiagnosticsContext(webview) {
        try {
            // 모든 진단 정보 가져오기
     ...
     */
    formatDiagnosticsContext(items, errorCount, warningCount) {
        if (items.length === 0) {
     ...
     * - 토큰 사용량 및 컨텍스트 수 복원
     */
    restoreSessionOnStartup(webview) {
        ...
        setTimeout(async () => {
            try {
                ...
                const config = vscode.workspace.getConfiguration('codepilot');
                const maxTokens = config.get('maxInputTokens') || 128000;
                webview.postMessage({
                    command: 'updateContextInfo',
                    contextInfo: {
