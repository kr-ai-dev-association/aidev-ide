          ...
          try {
            ...
            try {
              ...
              adminModels = (aiModelSettings || [])
                .filter(
                  (s: any) =>
                    (s.source === "admin" || s.source === "project") &&
                    s.value &&
                    s.value.enabled !== false,
                )
                .map((s: any) => ({
...
[실행 시 주의할 점들]`;
        ...
        case "sendMessage":
          ...
          console.log(
            `[ChatViewProvider] sendMessage mode=${data.mode}, promptType=${promptType}`,
          );
        ...
        case "executeSlashCommand": {
          ...
          try {
            switch (action) {
              ...
              case "viewMcpServers":
                await vscode.commands.executeCommand(
                  "codepilot.viewMcpServers",
                );
                break;
              case "connectMcpServer":
                await vscode.commands.executeCommand(
                  "codepilot.connectMcpServer",
                );
                break;
