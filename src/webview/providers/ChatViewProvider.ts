...
export class ChatViewProvider implements vscode.WebviewViewProvider {
  ...
  public resolveWebviewView(
    ...
    try {
      ...
      try {
        webviewView.title = "Codepilot-Standalone";
      } catch {}
            ...
            if (adminSetting?.value) {
              ...
              const chatAdminUserApiKey = adminPresetGroup
                ? this.context.globalState.get<string>(
                    `codepilot.apiKey.${adminPresetGroup}`,
                  ) || ""
                : this.context.globalState.get<string>(
                    "codepilot-standalone.adminApiKey",
                  ) || "";
              const adminConfig = {
                key: adminKey,
                provider: v.provider || "",
            ...
            switch (action) {
              case "viewCacheStats":
                await vscode.commands.executeCommand(
                  "codepilot-standalone.viewCacheStats",
                );
                break;
              case "clearCache":
                await vscode.commands.executeCommand(
                  "codepilot-standalone.clearCache",
                );
                break;
              ...
              case "gitStatus":
                await vscode.commands.executeCommand(
                  "codepilot-standalone.gitStatus",
                );
                break;
              case "gitDiff":
                await vscode.commands.executeCommand(
                  "codepilot-standalone.gitDiff",
                );
                break;
              case "gitLog":
                await vscode.commands.executeCommand(
                  "codepilot-standalone.gitLog",
                );
                break;
              case "gitBranch":
                await vscode.commands.executeCommand(
                  "codepilot-standalone.gitBranch",
                );
                break;
              case "gitInfo":
                await vscode.commands.executeCommand(
                  "codepilot-standalone.gitInfo",
                );
                break;
              case "gitStaged":
                await vscode.commands.executeCommand(
                  "codepilot-standalone.gitStaged",
                );
                break;
              case "gitStash":
                await vscode.commands.executeCommand(
                  "codepilot-standalone.gitStash",
                );
                break;
              case "viewMcpServers":
                await vscode.commands.executeCommand(
                  "codepilot-standalone.viewMcpServers",
                );
                break;
              case "connectMcpServer":
                await vscode.commands.executeCommand(
                  "codepilot-standalone.connectMcpServer",
                );
                break;
        ...
        case "openDiff":
          try {
            ...
            const beforeUri = vscode.Uri.parse(
              `codepilot-diff:${absolutePath}.before`,
            );
    ...
    try {
      ...
      terminalName =
        osAdapter.osType === "win32"
          ? "CODEPILOT PowerShell Commands"
          : "CODEPILOT Shell Commands";
