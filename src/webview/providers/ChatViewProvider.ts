...
/**
 * Diff 가상 문서 프로바이더
 * agentgocoder-diff: 스킴으로 등록하여 before 내용을 제공
 */
class DiffDocumentProvider implements vscode.TextDocumentContentProvider {
  ...
  provideTextDocumentContent(uri: vscode.Uri): string {
    // URI에서 실제 파일 경로 추출 (agentgocoder-diff:/path/to/file.ts.before)
...
export class ChatViewProvider implements vscode.WebviewViewProvider {
  public static readonly viewType = "agentgocoder.chatView";
  ...
  public resolveWebviewView(
    ...
    try {
      ...
      try {
        webviewView.title = "AgentGoCoder";
      } catch {}
      ...
      const htmlContent = getHtmlContentWithUris(
        this.extensionUri,
        "chat",
        webviewView.webview,
      );
      ...
      (e) => {
        ...
        if (selection.isEmpty) {
          ...
          webviewView.webview.postMessage({
            command: "editorSelectionCleared",
          });
        } else {
          ...
          if (
            selectedText.trim().length <
              AgentConfig.EDITOR_SELECTION_MIN_LENGTH ||
            selectedText.length > AgentConfig.EDITOR_SELECTION_MAX_LENGTH
          )
            return;
    ...
    const pendingChangedDisposable =
      InlineDiffManager.getInstance().onPendingChanged(() => {
        ...
        webviewView.webview.postMessage({
          command: "updatePendingChangesByTurn",
          turns: turnStats,
        });
      });
    ...
    const configChangeDisposable = vscode.workspace.onDidChangeConfiguration(
      (e) => {
        if (e.affectsConfiguration("agentgocoder.chatTheme")) {
      ...
      switch (data.command) {
        ...
        case "getOllamaModels": {
          try {
            ...
            const rawModels =
              await ModelConnectionService.getOllamaModels(apiUrl);
            ...
            let currentModel = "";
            if (
              aiModelEngine?.startsWith("admin:") ||
              aiModelEngine?.startsWith("supported:")
            ) {
        ...
        case "setOllamaModel": {
          try {
            ...
            ModelConnectionService.getOllamaModelContextLength(
              modelName,
              this.ollamaApi?.getApiUrl?.(),
            )
              .then((ctxLen: number | null) => {
                if (ctxLen) {
                  const {
                    updateOllamaTokenLimits,
            ...
            if (adminSetting?.value) {
              ...
              const chatAdminUserApiKey = adminPresetGroup
                ? this.context.globalState.get<string>(
                    `agentgocoder.apiKey.${adminPresetGroup}`,
                  ) || ""
                : this.context.globalState.get<string>(
                    "agentgocoder.adminApiKey",
                  ) || "";
              const adminConfig = {
                key: adminKey,
                provider: v.provider || "",
          ...
          try {
            ...
            if (presetSetting?.value) {
              ...
              const chatUserApiKey = presetGroup
                ? this.context.globalState.get<string>(
                    `agentgocoder.apiKey.${presetGroup}`,
                  ) || ""
                : this.context.globalState.get<string>(
                    "agentgocoder.adminApiKey",
                  ) || "";
        ...
        case "analyzeErrors": {
          try {
            ...
            const { ErrorManager } =
              await import("../../core/managers/error/ErrorManager");
            const { ErrorSource } =
              await import("../../core/managers/error/types");
            const errorManager = ErrorManager.getInstance();
            const history = errorManager.getHistory();
            const recentErrors = history
              .getAll()
              .filter(
                (entry: any) => entry.error.source === ErrorSource.TERMINAL,
              )
...
[실행 시 주의할 점들]`;
        ...
        case "askQuestionResponse": {
          const { AskQuestionToolHandler } =
            await import("../../core/tools/interaction/AskQuestionToolHandler");
          AskQuestionToolHandler.resolveUserAnswer(
            data.requestId || "",
            data.answers || {},
          );
          break;
            ...
            switch (action) {
              case "viewCacheStats":
                await vscode.commands.executeCommand(
                  "agentgocoder.viewCacheStats",
                );
                break;
              case "clearCache":
                await vscode.commands.executeCommand("agentgocoder.clearCache");
                break;
              case "listSavedSessions":
                await vscode.commands.executeCommand(
                  "agentgocoder.listSavedSessions",
                );
                break;
              case "restoreSavedSession":
                await vscode.commands.executeCommand(
                  "agentgocoder.restoreSavedSession",
                );
                break;
              case "deleteSession":
                await vscode.commands.executeCommand(
                  "agentgocoder.deleteSession",
                );
                break;
              case "compactConversation":
                await vscode.commands.executeCommand(
                  "agentgocoder.compactConversation",
                );
                break;
              case "gitStatus":
                await vscode.commands.executeCommand("agentgocoder.gitStatus");
                break;
              case "gitDiff":
                await vscode.commands.executeCommand("agentgocoder.gitDiff");
                break;
              case "gitLog":
                await vscode.commands.executeCommand("agentgocoder.gitLog");
                break;
              case "gitBranch":
                await vscode.commands.executeCommand("agentgocoder.gitBranch");
                break;
              case "gitInfo":
                await vscode.commands.executeCommand("agentgocoder.gitInfo");
                break;
              case "gitStaged":
                await vscode.commands.executeCommand("agentgocoder.gitStaged");
                break;
              case "gitStash":
                await vscode.commands.executeCommand("agentgocoder.gitStash");
                break;
              case "viewMcpServers":
                await vscode.commands.executeCommand(
                  "agentgocoder.viewMcpServers",
                );
                break;
              case "connectMcpServer":
                await vscode.commands.executeCommand(
                  "agentgocoder.connectMcpServer",
                );
                break;
              case "disconnectMcpServer":
                await vscode.commands.executeCommand(
                  "agentgocoder.disconnectMcpServer",
                );
                break;
              default:
                console.warn(
                  `[ChatViewProvider] Unknown slash command: ${action}`,
                );
            }
          } catch (error) {
            console.error(
              `[ChatViewProvider] Slash command execution failed:`,
              error,
            );
          }
          break;
        ...
        case "approveAllChanges": {
          try {
            const diffModule =
              await import("../../core/managers/diff/InlineDiffManager");
            const inlineDiffManager =
              diffModule.InlineDiffManager.getInstance();
        ...
        case "acceptAllChangesForFile": {
          try {
            ...
            console.error(
              "[ChatViewProvider] acceptAllChangesForFile failed:",
              e,
            );
          }
          break;
        ...
        case "rejectAllChangesForFile": {
          try {
            ...
            console.error(
              "[ChatViewProvider] rejectAllChangesForFile failed:",
              e,
            );
          }
          break;
        ...
        case "projectTypeSelected": // 사용자가 프로젝트 타입을 선택한 경우
          try {
            ...
            this.notificationService.showInfoMessage(
              `프로젝트 타입이 선택되었습니다: ${data.projectType}`,
            );
        ...
        case "languageChanged":
          ...
          try {
            ...
            if (language && typeof language === "string") {
              // 언어 파일 경로
              const languageFilePath = vscode.Uri.joinPath(
                this.extensionUri,
                "webview",
                "locales",
                `lang_${language}.json`,
              ...
              // 파일 읽기
              const fileContent =
                await vscode.workspace.fs.readFile(languageFilePath);
            ...
            console.error(
              "Error loading language data in ChatViewProvider:",
              error,
            );
            // 오류 시 기본 한국어 데이터 반환
            try {
              const defaultLanguagePath = vscode.Uri.joinPath(
                this.extensionUri,
                "webview",
                "locales",
                "lang_ko.json",
              ...
              const defaultContent =
                await vscode.workspace.fs.readFile(defaultLanguagePath);
              const defaultData = JSON.parse(
                Buffer.from(defaultContent).toString("utf8"),
              );
              webviewView.webview.postMessage({
                command: "languageDataReceived",
                language: "ko",
              ...
              console.error(
                "Error loading fallback language data in ChatViewProvider:",
                fallbackError,
              );
            }
          }
        ...
        case "openFile":
          try {
            ...
            if (editor) {
              const diffManager = InlineDiffManager.getInstance();
              const firstModifiedLine =
                diffManager.getFirstModifiedLine(absolutePath);
            ...
            this.notificationService.showErrorMessage(
              `파일을 열 수 없습니다: ${error.message || error}`,
            );
          }
          break;
        case "openDiff":
          try {
            ...
            const diffManager = InlineDiffManager.getInstance();
            const beforeContent =
              diffManager.getCheckpointBeforeContent(absolutePath);

            if (!beforeContent) {
              this.notificationService.showErrorMessage(
                "Diff를 표시할 이전 버전이 없습니다.",
              );
              break;
            }
            ...
            const beforeUri = vscode.Uri.parse(
              `agentgocoder-diff:${absolutePath}.before`,
            );
            ...
            this.notificationService.showErrorMessage(
              `Diff를 열 수 없습니다: ${error.message || error}`,
            );
          }
          break;
        ...
        case "requestPendingChanges": {
          try {
            ...
            console.error(
              "[ChatViewProvider] Failed to get pending changes:",
              error,
            );
          }
          break;
        ...
        case "rejectPendingFile": {
          try {
            ...
            this.notificationService.showInfoMessage(
              `${path.basename(filePath)} 변경사항이 거부되었습니다.`,
            );
          } catch (error: any) {
            this.notificationService.showErrorMessage(
              `거부 실패: ${error.message || error}`,
            );
          }
          break;
        }
        ...
        case "requestTerminalList": {
          try {
            ...
            console.error(
              "[ChatViewProvider] Failed to get terminal list:",
              error,
            );
          }
          break;
        ...
        case "requestTerminalContext": {
          try {
            ...
            console.error(
              "[ChatViewProvider] Failed to get terminal context:",
              error,
            );
          }
          break;
        ...
        case "requestDiagnosticsContext": {
          try {
            ...
            console.error(
              "[ChatViewProvider] Failed to get diagnostics context:",
              error,
            );
          }
          break;
        ...
        case "saveChatTheme": {
          try {
            ...
            console.error(
              "[ChatViewProvider] Failed to save chat theme:",
              error,
            );
          }
          break;
        ...
        case "getChatTheme": {
          try {
            ...
            console.error(
              "[ChatViewProvider] Failed to get chat theme:",
              error,
            );
            webviewView.webview.postMessage({
              command: "chatTheme",
  ...
  private async sendActiveTerminalContext(webview: vscode.Webview) {
    try {
      ...
      console.log(
        `[ChatViewProvider] Reading active terminal content: "${terminalName}"`,
      );
      ...
      try {
        ...
        if (copiedContent && copiedContent.trim()) {
          console.log(
            `[ChatViewProvider] Got terminal content via clipboard: ${copiedContent.length} chars`,
          );
        ...
        console.warn(
          "[ChatViewProvider] Clipboard method failed:",
          clipboardError,
        );
      }
      ...
      if (!terminalContent) {
        ...
        const shellHistory =
          terminalManager.getShellIntegrationHistory(terminalName);

        if (shellHistory.length > 0) {
          console.log(
            `[ChatViewProvider] Using Shell Integration history: ${shellHistory.length} commands`,
          );
          ...
          const terminalHistory = history
            .getAll()
            .filter((entry: any) => entry.sessionName === terminalName)
            .slice(-10);

          if (terminalHistory.length > 0) {
            console.log(
              `[ChatViewProvider] Using TerminalHistory: ${terminalHistory.length} commands`,
            );
            ...
            terminalContent +=
              "Tip: 터미널에서 명령어를 실행한 후 다시 시도해주세요.\n";
          }
        }
      }
      ...
      console.log(
        `[ChatViewProvider] Terminal context ready: ${terminalContext.contextString.length} chars`,
      );
  ...
  private async sendDiagnosticsContext(webview: vscode.Webview) {
    try {
      ...
      // 컨텍스트 문자열 생성
      const contextString = this.formatDiagnosticsContext(
        diagnosticItems,
        errorCount,
        warningCount,
      );
  ...
  private async executeBashCommands(commands: string[]): Promise<void> {
    try {
      ...
      terminalName =
        osAdapter.osType === "win32"
          ? "AgentGoCoder PowerShell Commands"
          : "AgentGoCoder Shell Commands";
  ...
  public restoreConversationHistory(conversationHistory: any[]): void {
    if (!this._view) {
      console.warn(
        "[ChatViewProvider] Cannot restore conversation history: view not initialized",
      );
      return;
    }
    ...
    console.log(
      `[ChatViewProvider] Lazy loading: showing ${recentEntries.length}/${total} entries (from index ${startIdx})`,
    );
  ...
  private loadMoreHistory(): void {
    ...
    const olderEntries = this._fullConversationHistory.slice(
      startIdx,
      startIdx + loadCount,
    );
    ...
    console.log(
      `[ChatViewProvider] Loading ${loadCount} more entries (index ${startIdx}-${startIdx + loadCount - 1}), total loaded: ${this._historyLoadedCount}/${total}`,
    );
  ...
  private sendEntriesToWebview(entries: any[], isPrepend = false): void {
    entries.forEach((entry, index) => {
      ...
      if (entry.uiMessages && entry.uiMessages.length > 0) {
        entry.uiMessages.forEach(
          (msg: { sender: string; text: string; type?: string }) => {
            this._view?.webview.postMessage({
              command: isPrepend ? "prependMessage" : "receiveMessage",
              sender: msg.sender,
              text: msg.text,
              ...(entry.conversationTurnId
                ? { conversationTurnId: entry.conversationTurnId }
                : {}),
            });
          },
        );
   ...
   */
  private pushCurrentModelToWebview(
      ...
      try {
        ...
        try {
          ...
          supportedModels = (aiModelSettings || [])
            .filter(
              (s: any) =>
                s.source === "preset" && s.value && s.value.enabled !== false,
            )
            .map((s: any) => ({
              key: s.key,
              name: `supported:${s.key}`,
  ...
  private restoreSessionOnStartup(webview: vscode.Webview): void {
    ...
    setTimeout(async () => {
      try {
        ...
        console.log(
          `[ChatViewProvider] Restoring session: ${currentSession.id}`,
        );
        ...
        if (
          ...
          console.log(
            `[ChatViewProvider] Restored ${currentSession.conversationHistory.length} conversation entries`,
          );
        ...
        console.log(
          `[ChatViewProvider] Restored context info: ${stats.messageCount} messages, ${stats.totalTokensUsed} tokens`,
        );

        // Check for interrupted session — webview 팝업으로 표시
        const interrupted = sessionManager.wasLastSessionInterrupted();
        if (interrupted.interrupted && interrupted.lastUserQuery) {
          webview.postMessage({
            command: "showInterruptedSession",
            query: interrupted.lastUserQuery,
          });
        }
      } catch (error) {
