...
import { ActionManager, ExecutionManager, TerminalManager, TaskManager, ErrorManager, ContextManager, StateManager, SessionManager, SettingsManager, ProjectManager, AutoFix, LLMManager, } from "./core";
...
import { CreateFileToolHandler, UpdateFileToolHandler, RemoveFileToolHandler, ReadFileToolHandler, ListFilesToolHandler, SearchFilesToolHandler, RipgrepSearchToolHandler, ExpandAroundLineToolHandler, ListImportsToolHandler, StatFileToolHandler, } from "./core/tools/file";
...
export async function activate(context) {
    ...
    const initialBanyaApiUrl = config.get("banyaApiUrl") ||
    ...
    const userOS = require("os").platform() === "darwin"
        ? "macOS"
    ...
    const promptBuilder = new PromptBuilder(userOS, currentAiModel || defaultModelForPrompt);
    ...
    try {
        ...
        const autoFixLlmClient = async ({ error, context }) => {
            ...
            const raw = await errorManager.sendMessageForErrorCorrection(prompt, new LLMApiClient(geminiApi, ollamaApi, banyaApi, currentAiModel || defaultModelForError), undefined);
            if (!raw) {
    ...
    const llmApiClient = new LLMApiClient(geminiApi, ollamaApi, banyaApi, currentAiModel);
    const llmManager = LLMManager.getInstance(geminiApi, ollamaApi, banyaApi, currentAiModel);
    ...
    const chatViewProvider = new ChatViewProvider(context.extensionUri, context, (viewColumn) => openSettingsPanel(context.extensionUri, context, viewColumn, settingsManager, notificationService, geminiApi, licenseService, ollamaApi, undefined, undefined, chatViewProvider), settingsManager, notificationService, gitRepositoryService, geminiApi, ollamaApi, banyaApi);
    ...
    context.subscriptions.push(vscode.commands.registerCommand("codepilot.viewMcpServers", async () => {
        try {
            ...
            if (selected) {
                ...
                const serverTools = allTools.filter((t) => t.serverId === selected.serverId);
    ...
    // MCP 서버 연결 명령어
    context.subscriptions.push(vscode.commands.registerCommand("codepilot.connectMcpServer", async () => {
        try {
            const { MCPManager } = await import("./core/mcp/MCPManager");
            const mcpManager = MCPManager.getInstance();
            const servers = mcpManager.getServers();
            ...
            const items = disconnectedServers.map((server) => ({
                label: server.name,
                description: server.type === 'stdio' ? server.command : server.url,
                serverId: server.id,
            }));
    ...
    // MCP 서버 연결 해제 명령어
    context.subscriptions.push(vscode.commands.registerCommand("codepilot.disconnectMcpServer", async () => {
        try {
            const { MCPManager } = await import("./core/mcp/MCPManager");
            const mcpManager = MCPManager.getInstance();
            const servers = mcpManager.getServers();
            ...
            const items = connectedServers.map((server) => ({
                label: `🟢 ${server.name}`,
                description: server.type === 'stdio' ? server.command : server.url,
                serverId: server.id,
                serverName: server.name,
    ...
    context.subscriptions.push(vscode.commands.registerCommand("codepilot.restoreSavedSession", async () => {
        try {
            ...
            if (selected && selected.sessionId) {
                ...
                if (success) {
                    const session = sessionManager.getSession(selected.sessionId);
    ...
    context.subscriptions.push(vscode.commands.registerCommand("codepilot.compactConversation", async () => {
        try {
            ...
            const userParts = currentSession.conversationHistory.map((entry) => ({
            ...
            if (result.compacted && result.summary) {
                ...
                const savedPercent = ((result.savedTokens / result.originalTokens) *
                    100).toFixed(1);
                // 패널에 결과 메시지 표시
                chatViewProvider.postMessageToWebview({
                    command: "receiveMessage",
