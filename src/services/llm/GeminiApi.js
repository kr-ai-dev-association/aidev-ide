...
export class GeminiApi {
     ...
     */
    async callApi(endpoint, body, signal, retryCount = 0) {
        const MAX_RETRIES = 3;
     ...
     */
    async callApiStreaming(endpoint, body, onChunk, signal, retryCount = 0) {
        const MAX_RETRIES = 3;
        const url = `${API_BASE_URL}${endpoint}?key=${this.apiKey}`;
    ...
    handleApiError(error) {
        ...
        if (error.message) {
            ...
            const isOffline = lowerMsg.includes('err_internet_disconnected') ||
     ...
     * 스트리밍 응답을 위한 메서드
     */
    async sendMessageWithSystemPromptStreaming(systemInstructionText, userParts, onChunk, options) {
        if (!this.apiKey) {
            throw new Error("CODEPILOT API is not initialized. Please set your API Key in the CODEPILOT settings (License section).");
        }
        try {
            ...
            return await this.callApiStreaming(`/models/${this.modelName}:streamGenerateContent`, body, onChunk, options?.signal);
        }
        catch (error) {
