...
class ThinkingOnlyError extends Error {
  ...
  constructor(thinking: string) {
    super(
      `모델이 생각만 수행했습니다. 도구 호출 또는 텍스트 응답이 필요합니다. (Thinking length: ${thinking.length})`,
    );
...
export class OllamaApi {
   ...
   */
  public async sendFimCompletion(
    prefix: string,
    suffix: string,
    options?: SendOptions,
  ): Promise<string> {
  ...
  private static cleanFimResponse(raw: string, prefix: string): string | null {
    ...
    if (
      /^(It |This |Here |Note |The |You |To |In |A |An |I |We |Please |Sure |Certainly)/i.test(
        firstLine,
      )
    ) {
      console.log(
        `[OllamaApi] FIM: prose discarded → use base model (starcoder2:3b, deepseek-coder:1.3b, qwen2.5-coder:1.5b-base)`,
      );
      return null;
    }
    if (
    ...
    if (lines.length > 15 && /^import /.test(firstLine)) {
      console.log(
        `[OllamaApi] FIM: full-file regeneration discarded (${lines.length} lines starting with import)`,
      );
      return null;
    }
    // prefix 앞부분을 그대로 반복
    ...
    if (lines.length > 20) {
      console.log(
        `[OllamaApi] FIM: too long (${lines.length} lines), discarded`,
      );
      return null;
    }
  ...
  private static getFimTokens(
    ...
    if (
      ...
      n.includes("code-llama") ||
      n.includes("code_llama")
    ) {
      return {
        prefix: "<PRE> ",
        suffix: " <SUF>",
  ...
  private async sendMessageWithRetry(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        console.warn(
          `[OllamaApi] Attempt ${attempt}/${maxRetries} failed:`,
          errorMsg,
        );
        ...
        if (error instanceof ThinkingOnlyError) {
          ...
          const extractedFromThinking = this.extractToolCallsFromThinking(
            error.thinking,
          );
          if (extractedFromThinking) {
            console.log(
              `[OllamaApi] Extracted tool calls from thinking on attempt ${attempt} (${extractedFromThinking.length} chars)`,
            );
        ...
        if (errorMsg.includes("404") && errorMsg.includes("not found")) {
          console.error(
            `[OllamaApi] Model '${this.modelName}' not found in Ollama. Please pull the model first.`,
          );
          break;
        }
        ...
        if (
          ...
          console.error(
            `[OllamaApi] Quota exceeded — not retrying: ${errorMsg.substring(0, 100)}`,
          );
          break;
        }
        ...
        if (error instanceof RetryableNetworkError) {
          console.warn(
            `[OllamaApi] Retryable network error on attempt ${attempt}/${maxRetries}: ${errorMsg}`,
          );
        }
...
Do NOT leave the response field empty. Every turn must produce a non-empty response.`;
            ...
            if (options?.disableThinking) {
              console.log(
                `[OllamaApi] disableThinking=true but got thinking-only. Retrying with reinforced prompt (attempt ${attempt}/${maxRetries})`,
              );
            }
    ...
    if (lastThinking) {
      const extractedToolCalls =
        this.extractToolCallsFromThinking(lastThinking);
      if (extractedToolCalls) {
        console.log(
          `[OllamaApi] Extracted tool calls from thinking content (${extractedToolCalls.length} chars)`,
        );
        return `<think>${lastThinking}</think>\n${extractedToolCalls}`;
      }
      console.log(
        `[OllamaApi] All ${maxRetries} retries failed with thinking-only. Returning thinking as fallback (length: ${lastThinking.length})`,
      );
      return `<think>${lastThinking}</think>`;
    }
  ...
  private async sendMessageInternal(
    ...
    if (options?.disableThinking) {
      ...
      console.log(
        `[OllamaApi] Thinking disabled for this request (tool calling mode)`,
      );
    }
    ...
    if (options?.nativeTools && options.nativeTools.length > 0) {
      ...
      console.log(
        `[OllamaApi] Native tool calling enabled, tools count: ${options.nativeTools.length}`,
      );
    }

    console.log(
      `[OllamaApi] Sending request to ${url.toString()} with model ${this.modelName}`,
    );

    const rawResponse = await this.makeHttpRequest(url, requestData, options);

    console.log(
      `[OllamaApi] Raw response received: (${JSON.stringify(rawResponse).length} chars)`,
    );
    ...
    if (nativeToolCalls && nativeToolCalls.length > 0) {
      ...
      console.log(
        "[OllamaApi] Native tool_calls received, count:",
      ...
      const converted = nativeToolCalls
        .map((tc: any) => {
          ...
          if (strippedName !== fn.name) {
            console.log(
              `[OllamaApi] Tool name namespace stripped: ${fn.name} → ${strippedName}`,
            );
          }
      ...
      if (hasInvalidTool) {
        console.warn(
          "[OllamaApi] Native tool_calls contain unknown tools after stripping — retrying without native tools",
        );
    ...
    const hasToolCall =
      responseContent && /\{\s*["']tool["']\s*:\s*["']/.test(responseContent);

    const isJunkResponse =
      responseContent &&
      !hasToolCall &&
      (responseContent.includes("=== Tool Execution Results") ||
        ...
        (responseContent.length < 20 &&
          responseContent.includes("read_file") &&
    ...
    if (!responseContent || isJunkResponse) {
      if (
        ...
        console.log(
          "[OllamaApi] Thought detected but response is empty. Triggering retry.",
        );
    ...
    if (responseContent && !isJunkResponse) {
      ...
      // 호출자가 onThinking 콜백으로 UI에 전달할 수 있도록
      if (thinkingContent) {
  ...
  private async makeHttpRequest(
    ...
    return new Promise((resolve, reject) => {
      const isCloud =
        this.apiUrl.includes("ollama.com") || this.apiUrl.includes("cloud");
          ...
          res.on("end", () => {
            try {
              if (res.statusCode && res.statusCode >= 400) {
                ...
                reject(
                  new Error(
                    `Ollama API error (${res.statusCode}):${retryInfo} ${data}`,
                  ),
                );
                return;
              }
  ...
  public async sendMessageWithSystemPromptStreaming(
    ...
    return this.sendMessagesStreaming(
      [
        ...
        {
          role: "user",
          content: userParts.map((part) => part.text).join("\n"),
        },
      ],
      onChunk,
      options,
  ...
  private async sendMessagesStreaming(
    ...
    if (options?.disableThinking) {
      requestData.think = false;
      console.log(
        `[OllamaApi] Streaming: Thinking disabled for this request (tool calling mode)`,
      );
    }
    ...
    if (options?.nativeTools && options.nativeTools.length > 0) {
      requestData.tools = options.nativeTools;
      console.log(
        `[OllamaApi] Streaming: Native tool calling enabled, tools count: ${options.nativeTools.length}`,
      );
    }

    console.log(
      `[OllamaApi] Sending streaming request to ${url.toString()} with model ${this.modelName}`,
    );
        ...
        (res) => {
          if (res.statusCode && res.statusCode >= 400) {
            ...
            res.on("end", () => {
              ...
              reject(
                new Error(
                  `Ollama API error (${res.statusCode}):${retryInfo} ${errorData}`,
                ),
              );
            });
            return;
              ...
              try {
                ...
                if (nativeToolCalls?.length) {
                  for (const tc of nativeToolCalls) {
                    ...
                    try {
                      options?.onNativeToolComplete?.(fn.name, args);
                    } catch {
                      /* skip */
                    }
                  }
          ...
          res.on("end", () => {
                  ...
                  for (let i = prevLen; i < streamingToolCalls.length; i++) {
                    try {
                      options?.onNativeToolComplete?.(
                        ...
                        streamingToolCalls[i].arguments,
                      );
                    } catch {
                      /* skip */
                    }
            ...
            if (streamingToolCalls.length > 0) {
              ...
              const fullResult = thinkingText.trim()
                ...
                : converted;
              console.log(
                `[OllamaApi] Streaming: Native tool_calls converted, count: ${streamingToolCalls.length}`,
              );
              resolve(fullResult);
              return;
            }
            ...
            if (thinkingText.trim()) {
              ...
              if (!fullText.trim()) {
                const extracted =
                  this.extractToolCallsFromThinking(thinkingText);
                if (extracted) {
                  console.log(
                    `[OllamaApi] Streaming: Extracted tool calls from thinking (${extracted.length} chars)`,
                  );
