...
export class AdminModelApi {
  ...
  private async sendMessageInternal(
    ...
    console.log(
      `[AdminModelApi] detectProvider=${providerType} endpoint=${this.config.endpoint}`,
    );
  ...
  private static detectProvider(
  ...
  ): "gemini_native" | "anthropic_native" | "openai_compat" | "llama_cpp" {
    ...
    if (explicit === "vertex" || explicit === "vertex_ai")
    ...
    // llama.cpp /completion (native) 자동 감지 — /v1/chat/completions 가 아니고 /completion 으로 끝나면
    if (
      /\/completion\/?$/.test(endpoint) &&
      !endpoint.includes("/chat/completion")
    )
      return "llama_cpp";
    return "openai_compat";
  }
  ...
  private static createProvider(config: AdminModelConfig): ILLMProvider {
    ...
    if (type === "llama_cpp") return new LlamaCppProvider(config);
