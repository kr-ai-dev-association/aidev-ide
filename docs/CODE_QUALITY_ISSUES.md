...
### 2-1. ConversationManager — 5,700줄 God Class

**파일:** `src/core/managers/conversation/ConversationManager.ts`

**문제:**
단일 클래스가 다음을 모두 담당합니다.
- Intent 분석 / 계획 수립 / 조사 / 실행 / 검토 5개 페이즈
- Tool 실행 조율
- 에러 분류·재시도
- 컨텍스트 수집·압축
- 상태 머신 전환
...
**일반적인 방법:**
Cline, Continue 모두 `Phase` 혹은 `Handler` 단위로 분리합니다.
```
ConversationOrchestrator (진입점, 200줄 이내)
├── IntentPhaseHandler
├── PlanningPhaseHandler
├── InvestigationPhaseHandler
├── ExecutionPhaseHandler
└── ReviewPhaseHandler
```

**현재 문제:**
- 특정 페이즈 수정 시 5,700줄 파일 전체를 파악해야 함
- 단위 테스트 불가 (외부 의존성이 모두 내부에 혼재)
- 신규 페이즈 추가 시 이 파일을 반드시 수정해야 함

---

### 2-2. 병렬 에이전트 컨텍스트 공유 — Race Condition

**파일:** `src/core/orchestration/OrchestrationRouter.ts` / `SubAgentLoop.ts`

**문제:**
멀티 에이전트 실행 시 모든 Sub-Agent가 `ProjectManager`, `ContextManager`, `InlineDiffManager` 싱글톤을 공유합니다.
Agent A가 파일을 쓰는 동안 Agent B가 같은 파일을 읽으면 불일치 데이터를 볼 수 있습니다.

**일반적인 방법:**
각 에이전트에 격리된 컨텍스트 복사본을 주거나, 쓰기 작업에 락을 걸어야 합니다.
```typescript
// 올바른 패턴
const agentContext = SharedContext.fork(); // 격리된 복사
await agent.run(agentContext);
SharedContext.merge(agentContext); // 완료 후 병합
```

**현재 코드:** 싱글톤 직접 참조, 락 없음

---

### 2-3. InlineDiffManager — 비동기 상태 저장 Race Condition

**파일:** `src/core/managers/diff/InlineDiffManager.ts`

**문제:**
`globalState` 영속성 저장이 `await` 없이 비동기로 실행됩니다.
빠른 연속 편집 시 이전 상태가 덮어쓰여 diff 데이터가 유실될 수 있습니다.

**일반적인 방법:**
```typescript
// 잘못된 패턴 (현재)
context.globalState.update('diffState', data); // await 없음

// 올바른 패턴
await context.globalState.update('diffState', data);
// 또는 쓰기 큐로 직렬화
```
...
### 5-1. Deprecated 메서드 잔존

**파일:** `ConversationManager.ts` 라인 199-215

`@deprecated` 표시된 setter 메서드들이 실제로 남아 있어 신규 개발자가 잘못 사용할 수 있습니다.
...
### 즉시 (v12.0 이전)
1. `InlineDiffManager` globalState `await` 추가 — 데이터 유실 방지
2. `ToolExecutor` `Promise.all` → `Promise.allSettled` 전환
3. 병렬 에이전트 컨텍스트 격리 (최소한 파일 쓰기 직렬화)

### 단기 (v12.x)
1. `AdminModelApi` Provider 어댑터 패턴으로 리팩토링
2. Magic Number → `AgentConfig.ts` 중앙화
3. `any` 타입 핵심 인터페이스에서 제거
4. `deletedFiles` 크기 제한 추가

### 중기 (v13.x)
1. `ConversationManager` 페이즈별 클래스 분리
2. 도구 입력 Zod 검증 추가
