...
- **멀티모델 AI 지원**:
  ...
  - **Ollama 통합**: 오프라인 AI 처리를 위한 로컬 Ollama 서버 통합
    ...
    - **llama3.2:3b**: 3B 파라미터 모델로 가벼우면서 빠른 추론
    - **codellama:7b**: 7B 파라미터 모델로 코드 생성 및 분석에 특화
    - **gemma2:2b**: 2B 파라미터 모델로 빠른 응답과 기본 작업
    - **banya-llama31-lora-merged:latest**: 특수 작업을 위한 커스텀 파인튜닝 모델
    - **banya-helper-3b:latest**: 개발 어시스턴스에 최적화된 커스텀 헬퍼 모델
    - **banya-l31-base:latest**: 기본 AI 작업을 위한 커스텀 베이스 모델
    - **HammerAI/llama-3-lexi-uncensored:8b-q5_K_M**: 제한 없는 AI 상호작용을 위한 언센서드 모델
