...
## v5.2.0 (UI/UX 대전환 & 에이전트 루프 고도화)
- **UI/UX 대전환**:
  - **상태 표시 통합**: `TaskQueue` 패널과 상단 `ProcessingSteps`를 제거했습니다. 모든 진행 상태는 이제 하단 로딩 영역에서 터미널 스타일의 타자기 애니메이션으로 실시간 표시됩니다.
  - **조건부 스티키(Sticky) 바**: 진행 상태 바가 화면을 따라다닙니다. 화면에 보일 때는 인라인으로, 스크롤되어 사라질 때는 상단에 고정되어 현재 작업을 항상 확인할 수 있습니다.
  - **정돈된 인터페이스**: LLM의 내부 독백(Thinking)을 채팅창에서 숨기고, 사용자에게 꼭 필요한 추론과 작업 결과만 노출하도록 개선했습니다.
- **에이전트 루프 지능화**:
  - **인터리브드(Interleaved) 출력**: LLM의 설명과 도구 실행 결과(코드 미리보기 포함)를 순차적으로 배치하여 작업 흐름을 한눈에 파악할 수 있습니다.
  - **스마트 너징(Nudging)**: 분석만 하고 행동하지 않는 경우를 감지하여, LLM이 실제 도구를 호출하거나 계획을 세우도록 유도하는 로직을 추가했습니다.
  - **백그라운드 실행**: `npm install` 등 시간이 오래 걸리는 명령어는 "소프트 타임아웃"을 적용하여 루프를 멈추지 않고 백그라운드에서 계속 실행되도록 개선했습니다.
- **도구 시스템 강화**:
  - **초강력 update_file**: 공백 무시 구조적 매칭, 블록 앵커 매칭, 순서 무관 수정 등을 도입하여 파일 수정의 정확도와 안정성을 대폭 향상시켰습니다.
  - **지능형 list_files 필터링**: `node_modules`, `.git`, 빌드 폴더 등 불필요한 경로를 자동으로 제외하여 LLM에게 필요한 컨텍스트만 제공합니다.
  - **시스템 메시지 UI 표준화**: 도구 실행 알림(`✅ [Created]`, `🚀 [Executed]` 등)의 스타일을 아이콘과 코드 스니펫이 포함된 세련된 디자인으로 통일했습니다.
- **아키텍처 정교화**:
  - **ConversationManager 리팩토링**: 단일 책임 원칙(SRP)에 따라 로직을 분리하고 타입 안정성을 강화했습니다.
  - **동적 모델 로딩**: Ollama 모델의 하드코딩 매핑을 제거하고, 사용자가 UI에서 선택한 모델을 실시간으로 반영하도록 수정했습니다.
  - **중앙 집중식 UI 브리지**: 모든 웹뷰 통신을 `WebviewBridge` 서비스로 통합하여 통신 규격을 일원화했습니다.
