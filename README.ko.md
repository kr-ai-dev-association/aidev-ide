...
- **터미널 컨텍스트 지원**: '@' 메뉴에 Terminal 카테고리를 추가했습니다 (Continue IDE와 유사).
  ...
  - 터미널 선택 시 해당 터미널의 명령어 히스토리가 LLM 컨텍스트에 포함
  - 입력창에 주황색 배지로 터미널 멘션 블록 표시
  - 최근 20개 명령어와 출력이 컨텍스트에 포함
  - 종료 코드가 ✓ (성공) 또는 ✗(코드) (실패)로 표시
  - 긴 출력은 자동으로 500자로 잘림
- **파일/터미널 멘션 동기화**: 멘션 블록 삭제 처리를 개선했습니다.
  - 백스페이스로 파일/터미널 멘션 삭제 시 선택 상태가 올바르게 초기화됨
  - 멘션 제거 시 "(선택됨)" 표시가 정확하게 업데이트됨
  - `syncMentionsWithDOM()` 함수로 DOM과 상태 일관성 보장
- **컨텍스트 통합**: 파일과 터미널 컨텍스트가 모두 시스템 프롬프트에 포함됩니다.
  - 파일 컨텍스트: "**사용자가 선택한 파일들:**" 섹션에 전체 파일 내용 포함
  - 터미널 컨텍스트: "**터미널 컨텍스트:**" 섹션에 명령어 히스토리와 출력 포함
  - LLM이 두 컨텍스트를 참조하여 사용자의 개발 환경을 이해할 수 있음
- **빠른 Diagnostics 검사**: CLI 명령 실행 전 VS Code Diagnostics 검증 추가.
  - LSP 기반 diagnostics로 문법/타입 에러를 즉시 감지
  - `DiagnosticSeverity.Error`만 필터링 (Warning은 무시)
  - CLI 검증 명령 전에 실행하여 빠른 피드백 제공
  - 에러 발견 시 CLI 실행 없이 즉시 반환 (시간 절약)
  - 에러 형식: `file:line: message` (최대 10개 표시)

## v8.8.1 (요약 개선 및 세션 히스토리 복원)
- **요약 형식 개선**: 작업 완료 후 요약 출력 형식을 마크다운으로 개선했습니다.
  - `### 작업 완료 ✅`, `### 변경 내용`, `### 사용 방법` 섹션으로 구조화
  - 파일별 변경 내용을 `**파일명**: 설명` 형식으로 표시
  - 프로젝트 실행 방법을 코드 블록으로 제공
  - LLM이 도구 태그로 응답 시 재시도 로직 추가 (최대 3회)
- **세션 히스토리 완전 복원**: VSCode 재시작 시 대화 내용 완전 복원.
  - 코드 블록 (생성/수정된 파일 내용) 복원
  - 액션 영역 (📖 [Read], ✅ [Created], 📝 [Updated] 등) 복원
  - `uiMessages` 필드를 통해 웹뷰에 표시된 모든 메시지 저장
  - 구버전 데이터와의 하위 호환성 유지
- **Action Tracker UI 제거**: 불필요한 실시간 액션 트래커 UI 제거.
  - "작업 진행 중... 📖파일 읽기: App.tsx" 같은 표시 제거
  - `WebviewBridge.updateActionTracker`, `clearActionTracker` 메서드 제거
  - `ToolExecutor`에서 액션 트래커 호출 코드 제거
  - 관련 CSS 스타일 제거로 번들 크기 감소

## v8.8.0 (실시간 액션 트래커 및 컨텍스트 개선)
- **실시간 액션 트래커**: AI 작업 중 Windsurf 스타일의 실시간 액션 추적 UI를 추가했습니다.
  - 파일 읽기, 생성, 수정, 삭제 진행 상황을 실시간으로 표시
  - 명령 실행 상태를 실시간으로 표시
  - 검색 및 분석 작업을 시각적 표시기로 추적
  - 각 액션에 시작/완료/오류 상태를 아이콘과 함께 표시
  - 활성 작업에 대한 애니메이션 스피너 및 상태 표시기
  - 최대 10개 액션 표시 및 자동 스크롤
- **열린 탭 컨텍스트**: LLM 컨텍스트에 에디터의 열린 탭을 포함하는 기능을 추가했습니다.
  - `ContextManager.getOpenTabsContext()`가 열린 탭 목록을 반환
  - 파일 경로, 이름, 언어, 활성 상태, 수정 상태 포함
  - LLM이 사용자가 작업 중인 파일을 이해하는 데 도움
- **파일 부분 읽기**: `read_file` 도구에 줄 범위 지원을 추가했습니다.
  - 대용량 파일의 특정 부분을 읽기 위한 `startLine` 및 `endLine` 매개변수
  - 쉬운 참조를 위해 내용과 함께 줄 번호 반환
  - 파일의 일부만 필요할 때 토큰 사용량 절감
- **WebviewBridge 개선**: `updateActionTracker` 및 `clearActionTracker` 메서드 추가
- **ToolExecutor 통합**: 모든 도구 실행에 대한 자동 액션 추적
- **시각적 개선**:
  - 액션별 아이콘 및 색상 코딩 (읽기: 파란색, 생성: 녹색, 수정: 주황색, 삭제: 빨간색)
  - 새 액션에 대한 부드러운 슬라이드-인 애니메이션
  - 완료된 액션은 진행 상황을 나타내기 위해 페이드 처리

## v8.7.8 (입력 영역 스타일 개선)
- **입력 영역 테두리 스타일**: 더 깔끔한 UI를 위한 입력 영역 테두리 외관 개선.
  - 입력 영역(`.input-row`)에 연한 회색 테두리 추가
  - 포커스 시 파란색 테두리 제거, 일관된 연한 회색 테두리 유지
  - 더 깔끔하고 일관된 시각적 외관 제공
- **파일 선택 UX 개선**: '@' 명령어 파일 선택 동작 개선.
  - 파일 선택 후 입력창에 파일명이 남지 않도록 수정
  - 선택된 파일은 파일 선택 영역에만 표시됨
  - '@' 명령어 사용 시 더 깔끔한 입력 경험
- **UI 텍스트 일관성**: Pending Changes 버튼의 텍스트 대소문자 표준화.
  - "file/files"를 "File/Files"로 변경하여 일관성 유지

## v8.7.7 (UI 개선 및 코드 블록 토글 리팩토링)
- **코드 블록 토글 리팩토링**: 코드 블록 토글 기능을 이벤트 위임 방식으로 리팩토링했습니다.
  - 직접 이벤트 리스너에서 `codepilot://toggle` 스킴 기반 이벤트 위임으로 변경
  - 각 코드 블록에 고유 ID를 부여하여 안정적인 토글 상태 관리
  - 파일 열기 및 diff 아이콘과의 충돌을 방지하기 위한 클릭 처리 개선
  - 토글 버튼과 헤더가 앵커 태그와 커스텀 스킴을 사용하여 더 나은 이벤트 처리
- **UI 단순화**: 드롭다운 버튼과 옵션의 테두리와 배경을 제거하여 일반 텍스트처럼 보이도록 개선했습니다.
  - 모델 선택기와 Code/ASK 모드 선택기 버튼이 이제 일반 텍스트처럼 표시됨
  - 드롭다운 버튼과 옵션의 모든 왼쪽 색상 포인트 제거
  - 파일 버튼은 이미 테두리/배경이 없었으며, 이제 모든 컨트롤에서 일관성 유지
- **토큰 사용량 툴팁 개선**: 토큰 사용량 툴팁에 컨텍스트 정보를 추가했습니다.
  - 툴팁에 토큰 사용량과 컨텍스트 메시지 개수가 모두 표시됨
  - 형식: "토큰 사용량: X / Y\n컨텍스트: Z개 메시지"
  - 한눈에 포괄적인 컨텍스트 정보 제공
- **알림 정리**: 사용자 경험 개선을 위해 승인 알림 메시지를 제거했습니다.
  - 파일 변경 승인 시 "변경사항이 승인되었습니다" 알림이 더 이상 표시되지 않음
  - 개발 워크플로우 중 알림 노이즈 감소

## v8.7.6 (대화 히스토리 관리 아키텍처 개선)
- **통합된 히스토리 시스템**: 중복된 4개 저장소를 SessionManager 단일 저장소로 통합하여 컨텍스트 관리 명확화.
  - `SessionManager.conversationHistory`가 유일한 진실의 원천 (Single Source of Truth)
  - 전체 대화 내용 + 구조화된 메타데이터를 함께 저장
  - ASK 모드와 CODE 모드 모두 동일한 저장소 사용
- **ConversationEntry 타입 확장**: 전체 응답과 메타데이터를 모두 저장.
  - `assistantResponse`: LLM 전체 응답 저장 (ASK 모드 컨텍스트 재사용)
  - `filesCreated`, `filesModified`, `commandsExecuted`: 구조화된 메타데이터
  - `compactedSummaryId`: 압축된 요약 참조
  - `durationMs`: 실행 시간 추적
- **자동 세션 압축**: LLM 요약을 사용한 지능형 히스토리 압축.
  - 토큰 임계값 80% 초과 시 자동 압축
  - 오래된 대화를 LLM으로 요약 후 `compactedSummaries`에 영속 저장
  - 최근 20개 대화는 원본 유지
  - 압축 실패 시 단순 trim으로 폴백
- **ASK 모드 컨텍스트 개선**: 요약 + 최근 대화 구조로 컨텍스트 제공.
  - `getHistoryContext()` 메서드가 압축된 요약과 최근 대화를 결합
  - 과거 대화 컨텍스트가 소실되지 않고 요약 형태로 보존
  - Cursor/Copilot/Claude Code와 동등한 컨텍스트 관리 품질
- **ConversationCompactor 통합**: SessionManager와 ConversationCompactor 통합.
  - `generateSummaryFromText()` 메서드로 직접 요약 생성 가능
  - SessionManager가 압축 시점 결정 및 요약 영속화 담당
  - 루프 내 임시 압축 + 세션 영속 압축 이중 전략
- **성능 최적화**: 불필요한 LLM 호출 제거 및 캐싱 활용.
  - CODE 모드는 파일 변경 정보만 저장 (전체 응답 생략 가능)
  - 구조화된 메타데이터로 빠른 검색 및 필터링
  - 압축된 요약 재사용으로 토큰 비용 절감

## v8.7.6 (컨텍스트 시각화 및 토큰 사용량 표시)
- **컨텍스트 시각화**: 입력 영역에 실시간 컨텍스트 정보 표시 추가.
  - 현재 컨텍스트에 포함된 메시지 수 표시
  - 토큰 사용량을 시각적 표시기로 표현
  - 토큰 사용량에 따른 색상 코드 경고 (70% 노란색, 90% 빨간색)
  - 대화 중 자동 업데이트
- **토큰 사용량 추적**: 실시간 토큰 소비량 모니터링.
  - 현재 토큰 / 최대 토큰 표시
  - 백분율 기반 표시로 직관적 이해
  - 컨텍스트 길이를 효과적으로 관리 가능
...
- **프로젝트 컨텍스트 캐싱**: 자주 참조하는 파일과 프로젝트 구조를 캐싱하여 성능을 대폭 향상시켰습니다.
  ...
  - 파일 변경 자동 감지 및 캐시 무효화
  - LRU 캐시 정책으로 메모리 효율적 관리
  - TTL (Time To Live) 기반 자동 만료 (5분 기본)
  - 최대 캐시 크기 제한 (10MB 기본)
  - 디스크 영구 저장으로 재시작 후에도 캐시 유지
  - 프로젝트 구조 캐싱 (파일 트리, 설정 파일 목록)
  - 캐시 히트율 및 통계 조회 기능
- **채팅 패널 슬래시 명령어**: 채팅 입력창에서 슬래시 명령어 지원 추가.
  - 채팅 입력창에 `/` 입력하면 사용 가능한 명령어 표시
  - `/cache` - 캐시 통계 보기
  - `/clear-cache` - 컨텍스트 캐시 초기화
  - `/sessions` - 저장된 세션 목록 보기
  - `/restore` - 저장된 세션 복원
  - 키보드 네비게이션 지원 (방향키 위/아래, Enter, Escape)
- **새로운 명령어 추가**:
  - `Codepilot: View Cache Statistics` - QuickPick UI로 캐시 통계 보기
  - `Codepilot: Clear Context Cache` - 확인 프롬프트와 함께 컨텍스트 캐시 초기화
  - `Codepilot: List Saved Sessions` - 저장된 세션 목록 보기
  - `Codepilot: Restore Saved Session` - 저장된 세션 복원
- **SessionManager 확장**: 프로젝트 컨텍스트 캐싱을 통한 성능 향상
...
- **ContextCompat**: 하이브리드 요약 전략을 구현하는 새 클래스 (`.ts`):
