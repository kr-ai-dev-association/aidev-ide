...
- **통합 의도 분석 엔진**: `IntentDetector`가 현재 활성화된 LLM(Gemini 또는 Ollama)을 사용하도록 리팩토링했습니다. 이제 Gemini 모드에서 의도 분석 시 Ollama를 호출하던 문제가 해결되었습니다.
...
## v6.0.0 (LLM 중심 의도 분석 및 지능형 에러 처리)
- **LLM 중심 의도 분석**: 하드코딩된 키워드 매칭을 완전히 제거하고, LLM 기반의 의도 분류를 도입하여 정확도와 유연성을 극대화했습니다.
