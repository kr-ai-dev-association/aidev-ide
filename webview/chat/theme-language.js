...
export function updateChatContainerPadding() {
  ...
  const totalBottomHeight =
    pendingHeight + fileSelectionHeight + chatInputHeight + 20;
...
export function applyTheme(theme) {
  ...
  if (theme === "auto") {
    ...
    effectiveTheme =
      vscodeThemeKind && vscodeThemeKind.includes("light") ? "light" : "dark";
  }
...
// ASK / CODE 모드 보내기 버튼 스타일 (AGENT·PLAN 모드 UI 제거)
// effectiveMode는 window.chatMode에서 읽음 (chat.js의 chat-mode-changed 이벤트로 갱신됨)
export function updateSendButtonStyle() {
  ...
  const raw = window.chatMode || "CODE";
  const effectiveMode = raw === "ASK" ? "ASK" : "CODE";
