...
if (criticPassToggle) {
  criticPassToggle.addEventListener("change", () => {
    ...
    if (criticPassStatus) {
      criticPassStatus.textContent = enabled
        ? languageData["criticPassOn"] || "코드검증: 켜짐"
        : languageData["criticPassOff"] || "코드검증: 꺼짐";
      criticPassStatus.className = enabled
        ? "info-message success-message"
        : "info-message";
    }
    if (vscode) {
      vscode.postMessage({ command: "setCriticPassEnabled", enabled });
    }
  });
}
  ...
  switch (message.command) {
    ...
    case "currentSettings":
      ...
      if (typeof message.criticPassEnabled === "boolean" && criticPassToggle) {
        ...
        if (criticPassStatus) {
          criticPassStatus.textContent = message.criticPassEnabled
            ? languageData["criticPassOn"] || "코드검증: 켜짐"
            : languageData["criticPassOff"] || "코드검증: 꺼짐";
          criticPassStatus.className = message.criticPassEnabled
            ? "info-message success-message"
            : "info-message";
        }
      }
