...
function showSupportedModelSettings(groupName, selectedKey) {
  ...
  const isCustom = typeof resolvedKey === "string" && resolvedKey.startsWith("custom::");
  let customModelId = "";
  if (isCustom) {
  ...
  const activePreset = !isCustom && resolvedKey ? groupModels.find(s => s.key === resolvedKey) || groupModels[0] : groupModels[0];
...
const supportedModelCustomGroup = document.getElementById("supported-model-custom-group");
const supportedModelCustomId = document.getElementById("supported-model-custom-id");
const applySupportedModelCustomButton = document.getElementById("apply-supported-model-custom-button");
...
function applyLanguage() {
  ...
  infoMessages.forEach(msg => {
    ...
    if (text && (text.includes("AgentGoCoder가 AI 응답을 생성할 때 참조할 소스 코드 경로 목록입니다") || text.includes("This is a list of source code paths that AgentGoCoder will reference") || text.includes("Esta es una lista de rutas de código fuente que AgentGoCoder referenciará") || text.includes("Ceci est une liste de chemins de code source que AgentGoCoder référencera") || text.includes("这是 AgentGoCoder 在生成 AI 响应时将引用的源代码路径列表") || text.includes("これは、AgentGoCoderがAI応答を生成する際に参照するソースコードパスのリストです"))) {
      // 소스 경로 설명
...
if (supportedModelSubselect) {
  supportedModelSubselect.addEventListener("change", () => {
    ...
    if (newKey === "__custom__") {
      if (supportedModelCustomGroup) supportedModelCustomGroup.style.display = "block";
      if (supportedModelCustomId) supportedModelCustomId.focus();
      currentSupportedModelKey = null;
      const aiModels = cachedServerSettings["ai_model"] || [];
      const gm = aiModels.find(s => s.source === "preset" && (s.group || "default") === currentSupportedGroup);
      if (gm) updateSupportedModelApiKeySection(gm.value || {});
      return;
    }
    if (supportedModelCustomGroup) supportedModelCustomGroup.style.display = "none";
...
if (applySupportedModelCustomButton) {
  applySupportedModelCustomButton.addEventListener("click", () => {
    const modelId = supportedModelCustomId ? supportedModelCustomId.value.trim() : "";
    if (!modelId) {
    ...
    case "routingOllamaModels":
      {
        ...
        if (Array.isArray(message.models)) {
          ...
          const prefixes = ["compactor", "command", "intent", "error-fallback"];
  ...
  switch (message.command) {
    ...
    case "agentPolicyFileAdded":
      if (message.category && message.fileName) {
        ...
        const existingList = Array.isArray(agentPolicyFilesCache[cat]) ? agentPolicyFilesCache[cat].slice() : [];
        if (!existingList.includes(message.fileName)) {
          existingList.push(message.fileName);
          agentPolicyFilesCache[cat] = existingList;
          // policyType/skillDescription 이 message에 오면 캐시에 반영
...
(function initUserModels() {
  ...
  function applyProviderPlaceholders() {
    ...
    const ph = PROVIDER_PLACEHOLDERS[providerSel.value] || PROVIDER_PLACEHOLDERS.custom;
  ...
  function applyProviderEndpoint() {
    ...
    const preset = PROVIDER_PLACEHOLDERS[providerSel.value] || PROVIDER_PLACEHOLDERS.custom;
    endpointInput.value = preset.endpoint;
  }
  function applyAuthTypeVisibility() {
    if (!authTypeSel || !authHeaderGroup) return;
    authHeaderGroup.style.display = authTypeSel.value === "custom_header" ? "flex" : "none";
  }
  function resetForm(isEdit) {
    if (formTitle) formTitle.textContent = isEdit ? "AI 모델 수정" : "새 AI 모델 추가";
  ...
  function populateForm(m) {
    ...
    if (apiKeyInput) apiKeyInput.value = "";
    if (apiKeyHint) {
      apiKeyHint.textContent = m.hasApiKey ? "이미 저장된 API 키가 있습니다. 변경하려면 새 키를 입력하세요." : "이 모델에 API 키를 등록하려면 입력하세요.";
      apiKeyHint.style.display = "block";
    }
    if (contextInput) contextInput.value = m.contextWindow != null ? String(m.contextWindow) : "";
    if (maxOutInput) maxOutInput.value = m.maxOutputTokens != null ? String(m.maxOutputTokens) : "";
    if (tempInput) tempInput.value = m.defaultTemperature != null ? String(m.defaultTemperature) : "0.7";
    if (topPInput) topPInput.value = m.topP != null ? String(m.topP) : "0.9";
    if (customHeadersInput) {
      customHeadersInput.value = m.customHeaders && typeof m.customHeaders === "object" ? JSON.stringify(m.customHeaders, null, 2) : "";
    }
    if (streamingSel) streamingSel.value = m.streamingSupported === false ? "false" : "true";
    if (nativeToolsSel) nativeToolsSel.value = m.nativeToolCallingSupported ? "true" : "false";
    applyAuthTypeVisibility();
    applyProviderPlaceholders();
  }
  function showForm() {
  ...
  function setStatus(text, isError) {
    ...
    statusEl.className = "info-message" + (isError ? " error-message" : text ? " success-message" : "");
  }
  function collectFormInput() {
    return {
      ...
      authHeaderName: authHeaderNameInput ? authHeaderNameInput.value.trim() : "",
      contextWindow: contextInput && contextInput.value !== "" ? Number(contextInput.value) : undefined,
      maxOutputTokens: maxOutInput && maxOutInput.value !== "" ? Number(maxOutInput.value) : undefined,
      ...
      nativeToolCallingSupported: nativeToolsSel ? nativeToolsSel.value === "true" : false
    };
  }
  if (addBtn) {
    addBtn.addEventListener("click", () => {
  ...
  if (authTypeSel) authTypeSel.addEventListener("change", applyAuthTypeVisibility);
  if (saveBtn) {
    saveBtn.addEventListener("click", () => {
      if (formErrorEl) formErrorEl.textContent = "";
      ...
      if (input.authType === "custom_header" && !input.authHeaderName) {
        formErrorEl.textContent = "custom_header 인증 방식에서는 헤더 이름이 필요합니다.";
        return;
      }
  ...
  if (listEl) {
    listEl.addEventListener("click", e => {
      ...
      if (editKey) {
        ...
        if (confirm("이 사용자 모델을 삭제하시겠습니까? API 키도 함께 삭제됩니다.")) {
          vscode.postMessage({
            command: "deleteUserModel",
            key: deleteKey
          });
        }
      }
  ...
  window.addEventListener("message", event => {
    ...
    switch (message.command) {
      ...
      case "userModelConnectionTestResult":
        {
          setStatus((message.success ? "✅ " : "❌ ") + (message.message || ""), !message.success);
          break;
        }
    }
  });
