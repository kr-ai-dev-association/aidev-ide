...
- **`application/octet-stream` Content-Type 허용**: URL 이 `.md`/`.markdown` 확장자인 경우에만 octet-stream 통과
- **frontmatter 필수 검증 제거**: `injectOriginMetadata` 가 frontmatter 없으면 origin 메타만 포함한 블록 자동 생성

### 테마/스타일 개선 — 라이트 모드

- **일반 탭 저장 버튼 outline 스타일**: 파란 배경 → 투명 + 회색 테두리 + 회색 글씨
- **규칙/스킬 토글 `.policy-type-btn`**: 양쪽 테마 모두 active 파란색 유지
- **보안 탭 기본 차단 명령어 목록**: 컨테이너 회색 배경 + 각 아이템 배경 투명
- **미리보기 모달 색상**: 박스 배경 회색 + 내부 pre 흰색 + 스크롤바 흰색 계열
- **미리보기 메타 색상**: label + value 모두 `color: inherit` 통일

### 알림 메시지 한글화

- `CODEPILOT: {파일명} saved/deleted` → `CODEPILOT: {파일명} 저장됨/삭제됨`
- `Error adding/deleting Agent Policy file: …` → `CODEPILOT: 파일 저장/삭제 실패 — …`
- 카테고리별 저장/삭제/에러 메시지 한글
...
- **Skills 설정 UI에 URL 다운로드 추가**: 6개 카테고리 각각의 경로 입력 그룹 뒤에 URL 입력+"URL 다운로드" 버튼 동적 주입
...
- **파일 시스템 안전장치**: URL→파일명 slug 변환, `..` 차단, 80자 상한, `.md` 강제
...
- **래핑 대상**: fetch_url **만**. Skills/Rules/사용자 메시지/시스템 프롬프트 래핑 안 함 (standalone은 RAG 비활성화됨)
...
- **저장 분리**: `apiKey`는 `vscode.SecretStorage`, 나머지는 `globalState['codepilot-standalone.userModels']`. `saveAiModel` 경로에 `user:{key}` 접두어 분기 추가 — 기존 admin·supported 경로와 대칭
...
- **의의**: Admin 대시보드 없이도 IDE 단독으로 임의의 OpenAI-호환·Anthropic·Gemini 엔드포인트를 사용할 수 있음 — standalone 모드의 모델 설정 공백 해소

### 알림 메시지 브랜드명 통일 (`CODEPILOT` → `CODEPILOT-STANDALONE`)

- **4개 파일 64곳** 알림 prefix를 `"CODEPILOT:"` → `"CODEPILOT-STANDALONE:"`으로 일괄 변경: `SettingsPanelProvider.ts`, `AgentPolicyHandler.ts`, `UserModelHandler.ts`, `AskViewProvider.ts`
- i18n 문자열·타이틀 등에서 이미 `CODEPILOT-STANDALONE`을 쓰던 것과 정합
...
- **fromSettings 플래그 유지**: COMMAND_NOT_FOUND 발생 시 기존 폴백 로직 그대로 작동
...
- **webview 팝업으로 변경**: 이전 세션 중단 감지 시 VS Code toast 대신 webview 모달 팝업으로 표시 — 사용자 놓침 방지
- **복구 모드 전달 수정**: 중단된 세션 이어하기 시 `mode: "CODE"` 누락 수정

### TestRunner 개선

- **TypeScript 빌드 검증 강화**: `npx tsc --noEmit` → `npx tsc --noEmit && ${pm} run build` — build 스크립트 있는 프로젝트에서 빌드까지 검증
- **autoTestRetryEnabled 기본값 변경**: `false` → `true` — 테스트 실패 시 자동 재시도 기본 활성화
- **TestRunner npm install 자동 실행 제거**: LLM 자율 판단으로 변경

### 스트리밍 안정성

- **streamingHandledPaths 전체 실패 추적**: 모든 실패 케이스 추적으로 확장 — 스트리밍 중복 실행 방지
...
- **SubAgentLoop 사용자 설정 존중**: SubAgentLoop에서 `isNativeToolCallingEnabled()` 사용자 설정을 무시하고 서버 설정(`nativeToolCallingSupported`)만 확인하던 문제 수정
...
- **세팅 > 일반 > 토글 추가**: "프로젝트 외부 파일 차단" (기본: ON)
- **하드코딩 제거**: 사용자 설정으로 전환
- **차단 시 채팅 메시지 표시**: `🚫 [차단]` 메시지 표시 + 중복 실행 방지
- **READ_ONLY_FILES 죽은 코드 제거**

### 버그 수정

- **AGENT 모드 Prompt Suggestion 설정 무시 수정**: OFF인데도 제안 표시되던 문제 수정
- **Windows PowerShell 실행 방식 개선**: `shell: false` 직접 실행으로 변경 — 프로세스 1개, 인자 파싱 안전성 향상
...
- **AGENT 모드 cwd 자동 추론 스킵**: LLM이 직접 경로 관리 (자율 방식)
...
- **repair agent MAX_TURNS**: 25턴 → **10턴**
...
- **Session Memory 자동 추출**: 20K토큰 + 5턴 임계값, 최대 3개 항목 자동 저장
- **Diagnostic Tracker**: LSP 진단 baseline + delta 감지
- **File Checkpoint UUID 매칭**: 재시작 후 UNDO 정상 동작

### 성능 / 캐시

- **ToolSpecBuilder 캐시**: 도구 스펙 캐싱
- **Cache Break Detection**: 프롬프트 변경 시 캐시 클리어
...
- **자동 검증 스킵**: AGENT 모드에서 시스템 자동 tsc 검증 제거 — LLM이 직접 `run_command`로 검증 (자율 방식)
...
- **크래시 복구**: 중단된 세션 감지 + "이어서 진행" 알림
- **세션 간 이어하기**: 이전 요약 자동 주입
- **PLAN 계획 파일 저장**: 승인 시 `plans/plan_{sessionId}.md` 저장
- **세션 삭제 시 plan 파일 정리**

### 도구/프롬프트

- **ask_question 유일성 검증**: 중복 질문/옵션 체크
- **프롬프트 차별화**: CodePilot 고유 스타일로 재작성 (코드 품질, Git, 보안)
...
- **Cyber Risk / OWASP 보안 프롬프트**: 보안 가이드라인 + 취약점 방지 지시
- **Git 워크플로 / 코드 품질 프롬프트**: conventional commit, 과도한 추상화 금지
- **spawn_agent sync vs background 결정 트리**

### UI 한글화

- **processStep 영어 → 한글**: 명령 준비, 검증 실행, 자동 수정 관련 메시지
...
- **Zsh 위험 명령 +9 패턴**: `zmodload`, `sysopen`, `zpty`, `curl|bash` 등
- **읽기 전용 화이트리스트 인프라**: `isReadOnlySafeCommand()` 추가

### 컨텍스트 압축

- **9섹션 구조화 요약 프롬프트**: 압축 후 파일 경로/에러/사용자 메시지 정확 보존
- **이미지 제거**: 압축 전 `[image]` 마커로 대체

### 프로세스/프로젝트

- **출력 크기 감시**: 10MB 초과 경고
- **Go/Rust 스택 감지**: go.mod/Cargo.toml 파싱, 프레임워크/ORM/워크스페이스 감지
...
- **서브에이전트 토큰 최적화** (`OrchestrationRouter`, `SubAgentLoop`): Hot Load 제외, 대화 압축 (30K chars 초과 시 중간 도구 결과 요약)
...
- **`FileChangeHandler` import 경로 수정** (`FileChangeHandler.ts`): `../../managers/context/file/FileSearcher` → `../../context/file/FileSearcher`로 수정 — 빌드 warning 해소
...
- **서버 동기화 레거시 코드 제거** (`SettingsManager`): standalone에서 no-op이던 `syncServerSettings()`, `isCacheValid()`, `CACHE_TTL_MS`, `syncInProgress` 및 관련 import 제거 — `loadOfflineCache()` 및 프리셋 모델 목록은 Admin 모델 기능에 필요하므로 유지
- **Admin MCP 서버 레거시 코드 제거** (`MCPManager`, `SettingsPanelProvider`, `SettingsManager`): 서버 push MCP 설정이 항상 빈 배열이던 `adminServers` 필드, `mergeServerMCPConfigs()`, `getAdminServers()`, `isAdminServer()`, `toggleAdminServer()`, `getServerMCPConfigs()` 및 관련 UI 케이스(`toggleAdminMcpServer`) 제거
