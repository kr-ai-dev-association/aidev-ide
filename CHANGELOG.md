...
- **ASK = 자연어 답변 (plan/도구 출력 방지)**: 전용 ASK 프롬프트(`getGeneralAskPrompt`)에 읽기 전용 규칙을 명시하여, 약한 모델이 `plan JSON`·`<tool_code>` 같은 작업 형식을 출력하는 대신 코드 예시와 설명을 자연어로 답하도록 했습니다. ASK 텍스트 응답을 최종 답변으로 수용해 INVESTIGATION 무한 루프도 제거했습니다.
- **ASK = 설명 질문도 자연어 (컨텍스트 grounding)**: "이 코드 설명"·"프로젝트 구조 설명" 류 질문에서 모델이 `read_file` 도구 코드를 출력하던 문제를, ASK 프롬프트에 Repo Map·프로젝트 구조·메모리 컨텍스트를 주입해 도구 호출 없이 답하도록 수정했습니다.
- **ASK 대화 저장/복원 수정**: ASK 응답이 빈 entry로 저장돼(라벨 "CODE" 하드코딩) 재시작 시 "이전 작업이 중단되었습니다" resume 오탐 + 히스토리 누락이 발생하던 문제를 수정했습니다. ASK 응답을 보존하고 라벨을 "ASK"로 저장하며, code-modify·analysis 두 응답 경로 모두에 적용했습니다.
