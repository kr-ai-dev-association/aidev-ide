...
exports.GitBranchAnalysisService = void 0;
const child_process_1 = require("child_process");
const util_1 = require("util");
const execAsync = (0, util_1.promisify)(child_process_1.exec);
class GitBranchAnalysisService {
    gitRepositoryService;
    constructor(gitRepositoryService) {
        this.gitRepositoryService = gitRepositoryService;
    }
    /**
     * 모든 브랜치의 이슈를 분석하고 개선 방안을 도출
     */
    async analyzeAllBranches(projectRoot) {
        try {
            // 모든 브랜치 목록 가져오기
     ...
     * 특정 브랜치의 이슈를 분석
     */
    async analyzeBranch(projectRoot, branch) {
        try {
            // 브랜치로 전환
     ...
     * 모든 브랜치 목록 가져오기
     */
    async getAllBranches(projectRoot) {
        try {
     ...
     * 브랜치로 전환
     */
    async switchToBranch(projectRoot, branch) {
        try {
     ...
     * 브랜치별 이슈 감지
     */
    async detectBranchIssues(projectRoot, branch) {
        const issues = [];
        try {
            // 1. 코드 품질 이슈 감지
     ...
     * 코드 품질 이슈 감지
     */
    async detectCodeQualityIssues(projectRoot) {
        const issues = [];
        try {
            // ESLint 오류 검사
            try {
     ...
     * 의존성 이슈 감지
     */
    async detectDependencyIssues(projectRoot) {
        const issues = [];
        try {
            // package.json이 있는 경우
            ...
            if (fs.existsSync(packageJsonPath)) {
                ...
                try {
                    ...
                    Object.entries(outdatedResults).forEach(([pkg, info]) => {
                        issues.push({
                            branch: 'current',
                            issue: `오래된 의존성: ${pkg}`,
                            severity: 'low',
     ...
     * 테스트 커버리지 이슈 감지
     */
    async detectTestCoverageIssues(projectRoot) {
        const issues = [];
        try {
            // Jest 테스트 커버리지 검사
            try {
     ...
     * 보안 이슈 감지
     */
    async detectSecurityIssues(projectRoot) {
        const issues = [];
        try {
     ...
     * 성능 이슈 감지
     */
    async detectPerformanceIssues(projectRoot) {
        const issues = [];
        try {
            // 큰 파일 검사
            const fs = require('fs');
     ...
     * 문서화 이슈 감지
     */
    async detectDocumentationIssues(projectRoot) {
        const issues = [];
        try {
            const fs = require('fs');
            // README 파일 검사
     ...
     * 개선 방안 제안
     */
    suggestImprovements(issues, branch) {
        const improvements = [];
     ...
     * 브랜치 건강도 계산
     */
    calculateBranchHealth(issues) {
        const criticalCount = issues.filter(issue => issue.severity === 'critical').length;
        const highCount = issues.filter(issue => issue.severity === 'high').length;
        const totalCount = issues.length;
        if (criticalCount > 0)
            return 'critical';
        if (highCount > 2 || totalCount > 10)
     ...
     * 전체 프로젝트 건강도 계산
     */
    calculateOverallHealth(branchAnalyses) {
        ...
        if (criticalBranches > 0)
            return 'critical';
        if (needsAttentionBranches > branchAnalyses.length / 2)
            return 'needs_attention';
        if (needsAttentionBranches > 0)
            return 'good';
        return 'excellent';
    }
    /**
     * 브랜치 간 공통 이슈 식별
     */
    identifyCrossBranchIssues(branchAnalyses) {
        const crossBranchIssues = [];
        // 모든 브랜치에서 공통으로 발견되는 이슈 패턴 찾기
        const allIssues = branchAnalyses.flatMap(branch => branch.issuesByCategory.bug || []);
        // 중복된 이슈 패턴 찾기
     ...
     * 권장 액션 생성
     */
    generateRecommendedActions(branchAnalyses, crossBranchIssues) {
        const actions = [];
        // 공통 이슈 우선 처리
        ...
        // 가장 문제가 많은 브랜치 식별
        const problematicBranches = sortedBranches.filter(branch => branch.branchHealth === 'critical' || branch.branchHealth === 'needs_attention');
        if (problematicBranches.length > 0) {
            actions.push(`🔧 다음 브랜치들을 우선적으로 개선하세요: ${problematicBranches.map(b => b.branch).join(', ')}`);
        }
        // 카테고리별 액션 제안
        const allIssues = branchAnalyses.flatMap(branch => Object.values(branch.issuesByCategory).flat());
    ...
    detectDuplicateCode(projectRoot) {
        // 간단한 중복 코드 감지 (실제로는 더 정교한 알고리즘이 필요)
        const issues = [];
        // TODO: 중복 코드 감지 로직 구현
        return issues;
    }
    findApiDocFiles(projectRoot) {
        const fs = require('fs');
        const path = require('path');
        const apiDocFiles = [];
        const findFiles = (dir) => {
...
exports.GitBranchAnalysisService = GitBranchAnalysisService;
//# sourceMappingURL=GitBranchAnalysisService.js.map