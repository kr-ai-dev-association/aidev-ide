...
class ToolSpecBuilder {
     ...
     * 모든 툴 스펙 생성 (프롬프트에 포함)
     */
    static buildToolSpecs(allowedTools) {
        const specs = [];
        // create_file
        if (!allowedTools || allowedTools.includes(types_1.Tool.CREATE_FILE)) {
            specs.push({
        ...
        // update_file
        if (!allowedTools || allowedTools.includes(types_1.Tool.UPDATE_FILE)) {
            specs.push({
        ...
        // remove_file
        if (!allowedTools || allowedTools.includes(types_1.Tool.REMOVE_FILE)) {
            specs.push({
        ...
        // read_file
        if (!allowedTools || allowedTools.includes(types_1.Tool.READ_FILE)) {
            specs.push({
        ...
        // list_files
        if (!allowedTools || allowedTools.includes(types_1.Tool.LIST_FILES)) {
            specs.push({
        ...
        // search_files
        if (!allowedTools || allowedTools.includes(types_1.Tool.SEARCH_FILES)) {
            specs.push({
        ...
        // ripgrep_search
        if (!allowedTools || allowedTools.includes(types_1.Tool.RIPGREP_SEARCH)) {
            specs.push({
        ...
        // run_command
        if (!allowedTools || allowedTools.includes(types_1.Tool.RUN_COMMAND)) {
            specs.push({
    ...
    static buildToolPromptSection(allowedTools) {
        ...
        for (const spec of specs) {
            ...
            if (spec.name === types_1.Tool.UPDATE_FILE) {
