...
const IntentDetector_1 = require("../action/IntentDetector");
const ProjectManager_1 = require("../project/ProjectManager");
const ProjectDetector_1 = require("../project/ProjectDetector");
const types_2 = require("../project/types");
const InvestigationManager_1 = require("../investigation/InvestigationManager");
const SettingsManager_1 = require("../state/SettingsManager");
const services_1 = require("../../../services");
const AgentStateManager_1 = require("./AgentStateManager");
const task_1 = require("../context/prompts/task");
const fs = __importStar(require("fs/promises"));
const fsSync = __importStar(require("fs"));
const path = __importStar(require("path"));
const TestRunner_1 = require("./handlers/TestRunner");
const ResponseProcessor_1 = require("./handlers/ResponseProcessor");
const ToolExecutionCoordinator_1 = require("./handlers/ToolExecutionCoordinator");
const AgentConfig_1 = require("../../config/AgentConfig");
const StringUtils_1 = require("../../utils/StringUtils");
const phase_1 = require("../context/prompts/phase");
const rules_1 = require("../context/prompts/rules");
const generalAnalysis_1 = require("../context/prompts/analysis/generalAnalysis");
...
class ConversationManager {
    ...
    llmManager;
    responseProcessor;
    constructor(userOS, geminiApi, ollamaApi) {
        this.promptBuilder = new PromptBuilder_1.PromptBuilder(userOS, services_1.AiModelType.OLLAMA);
        this.contextManager = ContextManager_1.ContextManager.getInstance();
        this.llmManager = LLMManager_1.LLMManager.getInstance(geminiApi, ollamaApi);
        this.responseProcessor = new ResponseProcessor_1.ResponseProcessor(this.llmManager);
    }
    static getInstance(userOS = process.platform, geminiApi, ollamaApi) {
    ...
    setPromptBuilder(builder) { this.promptBuilder = builder; }
    ...
    async handleUserMessageAndRespond(options) {
        ...
        try {
            ...
            const promptOptions = {
                ...
                modelType: options.currentModelType || services_1.AiModelType.OLLAMA,
            ...
            // 5. 작업 타입에 따른 실행 분기
            if (options.promptType === PromptBuilder_1.PromptType.CODE_GENERATION) {
            ...
            WebviewBridge_1.WebviewBridge.hideLoading(webviewToRespond);
        }
    }
    /**
     * UI 초기 상태 설정
     */
    prepareUI(webview) {
        WebviewBridge_1.WebviewBridge.sendProcessingStep(webview, 'intent');
        WebviewBridge_1.WebviewBridge.sendProcessingStatus(webview, 'intent', '사용자 요청 분석 중...');
        // 새로운 요청이 시작되면 기존 작업 큐 초기화 및 UI 숨김
        const taskManager = TaskManager_1.TaskManager.getInstance();
        taskManager.clearPlanQueue();
        WebviewBridge_1.WebviewBridge.clearTaskQueue(webview);
    }
    /**
     * 사용자 의도 및 작업 타입 감지
     */
    async detectIntent(query) {
        const detector = new IntentDetector_1.IntentDetector(this.llmManager);
    ...
    async gatherContext(options, intent) {
        WebviewBridge_1.WebviewBridge.sendProcessingStep(options.webviewToRespond, 'assembling');
        WebviewBridge_1.WebviewBridge.sendProcessingStatus(options.webviewToRespond, 'assembling', '컨텍스트 수집 중...');
    ...
    async executeAgentLoop(systemPrompt, userParts, options, intent) {
        ...
        const maxTurns = AgentConfig_1.AgentConfig.MAX_TURNS;
        let turnCount = 0;
        let accumulatedUserParts = [...userParts];
        let testFixAttempts = 0; // 테스트 실패 시 자동 수정 시도 횟수
        const maxTestFixAttempts = await SettingsManager_1.SettingsManager.getInstance().getTestRetryCount(); // 설정에서 최대 시도 횟수 가져오기
        const isAutoTestRetryEnabled = await SettingsManager_1.SettingsManager.getInstance().isAutoTestRetryEnabled(); // 자동 테스트 재시도 설정 확인
        ...
        const taskManager = TaskManager_1.TaskManager.getInstance();
        const actionManager = ActionManager_1.ActionManager.getInstance();
        ...
        const investigationManager = InvestigationManager_1.InvestigationManager.getInstance();
        ...
        const hasNoIntent = !intent ||
            intent.confidence < AgentConfig_1.AgentConfig.MIN_INTENT_CONFIDENCE ||
            (!intent.subtype && !intent.category) ||
            (intent.subtype === null && !intent.category) ||
            (intent.reasoning && intent.reasoning.includes('인사') && intent.confidence < AgentConfig_1.AgentConfig.MIN_GREETING_CONFIDENCE);
        if (hasNoIntent && !hasActivePlan) {
            ...
            const greetingResponse = await this.llmManager.sendMessageWithSystemPrompt(systemPrompt, accumulatedUserParts, { signal: abortSignal });
            ...
            if (!cleanGreetingResponse || cleanGreetingResponse.trim().length < AgentConfig_1.AgentConfig.MIN_RESPONSE_LENGTH) {
                console.warn('[ConversationManager] Greeting response is empty or too short, using default response.');
                cleanGreetingResponse = AgentConfig_1.AgentConfig.DEFAULT_GREETING_MESSAGE;
            }
            // 최종 정제: 앞뒤 공백 제거
            ...
            WebviewBridge_1.WebviewBridge.receiveMessage(webviewToRespond, 'CODEPILOT', cleanGreetingResponse);
        ...
        const currentProjectForInitial = ProjectManager_1.ProjectManager.getInstance().getCurrentProject();
        ...
        if (workspaceRootForInitial) {
            try {
                ...
                hasExistingProject = entries.some(e => {
                    const name = e.name;
                    // 숨김/무시 대상
                    if (AgentConfig_1.AgentConfig.IGNORED_DIRECTORIES.includes(name))
                        return false;
        ...
        const initialState = hasActivePlan
            ? AgentStateManager_1.AgentPhase.EXECUTION
            : (isExecutionFirstTask && !hasExistingProject ? AgentStateManager_1.AgentPhase.EXECUTION : AgentStateManager_1.AgentPhase.INVESTIGATION);
        const stateManager = new AgentStateManager_1.AgentStateManager(initialState);
        ...
        if (initialState === AgentStateManager_1.AgentPhase.INVESTIGATION && !hasActivePlan) {
            try {
                const projectManager = ProjectManager_1.ProjectManager.getInstance();
                const inventory = await projectManager.buildProjectInventorySection(AgentConfig_1.AgentConfig.MAX_PROJECT_INVENTORY_FILES);
                if (inventory) {
        ...
        while (turnCount < maxTurns) {
            ...
            if (allItems.length > 0) {
                WebviewBridge_1.WebviewBridge.updateTaskQueue(webviewToRespond, allItems);
            }
            // 현재 활성 계획 아이템 확인
            ...
            if (currentPhase === AgentStateManager_1.AgentPhase.REVIEW) {
                ...
                if (this.reviewProcessed === reviewProcessedKey) {
                    ...
                    stateManager.transitionTo(AgentStateManager_1.AgentPhase.DONE);
                    break;
                }
                ...
                if (createdFiles.length > 0 || modifiedFiles.length > 0) {
                    ...
                    const verifiedSummary = await this.responseProcessor.generateVerifiedSummary('', // 원본 요약 없음 (시스템이 직접 생성)
                    createdFiles, modifiedFiles, workspaceRoot, activeSystemPrompt, accumulatedUserParts, abortSignal);
                    // 요약이 생성되었으면 UI에 출력
                    if (verifiedSummary && verifiedSummary.trim()) {
                        ...
                        WebviewBridge_1.WebviewBridge.receiveMessage(webviewToRespond, 'CODEPILOT', parsedSummary);
                    }
                    else {
                        ...
                        WebviewBridge_1.WebviewBridge.receiveMessage(webviewToRespond, 'CODEPILOT', defaultSummary);
                    }
                }
                else {
                    // 파일 변경이 없으면 기본 완료 메시지
                    WebviewBridge_1.WebviewBridge.receiveMessage(webviewToRespond, 'CODEPILOT', '작업이 완료되었습니다.');
                }
            ...
            if (currentPhase === AgentStateManager_1.AgentPhase.DONE) {
                console.log('[ConversationManager] DONE phase: All work completed.');
                break; // 이미 완료 상태이므로 루프 종료
            }
            const phaseLabel = currentPhase === AgentStateManager_1.AgentPhase.INVESTIGATION ? '[조사]' : '[실행]';
            const actionText = currentPhase === AgentStateManager_1.AgentPhase.INVESTIGATION ? '조사 및 분석' : '작업 진행';
            WebviewBridge_1.WebviewBridge.sendProcessingStep(webviewToRespond, 'thinking');
            WebviewBridge_1.WebviewBridge.sendProcessingStatus(webviewToRespond, 'thinking', `${phaseLabel}[생각 ${turnCount + 1}] ${statusPrefix}${actionText} 중...`);
            // 페이즈별 프롬프트 보정 및 도구 제한
            let activeSystemPrompt = systemPrompt;
            let allowedTools = undefined;
            if (currentPhase === AgentStateManager_1.AgentPhase.INVESTIGATION) {
                ...
                if (this.isExecutionFirstTask(intent, hasExecutionIntentEver, hasActivePlan)) {
                    activeSystemPrompt += (0, rules_1.getExecutionFirstRulePrompt)();
                }
            }
            else if (currentPhase === AgentStateManager_1.AgentPhase.EXECUTION) {
                // ⚠️ EXECUTION 단계에서는 설명 금지, 도구 호출만 허용
                // 🔥 핵심: LLM을 "DSL 컴파일러"처럼 사용 - Planning/Reasoning 금지, Execution만 허용
                activeSystemPrompt += (0, phase_1.getExecutionPhasePrompt)();
            }
            ...
            if (currentPhaseForExecution === AgentStateManager_1.AgentPhase.EXECUTION && currentPlanItem) {
                ...
                if (toolCallsFromPlanCreation.length > 0) {
                    ...
                    const currentProject = ProjectManager_1.ProjectManager.getInstance().getCurrentProject();
                    const workspaceRoot = currentProject?.root || '';
                    WebviewBridge_1.WebviewBridge.sendProcessingStep(webviewToRespond, 'executing');
                    WebviewBridge_1.WebviewBridge.sendProcessingStatus(webviewToRespond, 'executing', `${phaseLabel}도구 실행 중...`);
                    ...
                    ToolExecutionCoordinator_1.ToolExecutionCoordinator.sendToolExecutionResultsToUI(webviewToRespond, toolCallsFromPlanCreation, toolResults);
                    // read_file 결과를 preloadedFiles에 추가 (중복 읽기 방지)
                    toolCallsFromPlanCreation.forEach((call, index) => {
                        if (call.name === types_1.Tool.READ_FILE && toolResults[index]?.success) {
                    ...
                    // 파일 변경 추적 (요약 검증용)
                    ToolExecutionCoordinator_1.ToolExecutionCoordinator.trackFileChanges(toolCallsFromPlanCreation, toolResults, createdFiles, modifiedFiles);
                    // 현재 Plan Item 완료 처리
                    if (ToolExecutionCoordinator_1.ToolExecutionCoordinator.hasSideEffects(toolCallsFromPlanCreation, toolResults) && currentPlanItem) {
                        taskManager.updatePlanItemStatus(currentPlanItem.id, 'done');
                        WebviewBridge_1.WebviewBridge.updateTaskQueue(webviewToRespond, taskManager.listPlanItems());
                    }
                    ...
                    if (nextItem) {
                        ...
                        if (isAutoTestRetryEnabled) {
                            ...
                            const currentProject = ProjectManager_1.ProjectManager.getInstance().getCurrentProject();
                            const workspaceRoot = currentProject?.root || '';
                            const testResult = await TestRunner_1.TestRunner.runAutomatedTests(webviewToRespond, workspaceRoot, createdFiles, modifiedFiles);
                            if (testResult.success) {
                                // 테스트 통과 → REVIEW로 전환
                                console.log('[ConversationManager] Tests passed. Transitioning to REVIEW phase.');
                                stateManager.transitionTo(AgentStateManager_1.AgentPhase.REVIEW);
                                turnCount++;
                                ...
                                if (testFixAttempts < maxTestFixAttempts) {
                                    ...
                                    WebviewBridge_1.WebviewBridge.receiveMessage(webviewToRespond, 'System', `⚠️ 테스트 수정 시도 횟수 초과 (${maxTestFixAttempts}회). 최종 오류:\n${testResult.errorMessage || '알 수 없는 오류'}`);
                            ...
                            // 자동 테스트 수정이 off일 때는 테스트를 실행하지 않고 바로 REVIEW로 전환
                            console.log('[ConversationManager] All plan items completed. Auto-test retry is disabled. Transitioning to REVIEW phase.');
                            stateManager.transitionTo(AgentStateManager_1.AgentPhase.REVIEW);
                            turnCount++;
                            continue;
                        }
                                ...
                                if (isAutoTestRetryEnabled) {
                                    ...
                                    if (testResult.success) {
                                        ...
                                        if (testFixAttempts < maxTestFixAttempts) {
                                            ...
                                            console.log(`[ConversationManager] 테스트 수정 시도 횟수 초과 (${maxTestFixAttempts}회). REVIEW로 전환합니다.`);
                                            WebviewBridge_1.WebviewBridge.receiveMessage(webviewToRespond, 'System', (0, rules_1.getTestRetryExceededMessage)(maxTestFixAttempts, testResult.errorMessage || '알 수 없는 오류'));
                                            stateManager.transitionTo(AgentStateManager_1.AgentPhase.REVIEW);
                                            turnCount++;
                                            continue;
                                        }
                    ...
                    if (!hasAnyFileChange && currentPlanItem && currentPlanItem.kind !== 'investigation') {
                        ...
                        if (preloadedFiles.size > 0 || preloadedFilesContent.length > 0) {
                            ...
                            if (preloadedFilesContent.length > 0) {
                                ...
                                preloadedFilesContent.forEach(({ path, content }) => {
                                    ...
                                    const preview = StringUtils_1.StringUtils.truncateLines(content, AgentConfig_1.AgentConfig.MAX_FILE_PREVIEW_LINES, '\n... (파일이 길어 일부만 표시)');
                                    preloadedFilesContextForExecution += `\n**파일: ${path}**\n\`\`\`\n${preview}\n\`\`\`\n`;
                                });
                        ...
                        const planContextForExecution = `\n\n[EXECUTION PHASE - ABSOLUTE RULES (NO EXCEPTIONS)]\n` +
                            ...
                            `설명 없이 도구만 호출하세요.`;
                        const llmResponseForExecution = await this.llmManager.sendMessageWithSystemPrompt(activeSystemPrompt + planContextForExecution, accumulatedUserParts, { signal: abortSignal });
                        const cleanExecutionResponse = llmResponseForExecution.replace(/<think>[\s\S]*?<\/think>/gi, '').trim();
                        const toolCallsFromExecution = ToolParser_1.ToolParser.parseToolCalls(cleanExecutionResponse);
                        if (toolCallsFromExecution.length > 0) {
                            ...
                            ToolExecutionCoordinator_1.ToolExecutionCoordinator.sendToolExecutionResultsToUI(webviewToRespond, toolCallsFromExecution, toolResults);
                            // read_file 결과를 preloadedFiles에 추가 (중복 읽기 방지)
                            ...
                            // 파일 변경 추적 (요약 검증용)
                            ToolExecutionCoordinator_1.ToolExecutionCoordinator.trackFileChanges(toolCallsFromExecution, toolResults, createdFiles, modifiedFiles);
                            const resultSummary = ToolExecutionCoordinator_1.ToolExecutionCoordinator.createToolResultSummary(turnCount, toolCallsFromExecution, toolResults);
                            if (ToolExecutionCoordinator_1.ToolExecutionCoordinator.hasSideEffects(toolCallsFromExecution, toolResults) && currentPlanItem) {
                                taskManager.updatePlanItemStatus(currentPlanItem.id, 'done');
                                WebviewBridge_1.WebviewBridge.updateTaskQueue(webviewToRespond, taskManager.listPlanItems());
                            }
                            // 다음 계획 항목이 있으면 계속, 없으면 자동 테스트 후 REVIEW로 전환
                                ...
                                if (testResult.success) {
                                    ...
                                    if (isAutoTestRetryEnabled && testFixAttempts < maxTestFixAttempts) {
                                        ...
                                        if (isAutoTestRetryEnabled) {
                                            ...
                                            WebviewBridge_1.WebviewBridge.receiveMessage(webviewToRespond, 'System', `⚠️ 자동 테스트가 실패했습니다:\n${testResult.errorMessage || '알 수 없는 오류'}\n`);
                                        }
                                        // 실패해도 REVIEW로 전환하여 요약 생성
                                        stateManager.transitionTo(AgentStateManager_1.AgentPhase.REVIEW);
                                        turnCount++;
                            ...
                            if (textResponse && textResponse.trim().length > 0) {
                                ...
                                WebviewBridge_1.WebviewBridge.receiveMessage(webviewToRespond, 'CODEPILOT', textResponse);
            ...
            const llmResponse = await this.llmManager.sendMessageWithSystemPrompt(activeSystemPrompt + planContext, accumulatedUserParts, { signal: abortSignal });
            ...
            let cleanResponse = StringUtils_1.StringUtils.cleanText(llmResponse, {
                removeThinking: true,
            ...
            if (currentPhase === AgentStateManager_1.AgentPhase.EXECUTION) {
            ...
            const investigationDoneToken = ToolParser_1.ToolParser.parseInvestigationDone(llmResponse);
            ...
            if (currentPhase === AgentStateManager_1.AgentPhase.EXECUTION && cleanResponse.trim()) {
                // XML 태그가 있는지 확인
            ...
            const allToolCallsFromResponse = ToolParser_1.ToolParser.parseToolCalls(cleanResponse);
            ...
            for (const part of parts) {
                ...
                if (isTag) {
                    ...
                    if (part.toLowerCase().includes('<plan>')) {
                        ...
                        if (existingPlanItems.length > 0 && currentPhase === AgentStateManager_1.AgentPhase.EXECUTION) {
                        ...
                        // 수립 시작 알림 추가
                        WebviewBridge_1.WebviewBridge.sendProcessingStep(webviewToRespond, 'plan');
                        WebviewBridge_1.WebviewBridge.sendProcessingStatus(webviewToRespond, 'plan', '작업 계획 분석 및 파싱 중...');
                        const planItems = ToolParser_1.ToolParser.parsePlanItems(part);
                        if (planItems.length > 0) {
                            ...
                            const allToolCallsInResponse = ToolParser_1.ToolParser.parseToolCalls(cleanResponse, toolParseWarnings);
                            const investigationTools = [types_1.Tool.READ_FILE, types_1.Tool.LIST_FILES, types_1.Tool.SEARCH_FILES, types_1.Tool.RIPGREP_SEARCH];
                            const hasInvestigationToolCalls = allToolCallsInResponse.some(call => investigationTools.includes(call.name));
                            ...
                            if (currentPhase === AgentStateManager_1.AgentPhase.INVESTIGATION && hasInvestigationToolCalls) {
                                // 조사 도구만 필터링
                                const investigationToolCalls = allToolCallsInResponse.filter((call) => investigationTools.includes(call.name));
                                ...
                                const filteredToolCalls = investigationToolCalls.filter((call) => {
                                    if (call.name === types_1.Tool.READ_FILE) {
                                        ...
                                        const alreadyRead = accumulatedUserParts.some(part => part.text &&
                                ...
                                if (filteredToolCalls.length > 0) {
                                    ...
                                    for (let index = 0; index < toolResults.length; index++) {
                                        ...
                                        if (result.success) {
                                            ...
                                            if (toolCall.name === types_1.Tool.READ_FILE) {
                                            ...
                                            else if (toolCall.name === types_1.Tool.LIST_FILES) {
                                                hasListFiles = true;
                                            }
                            ...
                            if (investigationItems.length > 0 && !hasInvestigationToolCalls) {
                                ...
                                investigationItems.forEach(item => {
                                    if (item.detail) {
                                        ...
                                        const detailToolCalls = ToolParser_1.ToolParser.parseToolCalls(item.detail);
                                        detailToolCalls.forEach(toolCall => {
                                            // 조사 도구만 허용 (ripgrep_search, search_files 등)
                                            const investigationTools = [types_1.Tool.RIPGREP_SEARCH, types_1.Tool.SEARCH_FILES, types_1.Tool.LIST_FILES, types_1.Tool.READ_FILE];
                                            if (investigationTools.includes(toolCall.name)) {
                                ...
                                // 자동 조사 도구 실행 목록
                                const autoInvestigationToolCalls = [];
                                ...
                                if (autoInvestigationToolCalls.length > 0) {
                                    ...
                                    ToolExecutionCoordinator_1.ToolExecutionCoordinator.sendToolExecutionResultsToUI(webviewToRespond, autoInvestigationToolCalls, toolResults);
                                    // 조사 도구 실행 결과를 accumulatedUserParts에 추가
                                    ...
                                    for (let index = 0; index < toolResults.length; index++) {
                                        ...
                                        if (result.success) {
                                            ...
                                            if (toolCall.name === types_1.Tool.RIPGREP_SEARCH) {
                                            ...
                                            else if (toolCall.name === types_1.Tool.READ_FILE) {
                                                const filePath = toolCall.params.path;
                                                preloadedFiles.add(filePath);
                                                if (result.data?.content) {
                                                    accumulatedUserParts.push({
                                    ...
                                    if (hasExecutionItem) {
                                        ...
                                        const transitionResult = stateManager.transitionTo(AgentStateManager_1.AgentPhase.EXECUTION, transitionContext);
                            ...
                            console.log('└──────────────────────────────────────────────────┘\n');
                            WebviewBridge_1.WebviewBridge.sendProcessingStatus(webviewToRespond, 'plan', `작업 계획 수립 완료`);
                            ...
                            if (currentPhase === AgentStateManager_1.AgentPhase.INVESTIGATION) {
                                ...
                                const allToolCallsInThisTurn = ToolParser_1.ToolParser.parseToolCalls(cleanResponse);
                                // 조사 도구 목록
                                const investigationTools = [types_1.Tool.READ_FILE, types_1.Tool.LIST_FILES, types_1.Tool.SEARCH_FILES, types_1.Tool.RIPGREP_SEARCH];
                                // 실행 도구가 있는지 확인
                                const hasExecutionTool = allToolCallsInThisTurn.some(call => !investigationTools.includes(call.name));
                                ...
                                if (investigationItems.length > 0 && currentPhase === AgentStateManager_1.AgentPhase.INVESTIGATION) {
                                ...
                                const canTransitionToExecution = (investigationDone || allInvestigationItemsCompleted) && ((hasExecutionItems && isExecutionFirst) || // execution item이 있고 execution-first면 전환
                                ...
                                if (investigationDone && intent && intent.category === 'analysis' && !isExecutionFirstForAnalysis) {
                                    ...
                                    const analysisPrompt = systemPrompt + (0, generalAnalysis_1.getGeneralAnalysisPrompt)();
                                    const analysisResponse = await this.llmManager.sendMessageWithSystemPrompt(analysisPrompt, accumulatedUserParts, { signal: abortSignal });
                                    // 응답 정제: StringUtils 사용
                                    let cleanAnalysisResponse = StringUtils_1.StringUtils.cleanText(analysisResponse, {
                                        removeThinking: true,
                                    ...
                                    WebviewBridge_1.WebviewBridge.receiveMessage(webviewToRespond, 'CODEPILOT', cleanAnalysisResponse);
                                    // DONE으로 전환
                                    stateManager.transitionTo(AgentStateManager_1.AgentPhase.DONE, {});
                                ...
                                if (canTransitionToExecution) {
                                    // EXECUTION으로 전환
                                    const toolCallsInThisTurn = []; // Investigation에서는 조사 도구만 실행, 실행 도구는 EXECUTION에서 처리
                                    // 전환 컨텍스트 준비
                                    const transitionContext = {
                                        hasPlan: true,
                                    ...
                                    if (!hasExecutionIntentEver && !hasExecutionIntent && allInvestigationItemsCompleted) {
                                        ...
                                        if (intent && intent.category === 'analysis') {
                                            ...
                                            WebviewBridge_1.WebviewBridge.receiveMessage(webviewToRespond, 'System', '조사(Investigation)만 필요한 작업이 모두 완료되었습니다. 추가 실행(execution) 작업이 계획에 없으므로 에이전트를 종료합니다.');
                                        }
                                        WebviewBridge_1.WebviewBridge.sendProcessingStatus(webviewToRespond, 'done', '조사만 필요한 작업이 완료되었습니다.');
                                        stateManager.transitionTo(AgentStateManager_1.AgentPhase.DONE);
                                        return;
                                    }
                                    else {
                    ...
                    // 2-2. 진행 상황 업데이트 처리
                        ...
                        const progress = ToolParser_1.ToolParser.parseTaskProgress(part);
                        if (progress) {
                    ...
                    // 2-3. 도구 실행 처리
                        ...
                        const toolCallsFromPart = ToolParser_1.ToolParser.parseToolCalls(part);
                        ...
                        if (toolCalls.length > 0) {
                            ...
                            if (blockedCalls.length > 0) {
                                ...
                                if (currentPhase === AgentStateManager_1.AgentPhase.INVESTIGATION) {
                                    ...
                                    if (hasExecutionIntentInHistory || executionIntent) {
                                        ...
                                        if (transitionResult.success) {
                                            ...
                                            const executionToolCalls = toolCalls.filter(call => {
                                                ...
                                                const executionTools = [types_1.Tool.CREATE_FILE, types_1.Tool.UPDATE_FILE, types_1.Tool.REMOVE_FILE, types_1.Tool.RUN_COMMAND];
                                                const isExecutionTool = executionTools.includes(call.name);
                                                if (isExecutionTool) {
                                    ...
                                    if (currentPhase === AgentStateManager_1.AgentPhase.EXECUTION) {
                                        ...
                                        const allowedCalls = toolCalls.filter(call => !blockedCalls.some(blocked => blocked.name === call.name && JSON.stringify(blocked.params) === JSON.stringify(call.params)));
                            ...
                            if (currentPhase === AgentStateManager_1.AgentPhase.INVESTIGATION) {
                                const investigationTools = [types_1.Tool.READ_FILE, types_1.Tool.LIST_FILES, types_1.Tool.SEARCH_FILES, types_1.Tool.RIPGREP_SEARCH];
                                if (toolCalls.some(call => investigationTools.includes(call.name))) {
                                    hasInvestigationHistory = true;
                                }
                            }
                            ...
                            const deduplicatedCalls = toolCalls.filter(call => {
                                ...
                                if (call.name === types_1.Tool.RUN_COMMAND && call.params.command) {
                            ...
                            const phaseLabelExec = currentPhase === AgentStateManager_1.AgentPhase.INVESTIGATION ? '[조사]' : '[실행]';
                            WebviewBridge_1.WebviewBridge.sendProcessingStatus(webviewToRespond, 'executing', `${phaseLabelExec}[단계 ${turnCount + 1}] ${executingPrefix}${ToolExecutionCoordinator_1.ToolExecutionCoordinator.getToolLabel(finalDeduplicatedCalls[0].name)} 실행 중...`);
                            const currentProject = ProjectManager_1.ProjectManager.getInstance().getCurrentProject();
                            const workspaceRoot = currentProject?.root || '';
                            ...
                            ToolExecutionCoordinator_1.ToolExecutionCoordinator.sendToolExecutionResultsToUI(webviewToRespond, finalDeduplicatedCalls, toolResults);
                            // read_file 결과를 preloadedFiles에 추가 (중복 읽기 방지)
                            ...
                            // 파일 변경 추적 (요약 검증용)
                            ToolExecutionCoordinator_1.ToolExecutionCoordinator.trackFileChanges(finalDeduplicatedCalls, toolResults, createdFiles, modifiedFiles);
                            // 결과 요약 누적
                            const resultSummary = ToolExecutionCoordinator_1.ToolExecutionCoordinator.createToolResultSummary(turnCount, finalDeduplicatedCalls, toolResults);
                            turnResultsSummary += resultSummary;
                            // ⚠️ 핵심 수정: 테스트 실패 후 수정 시도한 경우, 도구 실행 후 자동으로 테스트 재실행
                            const hasRunCommand = finalDeduplicatedCalls.some(call => call.name === types_1.Tool.RUN_COMMAND);
                            const hasWriteTool = finalDeduplicatedCalls.some(call => [types_1.Tool.CREATE_FILE, types_1.Tool.UPDATE_FILE, types_1.Tool.REMOVE_FILE, types_1.Tool.RUN_COMMAND].includes(call.name));
                            // 🔥 문제 1 해결: 중복 테스트 실행 방지
                            // "All tasks completed" 경로에서만 테스트 실행하므로, 여기서는 테스트를 실행하지 않음
                            if (ToolExecutionCoordinator_1.ToolExecutionCoordinator.hasSideEffects(finalDeduplicatedCalls, toolResults)) {
                                ...
                                if (activeItemForDoneCheck && activeItemForDoneCheck.detail && activeItemForDoneCheck.kind !== 'investigation') {
                                    ...
                                    if (mentionedInCurrentItem.length > 0 && currentActiveItem) {
                                        ...
                                        const completedForItem = mentionedInCurrentItem.filter(file => allCompletedFilesForDone.some(completed => completed === file || completed.endsWith(file) || file.endsWith(completed))).length;
                                ...
                                const writeTools = [types_1.Tool.CREATE_FILE, types_1.Tool.UPDATE_FILE, types_1.Tool.REMOVE_FILE, types_1.Tool.RUN_COMMAND];
                                const hasWriteTool = deduplicatedCalls.some(call => writeTools.includes(call.name));
                                const isCodeModifyIntent = intent && intent.subtype === 'code_modify';
                                if (currentPhase === AgentStateManager_1.AgentPhase.EXECUTION && !hasWriteTool && isCodeModifyIntent) {
                    ...
                    if (responseText && responseText.trim()) {
                        ...
                        WebviewBridge_1.WebviewBridge.receiveMessage(webviewToRespond, 'CODEPILOT', responseText);
                    }
                }
            }
            // 3. 루프 종료 조건 확인 및 턴 관리
            const totalToolCalls = ToolParser_1.ToolParser.parseToolCalls(cleanResponse, toolParseWarnings);
            ...
            if (cleanResponse.includes('<run_command>')) {
                ...
                const hasRunCommand = totalToolCalls.some(c => c.name === types_1.Tool.RUN_COMMAND);
                if (!hasRunCommand) {
            ...
            const validPlanReceived = hasPlanTag && TaskManager_1.TaskManager.getInstance().listPlanItems().length > 0;
            ...
            if (totalToolCalls.length > 0 || validPlanReceived) {
                ...
                if (nextPendingItem) {
                    ...
                    if (isAutoTestRetryEnabled) {
                        ...
                        const currentProjectForTest = ProjectManager_1.ProjectManager.getInstance().getCurrentProject();
                        ...
                        if (testResult.success) {
                            ...
                            if (testFixAttempts < maxTestFixAttempts) {
                                ...
                                const errorPattern = TestRunner_1.TestRunner.extractErrorPattern(errorMessage);
                        ...
                        // 자동 테스트 수정이 off일 때는 테스트를 실행하지 않고 바로 REVIEW로 전환
                        console.log('[ConversationManager] All tasks completed. Auto-test retry is disabled. Transitioning to REVIEW phase.');
                        stateManager.transitionTo(AgentStateManager_1.AgentPhase.REVIEW);
                        turnCount++;
                        continue;
                    }
            ...
            if (currentPhase === AgentStateManager_1.AgentPhase.INVESTIGATION && totalToolCalls.length === 0 && !validPlanReceived && totalResponseText.length > 5) {
                ...
                if (hasNoIntent) {
                    ...
                    // 🔥 문제 해결: 'Assistant' sender는 webview에서 처리되지 않으므로 'CODEPILOT'으로 변경
                    WebviewBridge_1.WebviewBridge.receiveMessage(webviewToRespond, 'CODEPILOT', totalResponseText);
                    return; // 즉시 종료
                }
                // ⚠️ 핵심 수정: analysis intent이고 조사가 완료된 경우, 자연어 답변 허용
                ...
                if (intent && intent.category === 'analysis' && hasInvestigationHistory && !investigationDoneToken) {
                    ...
                    if (hasRipgrepResults) {
                        ...
                        let cleanResponse = StringUtils_1.StringUtils.cleanText(totalResponseText, {
                            removeThinking: true,
                        ...
                        if (cleanResponse && cleanResponse.length > 2) {
                            // 🔥 문제 해결: 'Assistant' sender는 webview에서 처리되지 않으므로 'CODEPILOT'으로 변경
                            WebviewBridge_1.WebviewBridge.receiveMessage(webviewToRespond, 'CODEPILOT', cleanResponse);
                            // DONE으로 전환
                            stateManager.transitionTo(AgentStateManager_1.AgentPhase.DONE, {});
                            console.log('[ConversationManager] Analysis response sent. Transitioning to DONE.');
                            break;
            ...
            if (investigationDoneToken && intent && intent.category === 'analysis' && currentPhase === AgentStateManager_1.AgentPhase.INVESTIGATION) {
            ...
            if (currentPhase === AgentStateManager_1.AgentPhase.EXECUTION && totalToolCalls.length === 0 && totalResponseText.length > 10) {
            ...
            if (shouldNudge) {
                ...
                const maxNudges = currentPhase === AgentStateManager_1.AgentPhase.INVESTIGATION ? 3 : 1;
                const nudgeCount = turnCount; // 간단한 추적 (실제로는 별도 카운터가 필요할 수 있음)
                if (currentPhase === AgentStateManager_1.AgentPhase.INVESTIGATION || turnCount === 0) {
                    if (currentPhase === AgentStateManager_1.AgentPhase.INVESTIGATION || nudgeCount < maxNudges) {
            ...
            if ((investigationDoneToken || hasRipgrepResultsForAutoAnswer) && intent && intent.category === 'analysis' && currentPhase === AgentStateManager_1.AgentPhase.INVESTIGATION) {
                if (investigationDoneToken) {
                ...
                if (hasRipgrepResults && ripgrepResults) {
                    ...
                    cleanAnalysisResponse = StringUtils_1.StringUtils.cleanText(analysisResponse, {
                        removeThinking: true,
                ...
                if (turnCount === 0) {
                    WebviewBridge_1.WebviewBridge.receiveMessage(webviewToRespond, 'System', '⚠️ 에이전트가 생각만 하고 실제 도구를 호출하지 않았습니다. 모델을 바꾸거나 다시 시도해 보세요.');
                }
                // EXECUTION phase에서 파일이 생성/수정되었으면 REVIEW로 전환
                if (currentPhase === AgentStateManager_1.AgentPhase.EXECUTION && (createdFiles.length > 0 || modifiedFiles.length > 0)) {
            ...
            // 단, 이미 REVIEW로 전환된 경우는 제외 (중복 실행 방지)
            if ((createdFiles.length > 0 || modifiedFiles.length > 0) && stateManager.getCurrentState() !== AgentStateManager_1.AgentPhase.REVIEW && isAutoTestRetryEnabled) {
                const currentProject = ProjectManager_1.ProjectManager.getInstance().getCurrentProject();
                const workspaceRoot = currentProject?.root || '';
                const testResult = await this.runAutomatedTests(webviewToRespond, workspaceRoot, createdFiles, modifiedFiles);
                if (testResult.success) {
        ...
        if (turnCount >= maxTurns) {
            WebviewBridge_1.WebviewBridge.updateProcessingStatus(webviewToRespond, '최대 턴 수 도달로 중단되었습니다.', 'error');
        }
        else {
            ...
            WebviewBridge_1.WebviewBridge.sendProcessingStatus(webviewToRespond, 'done', '모든 작업이 완료되었습니다.');
        }
    }
    /**
     * 일반 질의응답 처리
    ...
    async handleGeneralAsk(systemPrompt, userParts, options) {
        ...
        WebviewBridge_1.WebviewBridge.receiveMessage(options.webviewToRespond, 'CODEPILOT', response);
    }
    /**
    ...
    handleError(error, webview) {
        ...
        WebviewBridge_1.WebviewBridge.receiveMessage(webview, 'System', `오류 발생: ${errorMessage}`);
        WebviewBridge_1.WebviewBridge.updateProcessingStatus(webview, '오류가 발생했습니다.', 'error');
    }
    /**
    ...
    validateOutputFormat(response, phase) {
        ...
        ToolParser_1.ToolParser.parseToolCalls(response, validationWarnings);
        ...
        // 3. Phase별 허용 형식 검증
        if (phase === AgentStateManager_1.AgentPhase.INVESTIGATION) {
        ...
        else if (phase === AgentStateManager_1.AgentPhase.EXECUTION) {
     ...
     */
    async runAutomatedTests(webview, workspaceRoot, createdFiles, modifiedFiles) {
        try {
            // 검증 시작
            ...
            if (projectInfo.type === types_2.ProjectType.UNKNOWN) {
                ...
                if (llmResult && llmResult.type !== types_2.ProjectType.UNKNOWN) {
            ...
            for (const file of criticalFiles) {
                try {
                    ...
                    if (projectInfo.type === types_2.ProjectType.SPRING_BOOT && projectInfo.buildTool.toString().includes('gradle') && (file === 'build.gradle' || file === 'build.gradle.kts')) {
                    ...
                    if ((projectInfo.type === types_2.ProjectType.PYTHON || projectInfo.type === types_2.ProjectType.DJANGO || projectInfo.type === types_2.ProjectType.FLASK || projectInfo.type === types_2.ProjectType.FASTAPI) && (file === 'requirements.txt' || file === 'pyproject.toml')) {
...
중요: 명령어는 실제로 실행 가능해야 하며, 프로젝트 타입에 맞는 검증 도구를 사용해야 합니다.`;
            ...
            if (validationCmd) {
                ...
                try {
                    const executionManager = ExecutionManager_1.ExecutionManager.getInstance();
                    const result = await executionManager.executeCommand(validationCmd.command, { cwd: workspaceRoot, timeout: 15000 });
