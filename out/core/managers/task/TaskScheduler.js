...
class TaskScheduler {
    ...
    isProcessing = false;
    constructor(queue, maxConcurrent = 3) {
        this.queue = queue;
    ...
    async executeTask(task) {
        ...
        this.updateTaskStatus(task.id, types_1.TaskStatus.IN_PROGRESS);
        const handler = this.handlers.get(task.type);
        if (!handler) {
            console.error(`[TaskScheduler] No handler for task type: ${task.type}`);
            this.updateTaskStatus(task.id, types_1.TaskStatus.FAILED, {
                code: 'NO_HANDLER',
        ...
        try {
            ...
            // 상태 업데이트
            if (result.success) {
                this.updateTaskStatus(task.id, types_1.TaskStatus.COMPLETED);
            }
            else {
    ...
    pauseTask(taskId) {
        const task = this.queue.get(taskId);
        if (task && task.status === types_1.TaskStatus.IN_PROGRESS) {
    ...
    resumeTask(taskId) {
        const task = this.queue.get(taskId);
        if (task && task.status === types_1.TaskStatus.PAUSED) {
    ...
    cancelTask(taskId) {
        const task = this.queue.get(taskId);
        if (task && (task.status === types_1.TaskStatus.PENDING || task.status === types_1.TaskStatus.PAUSED)) {
