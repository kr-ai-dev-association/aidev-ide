...
class ActionManager {
    ...
    fileContextTracker;
    constructor() {
        this.registry = ActionRegistry_1.ActionRegistry.getInstance();
        this.validator = new ActionValidator_1.ActionValidator();
        this.mapper = new ActionMapper_1.ActionMapper();
    }
    /**
     * FileChangeTracker 설정
     */
    setFileChangeTracker(tracker) {
    ...
    checkPermissions(action) {
        ...
        const dangerousPermissions = [
            types_1.Permission.DELETE_FILE,
            types_1.Permission.NETWORK_ACCESS
        ];
        const hasDangerousPermission = action.permissions.some(p => dangerousPermissions.includes(p));
    ...
    async executeCodeGeneration(action) {
        ...
        try {
            ...
            // 파일 생성/수정
            await fs_1.promises.writeFile(absolutePath, code, 'utf8');
     ...
     */
    async updatePackageJsonFromImports(projectRoot, code, sourceFilePath) {
     ...
     */
    needsTypeDefinitions(packageName) {
    ...
    async executeFileOperation(action) {
        ...
        try {
            ...
            const sourceUri = vscode.Uri.file(path.isAbsolute(sourcePath)
            ...
            switch (operation) {
                case types_1.FileOperationType.CREATE:
                    ...
                    // 변경사항 추적 (파일 생성 전)
                    let createBeforeContent;
                    try {
                        createBeforeContent = await fs_1.promises.readFile(sourceUri.fsPath, 'utf8');
                    }
                    catch {
                    ...
                    // 파일 생성
                    await fs_1.promises.writeFile(sourceUri.fsPath, content, 'utf8');
                    // 변경사항 기록
                    if (this.fileChangeTracker) {
                        await this.fileChangeTracker.recordChange(sourceUri.fsPath, 'create', createBeforeContent, content, {
                ...
                case types_1.FileOperationType.UPDATE:
                    if (content === undefined) {
                        return {
                            success: false,
                            actionId: action.id,
                    ...
                    // 변경사항 추적 (파일 수정 전)
                    let updateBeforeContent;
                    try {
                        updateBeforeContent = await fs_1.promises.readFile(sourceUri.fsPath, 'utf8');
                    }
                    catch {
                        // 파일이 없으면 undefined 유지
                    ...
                    // 변경사항 기록
                    if (this.fileChangeTracker) {
                        await this.fileChangeTracker.recordChange(sourceUri.fsPath, 'modify', updateBeforeContent, content, {
                            taskId: action.metadata?.taskId,
                ...
                case types_1.FileOperationType.DELETE:
                    // 변경사항 추적 (파일 삭제 전)
                    let deleteBeforeContent;
                    try {
                        deleteBeforeContent = await fs_1.promises.readFile(sourceUri.fsPath, 'utf8');
                    }
                    catch {
                        // 파일이 없으면 undefined 유지
                        deleteBeforeContent = undefined;
                    }
                    await fs_1.promises.unlink(sourceUri.fsPath);
                    // 변경사항 기록
                    if (this.fileChangeTracker) {
                        await this.fileChangeTracker.recordChange(sourceUri.fsPath, 'delete', deleteBeforeContent, undefined, {
                            taskId: action.metadata?.taskId,
                ...
                case types_1.FileOperationType.RENAME:
                case types_1.FileOperationType.MOVE:
                    if (!targetPath) {
                        return {
                            success: false,
                            actionId: action.id,
                    ...
                    const targetUri = vscode.Uri.file(path.isAbsolute(targetPath)
                    ...
                    // 파일 이동
                    await fs_1.promises.rename(sourceUri.fsPath, targetUri.fsPath);
                    return {
                        success: true,
                        actionId: action.id,
     ...
     * 터미널 명령어 액션을 실행합니다
     */
    async executeTerminalCommand(action) {
