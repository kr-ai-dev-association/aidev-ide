...
class ActionMapper {
    ...
    mapResponse(llmResponse) {
        ...
        if (llmResponse.actions && llmResponse.actions.length > 0) {
            return {
                ...
                confidence: AgentConfig_1.AgentConfig.ACTION_CONFIDENCE.LLM_PROVIDED
            };
        }
        // 텍스트에서 액션 추출
    ...
    normalizeFileOperationDirectives(content) {
        ...
        normalized = normalized.replace(/\*\*(\s*(?:새 파일|수정 파일|삭제 파일):[^\n*]+)\*\*/g, (_, inner) => inner.trim());
        normalized = normalized.replace(/__(\s*(?:새 파일|수정 파일|삭제 파일):[^\n_]+)__/g, (_, inner) => inner.trim());
        // 2) 목록 기호(-, *) 제거: "- 새 파일: ..." → "새 파일: ..."
        normalized = normalized.replace(/(^|\n)[\t ]*[-*]\s*((?:새 파일|수정 파일|삭제 파일):\s*[^\n]+)/g, (_, prefix, directive) => `${prefix}${directive}`);
        return normalized;
    }
    /**
     * 코드 블록에서 액션을 추출합니다
    ...
    extractCodeBlocks(content) {
        ...
        while ((match = markdownHeaderPattern.exec(content)) !== null) {
            ...
            if (filePath && code) {
                const isDuplicate = actions.some(a => a.type === types_1.ActionType.CODE_GENERATION &&
    ...
    deduplicateTerminalCommands(actions) {
        ...
        for (const action of actions) {
            if (action.type !== types_1.ActionType.TERMINAL_COMMAND) {
                result.push(action);
                continue;
            }
    ...
    extractFileOperations(content) {
        ...
        while ((match = deletePattern.exec(content)) !== null) {
            ...
            if (this.isValidFilePath(filePath)) {
                ...
                actions.push(this.createFileOperationAction(types_1.FileOperationType.DELETE, filePath));
            }
            else {
        ...
        while ((match = renamePattern.exec(content)) !== null) {
            ...
            if (this.isValidFilePath(sourcePath) && this.isValidFilePath(targetPath)) {
                actions.push(this.createFileOperationAction(types_1.FileOperationType.RENAME, sourcePath, targetPath));
            }
        }
        // 파일 이동 패턴
        ...
        while ((match = movePattern.exec(content)) !== null) {
            ...
            const targetPath = this.sanitizeFilePath(match[2]);
            if (this.isValidFilePath(sourcePath) && this.isValidFilePath(targetPath)) {
                actions.push(this.createFileOperationAction(types_1.FileOperationType.MOVE, sourcePath, targetPath));
            }
        }
        return actions;
    }
    ...
    createCodeGenerationAction(filePath, code, language) {
        return {
            ...
            metadata: {
                source: 'llm',
                timestamp: Date.now(),
                confidence: AgentConfig_1.AgentConfig.ACTION_CONFIDENCE.FILE_CREATE
            }
        };
    }
    /**
    ...
    createTerminalCommandAction(command, cwd) {
        return {
            ...
            metadata: {
                source: 'llm',
                timestamp: Date.now(),
                confidence: AgentConfig_1.AgentConfig.ACTION_CONFIDENCE.TERMINAL_SAFE
            }
        };
    }
    /**
    ...
    createFileOperationAction(operation, sourcePath, targetPath, content) {
        const permissions = [types_1.Permission.READ_FILE];
        if (operation === types_1.FileOperationType.DELETE) {
            permissions.push(types_1.Permission.DELETE_FILE);
        }
        else {
            permissions.push(types_1.Permission.WRITE_FILE);
        }
        return {
            id: this.generateActionId(),
            ...
            metadata: {
                source: 'llm',
                timestamp: Date.now(),
                confidence: AgentConfig_1.AgentConfig.ACTION_CONFIDENCE.FILE_OPERATION
            }
        };
    }
    /**
    ...
    calculateConfidence(actions, content) {
        ...
        for (const action of actions) {
            totalConfidence += action.metadata?.confidence || AgentConfig_1.AgentConfig.ACTION_CONFIDENCE.DEFAULT;
        }
