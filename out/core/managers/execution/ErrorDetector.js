...
class ErrorDetector {
    ...
    detectError(output, source = 'terminal') {
        ...
        if (this.hasGeneralErrorKeywords(output)) {
            return this.createErrorInfo(types_1.ErrorType.UNKNOWN, null, output, source);
        }
        return null;
    }
    /**
    ...
    extractDetails(type, match, output) {
        const details = {};
        // 포트 번호 추출
        if (type === types_1.ErrorType.PORT_CONFLICT) {
    ...
    determineSeverity(type) {
        const severityMap = {
            [types_1.ErrorType.PORT_CONFLICT]: 'high',
            [types_1.ErrorType.COMMAND_NOT_FOUND]: 'high',
            [types_1.ErrorType.PERMISSION_DENIED]: 'high',
            [types_1.ErrorType.SYNTAX_ERROR]: 'medium',
            [types_1.ErrorType.RUNTIME_ERROR]: 'medium',
            [types_1.ErrorType.NETWORK_ERROR]: 'medium',
            [types_1.ErrorType.FILE_NOT_FOUND]: 'medium',
            [types_1.ErrorType.OUT_OF_MEMORY]: 'critical',
            [types_1.ErrorType.TIMEOUT]: 'low',
            [types_1.ErrorType.UNKNOWN]: 'low'
        };
    ...
    getSuggestion(type, details) {
        switch (type) {
            ...
            case types_1.ErrorType.OUT_OF_MEMORY:
    ...
    registerDefaultPatterns() {
        // PORT_CONFLICT
        this.addPattern(types_1.ErrorType.PORT_CONFLICT, [
            /port\s+\d+\s+(?:is\s+)?(?:already\s+)?(?:in\s+use|occupied)/i,
            /EADDRINUSE/i,
            /address\s+already\s+in\s+use/i
        ]);
        // COMMAND_NOT_FOUND
        this.addPattern(types_1.ErrorType.COMMAND_NOT_FOUND, [
        ...
        // PERMISSION_DENIED
        this.addPattern(types_1.ErrorType.PERMISSION_DENIED, [
        ...
        // SYNTAX_ERROR
        this.addPattern(types_1.ErrorType.SYNTAX_ERROR, [
            /SyntaxError/i,
        ...
        // RUNTIME_ERROR
        this.addPattern(types_1.ErrorType.RUNTIME_ERROR, [
            /RuntimeError/i,
            /TypeError/i,
            /ReferenceError/i,
        ...
        // NETWORK_ERROR
        this.addPattern(types_1.ErrorType.NETWORK_ERROR, [
            /ECONNREFUSED/i,
            /ETIMEDOUT/i,
            /ENOTFOUND/i,
            /network\s+error/i,
        ...
        // FILE_NOT_FOUND
        this.addPattern(types_1.ErrorType.FILE_NOT_FOUND, [
            /ENOENT/i,
            /no\s+such\s+file/i,
        ...
        // OUT_OF_MEMORY
        this.addPattern(types_1.ErrorType.OUT_OF_MEMORY, [
            /out\s+of\s+memory/i,
            /OutOfMemoryError/i,
        ...
        // TIMEOUT
        this.addPattern(types_1.ErrorType.TIMEOUT, [
            /timeout/i,
            /timed\s+out/i,
            /ETIMEDOUT/i
        ]);
