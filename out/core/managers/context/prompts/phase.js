...
exports.getInvestigationPrompt = getInvestigationPrompt;
exports.getExecutionPhasePrompt = getExecutionPhasePrompt;
const base_1 = require("./base");
// ==================== Investigation Phase ====================
function getInvestigationPrompt(userQuery) {
    const noMonologueRules = (0, base_1.getNoInternalMonologueRules)();
    const planFormatRules = (0, base_1.getPlanFormatRules)();
    const multiFileRules = (0, base_1.getMultiFileReadRules)();
    const noDuplicateRules = (0, base_1.getNoDuplicateReadRules)();
    const noThinkingLeakage = (0, base_1.getNoThinkingLeakageRules)();
    return `
## 역할: 조사 관리자 (코드의 셜록 홈즈)
...
- **조사 단계 (Investigation)**: 필요한 정보 수집, 최소 LLM 호출
  ...
  - ${(0, base_1.getNoDuplicateReadRules)().split('\n').slice(1).join('\n  - ')}
...
// ==================== Execution Phase ====================
function getExecutionPhasePrompt() {
    const noMonologueRules = (0, base_1.getNoInternalMonologueRules)();
    const noThinkingLeakage = (0, base_1.getNoThinkingLeakageRules)();
    return `\n\n⚠️ **실행 단계 - 절대 규칙 (예외 없음)**\n\n` +
