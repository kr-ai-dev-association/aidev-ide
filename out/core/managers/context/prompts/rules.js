...
exports.getExecutionFirstRulePrompt = getExecutionFirstRulePrompt;
exports.getErrorRetryPrompt = getErrorRetryPrompt;
exports.getSimpleErrorRetryPrompt = getSimpleErrorRetryPrompt;
exports.getTestRetryExceededMessage = getTestRetryExceededMessage;
const base_1 = require("./base");
// ==================== Execution First Rule ====================
function getExecutionFirstRulePrompt() {
    return `\n\n⚠️ **실행 우선 작업 규칙 (중요)**\n\n` +
...
function getErrorRetryPrompt(options) {
    ...
    return `\n[System] ⚠️ **자동 테스트가 실패했습니다.**\n\n**오류 내용:**\n${errorMessage}\n\n**⚠️ 중요: 오류를 먼저 분석하세요**\n` +
        ...
        `   - 빌드 설정 오류 → 설정 파일 수정\n\n` +
        `${(0, base_1.getFileCreationContext)()}\n\n` +
        `**절대 하지 말 것**:\n` +
...
function getSimpleErrorRetryPrompt(errorMessage) {
    return `\n[System] ⚠️ 자동 테스트가 실패했습니다. 다음 오류를 수정하세요:\n${errorMessage || '알 수 없는 오류'}\n\n${(0, base_1.getFileCreationContext)()}\n\n` +
        `필요하다면 run_command 도구를 사용하여 의존성을 설치하세요 (예: npm install, pip install -r requirements.txt, mvn install 등).\n`;
}
function getTestRetryExceededMessage(maxTestFixAttempts, errorMessage) {
