...
class ProjectManager {
    ...
    codeParserAdapter;
    constructor() {
        this.detector = new ProjectDetector_1.ProjectDetector();
        this.indexer = new ProjectIndexer_1.ProjectIndexer();
        this.parser = new ConfigParser_1.ConfigParser();
        // 코드 파서 초기화
     ...
     */
    async detectProjectTypeFromQuery(userQuery, projectRoot, geminiApi, ollamaApi, currentModelType, abortSignal) {
        try {
            ...
            if (localProjectType === 'unknown') {
                ...
                if (keywordType && supportedTypes.includes(keywordType)) {
                    ...
                    return {
                        ...
                        confidence: AgentConfig_1.AgentConfig.PROJECT_TYPE_CONFIDENCE.KEYWORD_BASED,
...
사용자 요청: "${userQuery}"`;
            ...
            let response;
            if (currentModelType === services_1.AiModelType.GEMINI && geminiApi) {
                response = await geminiApi.sendMessage(projectTypePrompt, undefined, { signal: abortSignal });
            }
            else if (currentModelType === services_1.AiModelType.OLLAMA && ollamaApi) {
            ...
            if (jsonMatch) {
                try {
                    ...
                    if (result.projectType && supportedTypes.includes(result.projectType)) {
                        const confidence = result.confidence || AgentConfig_1.AgentConfig.MIN_PROJECT_TYPE_CONFIDENCE;
                        // confidence가 임계값 미만이면 사용자 선택 필요
                        const needsUserSelection = confidence < AgentConfig_1.AgentConfig.MIN_PROJECT_TYPE_CONFIDENCE || (result.needsUserSelection === true);
     ...
     */
    detectProjectTypeFromKeywords(userQuery) {
        const lower = userQuery.toLowerCase();
        // Node.js / TypeScript 백엔드
        const hasNode = lower.includes('node.js') ||
            lower.includes('nodejs') ||
            lower.includes('node ') ||
            lower.includes(' node') ||
            userQuery.includes('노드');
        const hasTypeScript = lower.includes('typescript') ||
        ...
        // Spring / Java 백엔드
        const hasSpring = lower.includes('spring boot') ||
     ...
     * 빌드 명령어를 찾습니다
     */
    async findBuildCommands(projectRoot) {
        const root = projectRoot || this.projectRoot;
        if (!root) {
            throw new Error('Project root not set');
        }
     ...
     * 설정 파일을 파싱합니다
     */
    async parseConfig(configFile) {
        return await this.parser.parseConfig(configFile);
    }
    /**
     * 파일을 인덱싱합니다
     */
    async indexFiles(pattern, options) {
     ...
     * @returns 파일 경로 리스트
     */
    async getAllProjectFiles(abortSignal) {
        const root = this.projectRoot;
        if (!root) {
            throw new Error('Project root not set');
        }
     ...
     * 설정 파일들을 파싱합니다
     */
    async parseConfigFiles(projectRoot, projectType) {
        const configFiles = [];
        const configPaths = [];
        // 프로젝트 타입별 설정 파일
        switch (projectType) {
            case types_1.ProjectType.TYPESCRIPT:
            case types_1.ProjectType.JAVASCRIPT:
     ...
     * 빌드 명령어를 추출합니다
     */
    async extractBuildCommands(projectRoot, projectType) {
        const commands = {};
        switch (projectType) {
            case types_1.ProjectType.TYPESCRIPT:
            case types_1.ProjectType.JAVASCRIPT:
     ...
     * 의존성을 추출합니다
     */
    async extractDependencies(projectRoot, projectType) {
        switch (projectType) {
            case types_1.ProjectType.TYPESCRIPT:
            case types_1.ProjectType.JAVASCRIPT:
            case types_1.ProjectType.REACT:
     ...
     * 메타데이터를 추출합니다
     */
    async extractMetadata(projectRoot, projectType) {
        switch (projectType) {
            case types_1.ProjectType.TYPESCRIPT:
            case types_1.ProjectType.JAVASCRIPT:
            case types_1.ProjectType.REACT:
     ...
     * 프레임워크를 감지합니다
     */
    detectFramework(projectRoot, projectType) {
        // LLM이 파일을 읽어 판단하도록 하기 위해, 여기서는 간단한 추론만 수행
        if (projectType === types_1.ProjectType.NODE) {
            // package.json을 읽어 React, Vue, Angular 등 프레임워크를 추론
            // 이 부분은 LLM이 직접 파일을 읽어 판단하도록 위임
            return 'Node.js';
        }
        if (projectType === types_1.ProjectType.JAVA) {
     ...
     */
    getCodeParserAdapter() {
        return this.codeParserAdapter;
    }
    /**
     * 프로젝트 타입을 언어로 매핑합니다
     */
    mapTypeToLanguage(type) {
        const mapping = {
            [types_1.ProjectType.TYPESCRIPT]: 'TypeScript',
            [types_1.ProjectType.JAVASCRIPT]: 'JavaScript',
            [types_1.ProjectType.REACT]: 'TypeScript/JavaScript',
            [types_1.ProjectType.REACT_NATIVE]: 'TypeScript/JavaScript',
            [types_1.ProjectType.VUE]: 'TypeScript/JavaScript',
            [types_1.ProjectType.ANGULAR]: 'TypeScript',
            [types_1.ProjectType.NODE]: 'JavaScript',
            [types_1.ProjectType.SPRING_BOOT]: 'Java',
            [types_1.ProjectType.JAVA]: 'Java',
            [types_1.ProjectType.PYTHON]: 'Python',
            [types_1.ProjectType.DJANGO]: 'Python',
            [types_1.ProjectType.FLASK]: 'Python',
            [types_1.ProjectType.FASTAPI]: 'Python',
            [types_1.ProjectType.GO]: 'Go',
            [types_1.ProjectType.RUST]: 'Rust',
            [types_1.ProjectType.FLUTTER]: 'Dart',
            [types_1.ProjectType.PHP]: 'PHP',
            [types_1.ProjectType.CSHARP]: 'C#',
            [types_1.ProjectType.RUBY]: 'Ruby',
            [types_1.ProjectType.SWIFT]: 'Swift',
            [types_1.ProjectType.C_CPP]: 'C/C++',
            [types_1.ProjectType.UNKNOWN]: 'Unknown'
        };
     ...
     * 파일 트리를 빌드합니다
     */
    async buildFileTree(dir, projectRoot, maxDepth = 5, currentDepth = 0) {
     ...
     * @returns 분석 결과
     */
    async analyzeProject(analysisPrompt, llmApiClient) {
        try {
     ...
     * @returns 프로젝트 프로필
     */
    async getProjectProfile(storage) {
        if (!this.projectRoot) {
            return undefined;
        }
        ...
        // 스토리지에서 기존 프로필 로드
        if (storage) {
            const existing = storage.get(PROFILE_KEY);
            if (existing) {
                return existing;
            }
        }
     ...
     * 프로젝트 프로필을 스캔합니다
     */
    async scanProjectProfile(storage) {
        if (!this.projectRoot) {
            throw new Error('Project root not set');
        }
     ...
     */
    async scanWorkspaceForProfile(rootPath) {
     ...
     * 언어를 감지합니다 (프로필용)
     */
    detectLanguageForProfile(inputs) {
     ...
     */
    populateNodeFrameworks(pkg, frameworks) {
        const deps = { ...(pkg.dependencies || {}), ...(pkg.devDependencies || {}) };
        const addFramework = (name, confidence, evidence) => {
            frameworks.push({ framework: name, confidence, evidence: [evidence] });
        };
        const has = (library) => deps[library] !== undefined;
        if (has('react') || has('react-dom')) {
            addFramework('React', AgentConfig_1.AgentConfig.FRAMEWORK_CONFIDENCE.MAJOR, 'Found react dependency');
        }
        if (has('vue')) {
            addFramework('Vue', AgentConfig_1.AgentConfig.FRAMEWORK_CONFIDENCE.MAJOR, 'Found vue dependency');
        }
        if (has('@angular/core')) {
            addFramework('Angular', AgentConfig_1.AgentConfig.FRAMEWORK_CONFIDENCE.MAJOR, 'Found @angular/core dependency');
        }
        if (has('vite')) {
            addFramework('Vite', AgentConfig_1.AgentConfig.FRAMEWORK_CONFIDENCE.COMMON, 'Found vite dependency');
        }
        if (has('next')) {
            addFramework('Next.js', AgentConfig_1.AgentConfig.FRAMEWORK_CONFIDENCE.COMMON, 'Found next dependency');
        }
        if (has('@nestjs/core')) {
            addFramework('NestJS', AgentConfig_1.AgentConfig.FRAMEWORK_CONFIDENCE.COMMON, 'Found @nestjs/core dependency');
        }
        if (has('express')) {
            addFramework('Express', AgentConfig_1.AgentConfig.FRAMEWORK_CONFIDENCE.COMMON, 'Found express dependency');
        }
    }
    /**
     * Python 프레임워크를 감지합니다
     */
    populatePythonFrameworks(inputs, frameworks) {
        const dependencies = [];
        if (inputs.pyProject) {
            ...
            for (const key of Object.keys(tool)) {
                ...
                if (cfg?.dependencies && Array.isArray(cfg.dependencies)) {
                    dependencies.push(...cfg.dependencies.map((dep) => String(dep)));
                }
            }
        }
        ...
        const add = (framework, evidence) => {
            frameworks.push({ framework, confidence: AgentConfig_1.AgentConfig.FRAMEWORK_CONFIDENCE.COMMON, evidence: [evidence] });
        };
        const matched = (needle) => dependencies.some(dep => dep.toLowerCase().includes(needle));
        if (matched('fastapi'))
            add('FastAPI', 'Python dependency includes fastapi');
        if (matched('flask'))
            add('Flask', 'Python dependency includes flask');
        if (matched('django'))
            add('Django', 'Python dependency includes django');
    }
    /**
     * Java 프레임워크를 감지합니다
     */
    populateJavaFrameworks(inputs, frameworks) {
        const add = (framework, evidence) => frameworks.push({ framework, confidence: AgentConfig_1.AgentConfig.FRAMEWORK_CONFIDENCE.COMMON, evidence: [evidence] });
        const detect = (content, needle) => content && content.includes(needle);
        if (detect(inputs.pomXml, 'spring-boot-starter'))
            add('Spring Boot', 'pom.xml contains spring-boot-starter');
        if (detect(inputs.pomXml, 'spring-framework'))
            add('Spring', 'pom.xml references spring-framework');
        if (detect(inputs.buildGradle, 'spring-boot-starter'))
            add('Spring Boot', 'build.gradle contains spring-boot-starter');
        if (detect(inputs.buildGradleKts, 'spring-boot-starter'))
            add('Spring Boot', 'build.gradle.kts contains spring-boot-starter');
    }
    /**
     * 엔트리 포인트를 감지합니다
     */
    detectEntryPoints(pkg, scripts) {
        const entryPoints = [];
        if (!pkg)
            return entryPoints;
        if (pkg.module)
        ...
        if (pkg.bin && typeof pkg.bin === 'object') {
            const binValues = Object.values(pkg.bin).filter((value) => typeof value === 'string');
     ...
     * 프로젝트 파일 인벤토리를 빌드합니다
     */
    async buildProjectInventorySection(maxEntries = 400) {
        try {
