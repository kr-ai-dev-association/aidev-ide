...
const ErrorManager_1 = require("../error/ErrorManager");
const AutoFix_1 = require("../error/AutoFix");
const types_1 = require("../error/types");
const SettingsManager_1 = require("../state/SettingsManager");
const SafeSettingsHelper_1 = require("../../utils/SafeSettingsHelper");
...
class TerminalManager {
    ...
    extensionContext;
    daemonProcessRef = null;
    daemonSocketPath = path.join(os.tmpdir(), 'terminal-daemon.sock');
    daemonOutputChannel;
    daemonHealthCheckInterval = null;
    daemonStartTimeout = null;
    isDaemonStarting = false;
    // terminal/terminalManager.ts 큐 관리 기능
    priorityQueue = [];
    normalQueue = [];
    isProcessingQueue = false;
    queuePausedForLongRunning = false;
    pendingCommands = [];
    currentCommandIndex = 0;
    currentWorkingDirectory = undefined;
    isWaitingForInput = false;
    errorRetryCount = 0;
    sameErrorRetryCount = 0;
    lastFailedCommand = undefined;
    lastErrorOutput = undefined;
    currentWebview = undefined;
    llmApiClient = undefined; // LLMApiClient 타입
    userOS = 'unknown';
    summarySent = false;
    static FILE_OP_PREFIX = '__AIDEV_FILE_OP__::';
    static MAX_ERROR_RETRIES = 5;
    // 손상된 PowerShell 스크립트 저장용 맵
    corruptedPowerShellScripts = new Map();
    constructor() {
        this.history = new TerminalHistory_1.TerminalHistory();
    ...
    createTerminal(options) {
        ...
        // TerminalSession 생성
        const session = new TerminalSession_1.TerminalSession(sessionId, vscodeTerminal, options);
    ...
    async executeCommand(command, options) {
        ...
        if (options?.captureOutput) {
            try {
                ...
                // 결과 업데이트
                session.updateCommandResult(commandId, result.exitCode ?? -1, result.duration, {
    ...
    static codePilotTerminal;
    static outputLogEnabled = true;
    static captureOutputChannel;
    /**
     * CODEPILOT 터미널을 가져오거나 생성합니다 (terminal/terminalManager.ts 호환)
     */
    static getCodepilotTerminal(projectRoot, alwaysNew = true) {
        if (alwaysNew) {
            const name = `codepilot Terminal ${++TerminalManager.terminalSeq}`;
            console.log(`[TerminalManager] 새로운 터미널 생성: ${name}`);
            const terminalOptions = { name };
            if (projectRoot) {
        ...
        if (!TerminalManager.codePilotTerminal || TerminalManager.codePilotTerminal.exitStatus !== undefined) {
            ...
            const terminalOptions = { name: 'codepilot Terminal' };
            if (projectRoot) {
                terminalOptions.cwd = projectRoot;
                console.log(`[TerminalManager] 터미널 작업 디렉토리 설정: ${projectRoot}`);
            }
     ...
     */
    static setOutputLogEnabled(enabled) {
     ...
     */
    static extractBashCommandsFromLlmResponse(llmResponse) {
        // 폴백: 간단한 버전
        const commands = [];
     ...
     */
    static executeBashCommandsFromLlmResponse(llmResponse, projectRoot) {
        // 폴백: 간단한 버전 사용
     ...
     */
    static executeBashCommand(command, projectRoot) {
     ...
     */
    static hasBashCommands(llmResponse) {
     ...
     */
    enqueueCommandsBatch(commands, priority = false, projectRoot) {
     ...
     */
    setErrorCorrectionServices(llmApiClient, webview) {
     ...
     */
    async executeFileOpFromToken(token) {
        try {
            ...
            try {
                ...
                errorManager.captureError(types_1.ErrorSource.TERMINAL, message, {
                    command: 'file-op',
                    cwd: errorPath
                }).catch(() => { });
            }
     ...
     */
    async handleInteractiveCommand(command, projectRoot) {
        // 명령어 실행 중지 상태 확인
        if (globalThis.terminalMonitorService && globalThis.terminalMonitorService.isExecutionStopped()) {
        ...
        if (isDevLong) {
            try {
                ...
                try {
                    const aidevTerminals = vscode.window.terminals.filter(t => t.name.startsWith('codepilot Terminal') && t.exitStatus === undefined);
        ...
        if (shouldUseTerminal) {
            ...
            if (this.isInteractiveCommand(lower)) {
                ...
                vscode.window.showInformationMessage(`codepilot: 대화형 명령어 실행됨 - ${command}\n기본 응답이 자동으로 제공됩니다.`, { modal: false });
            }
            else {
        ...
        try {
            ...
            (0, utils_1.debugLog)(`TerminalManager: execute -> ${finalCommand}`);
            terminal.show(true);
            const runPromise = this.executionManager.runCommandCapture(finalCommand, { cwd, shell: selectShellForCapture(finalCommand) || true }, (data) => {
                const osAdapter = this.executionManager.getOSAdapter();
                ...
                try {
                    const errorManager = ErrorManager_1.ErrorManager.getInstance();
                    errorManager.captureError(types_1.ErrorSource.TERMINAL, decoded, {
            ...
            if (isDevLong) {
                runPromise.then(async (res) => {
                    if (res.code !== 0) {
                        ...
                        const isAutoCorrectionEnabled = await SafeSettingsHelper_1.SafeSettingsHelper.isAutoCorrectionEnabled();
                        let retrySuccess = false;
                        if (allowAutoCorrection && isAutoCorrectionEnabled) {
                            retrySuccess = await this.tryAutoFixWithCore(cleanCommand, errorOutput, cwd || '', terminal.name);
                        }
                        if (!retrySuccess && (allowAutoCorrection && isAutoCorrectionEnabled)) {
                            retrySuccess = await this.handleCommandError(cleanCommand, errorOutput, cwd || '', async (correctedCommand) => {
            ...
            if (hasError) {
                ...
                if (result.stderr) {
                    ...
                    channel.appendLine(`Stderr: ${decodedStderr}`);
                    (0, utils_1.debugLog)(`TerminalManager: stderr -> ${decodedStderr.substring(0, 2000)}`);
                }
                if (result.stdout && hasErrorInStdout) {
                    ...
                    (0, utils_1.debugLog)(`TerminalManager: stdout(error) -> ${decodedStdout.substring(0, 2000)}`);
                }
                channel.show(true);
                ...
                (0, utils_1.debugLog)(`TerminalManager: error detected, exit=${result.code}, err=${hasErrorInStderr}, outErr=${hasErrorInStdout}`);
                console.log(`[TerminalManager] 오류 출력 길이: stderr=${(result.stderr || '').length}, stdout=${(result.stdout || '').length}`);
                const isAutoCorrectionEnabled = await SafeSettingsHelper_1.SafeSettingsHelper.isAutoCorrectionEnabled();
                let retrySuccess = false;
                if (isAutoCorrectionEnabled) {
                    retrySuccess = await this.tryAutoFixWithCore(cleanCommand, errorOutput, cwd || '');
                }
                if (!retrySuccess && isAutoCorrectionEnabled) {
                    retrySuccess = await this.handleCommandError(cleanCommand, errorOutput, cwd || '', async (correctedCommand) => {
                        channel.appendLine(`\n===== Retrying with corrected command: ${correctedCommand} =====`);
            ...
            (0, utils_1.debugLog)(`TerminalManager: success -> ${cleanCommand} (exit code: ${result.code})`);
            if (result.code === 0) {
                return true;
            }
            return false;
            ...
            const result = await this.executionManager.runCommandCapture(this.normalizeEncodedPowerShellCommand(cleanCommand), { cwd, shell: true }, (chunk) => {
                chunk.split(/\r?\n/).forEach(line => {
                ...
                chunk.split(/\r?\n/).forEach(line => {
                    ...
                    try {
                        const errorManager = ErrorManager_1.ErrorManager.getInstance();
                        errorManager.captureError(types_1.ErrorSource.TERMINAL, cleaned, {
     ...
     */
    async tryAutoFixWithCore(failedCommand, errorOutput, cwd, terminalName) {
        try {
            const errorManager = ErrorManager_1.ErrorManager.getInstance();
            const autoFixService = AutoFix_1.AutoFix.getInstance();
            const parsedError = await errorManager.captureError(types_1.ErrorSource.TERMINAL, errorOutput, {
     ...
     */
    async getCorrectedCommand(failedCommand, errorOutput, cwd) {
...
파일 작업이 필요 없는 경우 fileOperations는 빈 배열 []로 설정하세요.`;
            ...
            (0, utils_1.debugLog)(`TerminalManager:getCorrectedCommand LLM response length=${response.length}`);
            ...
            if (jsonMatches) {
                for (const jsonStr of jsonMatches) {
                    try {
                        ...
                        (0, utils_1.debugLog)(`TerminalManager: parsed JSON -> cmd=${hasValidCommand}, fileOps=${hasFileOps}`);
                        if (hasValidCommand || (hasFileOps && (!result.correctedCommand || result.correctedCommand === null))) {
                            ...
                            if (result.fileOperations && Array.isArray(result.fileOperations)) {
                                const isPomOrGradle = (p) => /pom\.xml|build\.gradle/i.test(p);
                                for (const op of result.fileOperations) {
                                    ...
                                    if (opType && op.path && (opType === 'create' || opType === 'modify' || opType === 'delete')) {
                                        ...
                                        const finalOpType = shouldForceModify ? 'modify' : opType;
                                        // 경로 검증
                                        const invalidChars = /[<>:"|?*]/;
     ...
     */
    async repairCorruptedPowerShellScript(corruptedScript, errorOutput) {
...
JSON 형식으로 응답해주세요:
{
  ...
  "confidence": 0.9}`;
     ...
     * 명령어를 큐에 추가합니다 (terminal/terminalManager.ts 호환)
     */
    enqueueCommands(commands, priority = false) {
        if (priority) {
     ...
     */
    async handleCommandError(failedCommand, errorOutput, cwd, onRetry) {
        // 동일한 오류가 반복되는지 확인
        if (this.lastFailedCommand === failedCommand && this.lastErrorOutput === errorOutput) {
            ...
            if (this.sameErrorRetryCount >= 2) {
                ...
                if (this.currentWebview) {
                    try {
                        this.currentWebview.postMessage({ command: 'hideProcessingSteps', step: 'error_correction' });
                    }
                    catch { }
                    try {
        ...
        if (this.errorRetryCount >= TerminalManager.MAX_ERROR_RETRIES) {
            ...
            if (this.currentWebview) {
                ...
                (0, utils_1.debugLog)('TerminalManager: hideProcessingSteps (max retry exceeded)');
            }
            this.errorRetryCount = 0;
            this.sameErrorRetryCount = 0;
            this.lastFailedCommand = undefined;
        ...
        if (!correctionResult) {
            ...
            if (this.currentWebview) {
                ...
                (0, utils_1.debugLog)('TerminalManager: hideProcessingSteps (no correctionResult)');
            }
            return false;
        }
        ...
        if (!correctedCommand && fileOperations.length > 0) {
            ...
            if (this.currentWebview) {
                ...
                (0, utils_1.debugLog)('TerminalManager: hideProcessingSteps (file ops only)');
            }
            this.errorRetryCount = 0;
            return true;
        }
        const isValidCorrected = async (cmd) => {
            if (!cmd)
                return false;
            const t = cmd.trim();
            if (!t)
        ...
        if (isValid && correctedCommand) {
            ...
            if (this.currentWebview) {
                ...
                (0, utils_1.debugLog)('TerminalManager: hideProcessingSteps (success)');
            }
            return true;
        }
        else {
            ...
            if (this.currentWebview) {
                ...
                (0, utils_1.debugLog)('TerminalManager: hideProcessingSteps (invalid corrected command)');
            }
            this.errorRetryCount = 0;
            return false;
        }
     ...
     */
    async sendErrorCorrectionSummary() {
        if (!this.currentWebview) {
...
*이 보고서는 codepilot의 자동 오류 수정 시스템에 의해 생성되었습니다.*`;
     ...
     */
    isLongRunningDevCommand(command) {
        const lower = command.toLowerCase();
     ...
     */
    decodeTerminalOutput(text, opts) {
        const osAdapter = this.executionManager.getOSAdapter();
     ...
     */
    normalizeEncodedPowerShellCommand(cmd) {
        try {
            ...
            const fixCorruptedScript = (script) => {
