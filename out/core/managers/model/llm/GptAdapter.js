...
exports.GptAdapter = void 0;
const ILLMAdapter_1 = require("./ILLMAdapter");
const services_1 = require("../../../../services");
const PromptComposer_1 = require("../../context/prompts/PromptComposer");
const base_1 = require("../../context/prompts/base");
const FrameworkPromptBuilder_1 = require("../../context/prompts/framework/FrameworkPromptBuilder");
/**
 * GPT LLM 어댑터
...
class GptAdapter {
    llmId = 'gpt';
    llmName = 'GPT';
    modelName = 'gpt'; // 실제 모델명은 프론트엔드에서 선택
    // ==================== 프롬프트 생성 ====================
    buildSystemPrompt(context) {
        ...
        try {
            ...
            const composerOptions = {
                ...
                modelType: services_1.AiModelType.OLLAMA, // GptAdapter이므로 Ollama 기본 타입 사용
                taskType: undefined, // 컨텍스트에서 추론 불가능하므로 optional
            ...
            const parts = [];
            parts.push((0, base_1.getAgentRole)());
            parts.push(PromptComposer_1.PromptComposer.getOSPrompt(context.osName));
            parts.push((0, base_1.getCodeGenerationGuide)());
            // GPT 특화 프롬프트는 PromptComposer에서 이미 포함됨
            if (context.projectType) {
                parts.push(FrameworkPromptBuilder_1.FrameworkPromptBuilder.buildProjectContextPrompt(context.projectType, context.framework));
            }
            return parts.join('\n\n');
        }
    }
    ...
    buildCodeGenerationPrompt(context) {
        const parts = [
            (0, base_1.getCodeGenerationGuide)(),
            '',
        ...
        // GPT 특화: 명확한 출력 형식 지정
        parts.push('', '## 출력 형식:', (0, base_1.getDefaultOutputFormat)());
        return parts.join('\n');
    }
    buildErrorCorrectionPrompt(context) {
        const parts = [
            (0, base_1.getErrorCorrectionGuide)(),
            '',
    ...
    buildCommandExecutionPrompt(context) {
        const shellPrompt = (0, base_1.buildShellSpecificPrompt)(context.shellType);
        const parts = [
            (0, base_1.getCommandExecutionGuide)(),
            '',
            (0, base_1.buildShellSpecificPrompt)(context.shellType),
            '',
            `프로젝트 타입: ${context.projectType}`,
    ...
    buildApiRequestBody(prompt, options) {
        return {
    ...
    getSupportedFeatures() {
        return [
            ILLMAdapter_1.LLMFeature.STREAMING,
            ILLMAdapter_1.LLMFeature.CODE_GENERATION,
            ILLMAdapter_1.LLMFeature.ERROR_CORRECTION,
            ILLMAdapter_1.LLMFeature.MULTI_TURN,
            ILLMAdapter_1.LLMFeature.FILE_OPERATIONS,
            ILLMAdapter_1.LLMFeature.COMMAND_EXECUTION,
        ];
    }
    supportsFeature(feature) {
        return this.getSupportedFeatures().includes(feature);
    }
    getModelSpecificSettings() {
        return {
            temperature: 0.7,
            topP: 0.9,
            maxTokens: 4096,
...
exports.GptAdapter = GptAdapter;
//# sourceMappingURL=GptAdapter.js.map